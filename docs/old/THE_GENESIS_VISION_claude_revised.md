...
# How the Methodology Is Constructed

Spec-Driven Development is not a single document. It is three independent bodies of work, each with a distinct responsibility. Understanding the separation is what makes the approach defensible under examination, and what lets each layer evolve without destabilising the others.

```mermaid
graph TB
    subgraph L1["Layer 1 — Methodology (the WHAT)"]
        SM[specification_methodology<br/>SPEC_METHOD · GRAPH_METHOD<br/>TICKET_METHOD · RELEASE_METHOD]
    end
    subgraph L2["Layer 2 — Algebra & Engine (the HOW)"]
        GTL[GTL<br/>graph-first<br/>declarative language]
        ABG[ABG<br/>event-sourced<br/>runtime]
        GTL -.interprets.-> ABG
    end
    subgraph L3["Layer 3 — Implementation (the PROCESS)"]
        ODD[odd_sdlc<br/>7-stage SDLC<br/>as a GTL Module]
    end
...
Authority flows downward. The methodology defines the rules. The algebra defines the vocabulary and the engine that enforces those rules at runtime. The implementation is one lawful expression of the methodology through that algebra. Replace the implementation and the methodology still stands. Replace the algebra and the implementation must be rebuilt. Replace the methodology and the meaning of "conforming" changes.

---

## The Methodology Layer — `specification_methodology`

The methodology layer defines what Spec-Driven Development means before any code is written. It is the constitutional source. Everything downstream derives from it and is required to remain consistent with it.

The central document is `SPEC_METHOD.md`. It states the litmus tests that every specification artifact must satisfy: live surface immutability (the authoritative document is the current one, not a migration narrative), trace closure (every downstream claim must resolve to an upstream source), anti-drift (informal notes do not outrank constitutional text), and installed-dev proof (methodology changes must be demonstrated on a real workspace before they are accepted).

The methodology layer carries its companions:

- `GRAPH_METHOD.md` — how specification content relates as a directed graph rather than a linear document set
- `TICKET_METHOD.md` — how defects, regressions, and change proposals are recorded as first-class artifacts with resolution criteria
- `RELEASE_METHOD.md` — how a set of artifacts becomes a release with stable identity and reproducible contents
- `WRITING_GUIDE.md` — how specification text is written so it remains useful to humans and machines over time

These documents are the "rules of the game." They are the reason the methodology can be audited at all. They state what a conforming specification looks like, what a conforming change looks like, and what a conforming release looks like. They do not describe software. They describe what specifications must do to be trusted.

The methodology layer is deliberately independent of any particular technology. A project built in Python, Scala, or a language that does not yet exist can be governed by the same methodology — because the methodology governs specifications, not code.

---

## The Algebra and Engine — `abiogenesis`

The methodology describes the rules. Something has to enforce them at runtime. That is `abiogenesis`.

`abiogenesis` contributes two distinct artifacts. They are separated on purpose and the separation is load-bearing.

**GTL — the Graph Topology Language.** GTL is the vocabulary in which specifications are written so they can be interpreted by a machine. Its primitives are graph-first: `Graph`, `Node`, `GraphVector`, `Context`, `Operator`, `Evaluator`, `Rule`, `GraphFunction`, `RefinementBoundary`, `CandidateFamily`, `Job`, `Role`, `Module`. Work is declared as a graph of lawful transitions between typed nodes. Convergence is declared as evaluators that attest whether a transition closes. Alternatives are declared as candidate families rather than hidden branches. The language is engine-agnostic; it does not assume how it will be executed.

**ABG — the runtime.** ABG is the canonical interpreter for GTL. It is event-sourced (every state change is an append-only event), provenance-carrying (every artifact records what produced it and under what constraints), replayable (the entire system state is derivable from the event stream), and governed by three evaluator regimes:

- `F_D` — deterministic checks (schemas, hashes, tests, coverage, and any program or script whose result is reproducible)
...
The rule that keeps the separation honest: deterministic truth closes first. An AI-produced artifact is not accepted because the AI says so; it is accepted when the deterministic evaluators attest that it satisfies the specification. A human approval does not override a deterministic failure. An AI construction does not bypass a human gate.

The value of the `abiogenesis` layer is that it makes the methodology operational. The methodology can state "every requirement must appear in a test before delivery advances." The engine is what stops delivery when it does not. The gate is not a slogan — it is a replayable event in an append-only log, signed by the evaluator that produced it.

---

## The Implementation — `odd_sdlc` (Outcome-Driven Development)

The methodology and the engine are reusable across many delivery shapes. `odd_sdlc` is one such delivery shape, and the one this project ships with. It is the embodiment of **Outcome-Driven Development (ODD)** — the position that a project is an active worksite, not a one-shot pipeline that ends at release.

A linear chain of stages — specify, design, build, test, ship — describes how work moves the first time. It does not describe how a real system is operated. Real systems return evidence after release, drift in production, surface gaps that were not visible until a user touched them, and require lawful re-entry rather than emergency patches. ODD treats the project as a worksite that cycles through request, gate, specify, design, implement, qualify, release, deploy, observe, return, retrofit, and relaunch — and any of those acts may legitimately re-open work upstream.
...
Four properties define what ODD is for, and they are the properties an organisation can demand from any AI-assisted delivery practice:

**Traceability.** Every requirement carries an identifier from the moment it is specified. The identifier is recorded in the design that addresses it, tagged in the code that implements it, tagged in the test that validates it, and bound to the production signal that monitors it. At any point in the worksite — first delivery, fifth retrofit, an audit two years later — the chain from a live behaviour to the requirement that authorised it can be read off the record.

**Recoverability.** The worksite maintains a machine-readable closure register. Every live requirement is classified — realised, partially realised, planned, specified, missing — and the classification is rebuilt on every run from the event log. Unresolved requirements remain visible and binding across runs until they are realised, withdrawn, or superseded. There is no version of "I think we did that one" that survives the next replay.

**Observability.** Disturbance is a first-class lawful act, not an exception path. When something the system was supposed to satisfy stops being satisfied — in development or in production — the worksite emits an explicit observation, classifies it (defect, drift, ambiguity, missing capability), selects a re-entry layer, and resumes work from there. Observation, triage, route selection, and re-specification are distinct recorded acts. They are not collapsed into a single opaque "fix it" step.

**Governance over agents.** The AI agents that construct, evaluate, and disambiguate work inside the worksite are constrained by the same rules as the human contributors. They operate within declared graph functions, against declared evaluators, under the `F_D` / `F_P` / `F_H` regime contract. An agent does not choose what stage it is in, does not invent its own gate criteria, and does not override a deterministic failure with a confident answer. Every agent action is an event with provenance: which run, which call, which manifest, which evaluators it answered to, what it produced, and what closed.

The result is that "AI is doing the work" becomes a verifiable claim rather than a slogan. The agent's freedom is bounded by the graph it operates inside; the operator's freedom is bounded by the gates the methodology declares; the worksite's freedom is bounded by what the specification authorises. None of those boundaries are enforced by trust. They are enforced by the engine that refuses to advance when they are violated.

This is what makes `odd_sdlc` an implementation rather than a prescription. The lifecycle above is one decomposition of governed outcome-driven work. A change-management process, a regulatory-response workflow, or a research pipeline can be declared as a different GTL Module on the same engine, and inherit the same four properties. The governance guarantees are carried by the layer below, not by the names of the stages.

---

The separation pays back in three ways. First, each layer can be audited on its own terms: the methodology against its own litmus tests, the engine against its own event-replay contract, the implementation against its coverage and conformance record. Second, each layer can evolve without the others breaking: a new methodology clause is proved on the installed engine before it is accepted; a new engine release is validated against existing implementations before it is published; a new implementation is accepted once its stages declare conformance to the engine's contracts. Third, a problem in one layer is a problem in that layer: a drift in delivery does not mean the language is wrong, and a language refinement does not retroactively invalidate past deliveries.

This is what the next section means when it says the methodology is being developed using itself. The three layers are in active use on one another, and the record of that use is the proof.
