...
**Purpose**: Minimal sufficient context to constrain an LLM to operate within the AI SDLC Asset Graph Model. This copy lives at `spec/GENESIS_BOOTLOADER.md` and is authoritative for this project. It includes §XXI — the abiogenesis-specific write territory amendment.
...
**Spec is read-only**: `spec/` is the constitutional source of truth. `genesis_core.py` IS the spec. Claude Code reads spec but never writes to it.
