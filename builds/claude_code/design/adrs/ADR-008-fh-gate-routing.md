...
F_H evaluators are detected at `bind_fd` time by checking the event stream for a
`review_approved` event matching `{edge, feature}`. If no such event exists, the
evaluator is in the `failing` set. The iteration loop in `schedule.py` emits
...
F_H criteria using the proxy protocol and emits `review_approved` with
`actor: "human-proxy"`.

## Rationale

- F_H gates are not retried — they block until a `review_approved` event exists
...
- `review_approved` event is the only token that passes an F_H gate
