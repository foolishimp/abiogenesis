...
**REQ-R-ABG3-PLUGIN-SEAMS-005**: Plugin availability, actor authority, and transport steering shall remain separate. `CapabilityDefinition` rows derive from the accepted capability-definition graph and public catalog and describe what may be supported. An effect-bearing invocation shall carry exact per-basis `CapabilityGrant` rows plus actor attribution in its admitted `InvocationAuthority`; availability or class-level F_H ratification does not grant execution. Agent selection, budget, environment binding, and other `TransportSteering` values are subordinate ingress with recorded provenance and may choose transport behavior only after admission; they shall not create a capability, widen a grant, extend the plugin catalog, select a declaration, or fork the kernel. Declared plugin selection still resolves against the one governed catalog under the same fail-closed law.

Gap: per-instance approval attribution is not fully realized across all plugin routes — any route lacking exact actor identity, capability-grant ref, approval/policy ref, scope, and basis shall remain typed non-admitted rather than infer authority from steering, environment, package presence, or class-level ratification. Owner: T-244 routing; implementation requires a singular realization leaf.
