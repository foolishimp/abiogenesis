...
function failure(failureClass: string, deterministic?: Readonly<{
...
}>, issues: readonly SemanticJobContractIssue[] = []): Readonly<LeafRealizationCandidate> {
  ...
  const resultCandidate = { kind: "semantic_stage_failure", schemaVersion: "5.0.0", failureClass, diagnosticRef,
    ...(issues.length === 0 ? {} : { contractIssues: issues as unknown as JsonValue }) };
...
function semanticActor(input: Readonly<Record<string, JsonValue>>, occurrence: Readonly<LeafExecutionOccurrence>, role: "author" | "assessor", prepareAssembly?: () => Readonly<NativeInstructionAssembly>):
  ...
  return deepFreeze({ kind: "prepared_probabilistic_leaf_invocation", schemaVersion: "5.0.0", workerRequest: assembly.request,
    complete(exchange) {
      ...
      if (observation.disposition !== "success") return failure(observation.failureClass ?? "transport_failure");
      try {
        ...
        if (designResponse && candidate === null) return failure("semantic_candidate_relation_mismatch", undefined,
          [{ path: "$", rule: "design_reference_response", expected: "input_bound_semantic_job_design_response", actual: null }]);
        ...
        return output === null ? failure("semantic_candidate_relation_mismatch", undefined,
          isSemanticJobEnvelope(input) ? evaluateSemanticJobActorCandidate(input, stage.declarationRef, role, candidate) : []) : success(input, output, owner.call.implementationRef!, false);
      } catch { return failure("semantic_candidate_malformed"); }
    } });
}
export function realizeSemanticAuthor(input: Readonly<Record<string, JsonValue>>, occurrence: Readonly<LeafExecutionOccurrence>, prepareAssembly?: () => Readonly<NativeInstructionAssembly>) {
  return semanticActor(input, occurrence, "author", prepareAssembly);
}
export function realizeSemanticAssessor(input: Readonly<Record<string, JsonValue>>, occurrence: Readonly<LeafExecutionOccurrence>, prepareAssembly?: () => Readonly<NativeInstructionAssembly>) {
...
export async function realizeSemanticJobContext(input: Readonly<Record<string, JsonValue>>, occurrence: Readonly<LeafExecutionOccurrence>) {
...
export async function realizeSemanticJobPlan(input: Readonly<Record<string, JsonValue>>, occurrence: Readonly<LeafExecutionOccurrence>) {
...
export async function realizeSemanticJobBridge(input: Readonly<Record<string, JsonValue>>, occurrence: Readonly<LeafExecutionOccurrence>) {
