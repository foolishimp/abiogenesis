...
import { qualificationIdentityDigest, type ExactCandidateQualification } from "../validator/qualification_contracts.js";
import { RELEASE_OPERATION_ARTIFACT_SCHEMA, RELEASE_OPERATION_CONTRACTS, RELEASE_IDENTITY_SCHEMA, releaseAuthorityScope, releaseArtifactCoordinate,
 type PublishedRcSnapshotRequest,type ReleasePublicationGrant,type ReleasePhysicalObservation,type ReleaseOperationArtifact } from "../product/release_snapshot_operations.js";
...
export function projectReleaseQualification(request:PublishedRcSnapshotRequest,proof:QualificationProofResource,selection:QualificationEvidenceSelection,current:DurablePrefixCoordinate){
 try{
  if(admitRuntimeContract(RELEASE_OPERATION_CONTRACTS.snapshot.published_rc.requestSchema,request).disposition!=="admitted"||!v.is(QUALIFICATION_PROOF_RESOURCE_SCHEMA,proof)||selection.kind!=="verdict_selection")return null;
  const basis=request.qualificationBasis;
  if(basis.subjectKind!=="pre_rc_candidate"||!isQualificationBasisReady(basis)||!same(basis.prospectiveRelease,request.requestedIdentity)||!same(basis.lawBasis,request.lawBasis))return null;
  ...
  return verdict!==null&&verdict.subjectKind==="pre_rc_candidate"&&verdict.disposition==="green"&&verdict.bypassRefs.length===0&&
...
export function isReleaseOperationArtifact(value:unknown):value is ReleaseOperationArtifact{
 ...
 const a=value,p=a.publicInvocation;
 if(admitExactDefinitionCall({invocation:p,resources:null},RELEASE_OPERATION_CONTRACTS.snapshot.published_rc)===null||!isRecord(p)||!isRecord(p.invocationAuthority)||!isRecord(p.invocationAuthority.slots))return false;
 ...
 if(!same(approved.grants,grants)||!same(approved.basis.request,a.request)||approved.basis.resourceScope.resourcesDigest!==a.resourceDigest||
    ...
    a.resourceDigest!==sha256Canonical({kind:"release_publication_resources",schemaVersion:"5.0.0",eventResource:a.eventResource,proof:a.proof,selection:a.selection,effectGrant:a.effectGrant} as unknown as JsonValue)||
 ...
 if(!same(slots.capability_grants,{requiredCapabilityRefs:RELEASE_OPERATION_CONTRACTS.snapshot.published_rc.metadata.capabilityRefs,grants:grants.map(g=>({ref:g.grantRef,digest:g.grantDigest}))})||grants.some(g=>g.actorRef!==a.actorRef||g.definitionDigest!==a.invocation.definitionDigest||!same(g.definitionKey,p.definitionKey)))return false;
 return isExactOperationInvocationCoordinate(a.invocation)&&same(a.scope,releaseAuthorityScope(a.request,a.proof,a.selection,a.effectGrant))&&
