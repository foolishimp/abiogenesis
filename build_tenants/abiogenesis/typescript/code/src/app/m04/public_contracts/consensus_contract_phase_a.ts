...
const CONSENSUS_SOURCE_MODULE_PATH =
  "code/src/abg/m03/contracts/consensus_contract_family.js";
const CONSENSUS_SOURCE_EXPORT = "CONSENSUS_PUBLIC_CONTRACT_FAMILY";
...
function deriveSchemaArtifact(
  contractKind: string,
  definition: (typeof CONSENSUS_PUBLIC_CONTRACT_FAMILY)[keyof typeof CONSENSUS_PUBLIC_CONTRACT_FAMILY]
): ConsensusPhaseASchemaArtifact {
  const nativeContract = defineNativeContract({
    identity: {
      ...
      nativeLocator: {
        ...
        modulePath: CONSENSUS_SOURCE_MODULE_PATH,
        exportName: CONSENSUS_SOURCE_EXPORT,
        memberPath: [contractKind, "schema"]
      }
    },
  ...
  if (digest !== nativeContract.projectionWitness.projectionDigest) {
    throw new TypeError(
      `Consensus Phase A: byte/projection digest mismatch for ${definition.contractId}`
    );
  }
  return Object.freeze({
    ...
    relativePath: schemaRelativePath(definition.contractId),
...
/** @internal */
export function deriveConsensusPhaseAArtifactSet(): ConsensusPhaseAArtifactSet {
  const schemaAssets = Object.freeze(
    Object.entries(CONSENSUS_PUBLIC_CONTRACT_FAMILY)
      .map(([contractKind, definition]) =>
        deriveSchemaArtifact(contractKind, definition)
      )
