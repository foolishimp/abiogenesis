...
/**
 * Verifies a receipt only by deterministically re-resolving its exact
 * Product- and Validator-owned authority relation.
 */
export function verifyAbi5RootBindingReceipt(
  value: unknown,
  carrier: unknown,
  publicationBinding: Readonly<ProductModulePublicationBinding>,
  publication: Readonly<ModulePublication>,
  ...
  const issued = resolveAbi5RootBinding(
    carrier,
    publicationBinding,
    publication,
  );
  return issued.kind === "abi5_root_binding_receipt" &&
    canonicalJson(admitted.output as unknown as JsonValue) ===
      canonicalJson(issued as unknown as JsonValue);
}
