...
import { isWorksiteCommandExecutionTask, type WorksiteCommandExecutionTask } from "../product/worksite_command_execution.js";
...
export function projectRetainedWorksiteInputAtPrefix(
  ...
  const events = runtimeEventsFromValidatedPrefix(prefix);
  if (route.kind !== "traversal_route_admitted" || !events.some((event) => same(event, route)) ||
  ...
  if (!isWorksitePreparationBoundInput(asserted.value) || typeof route.basisId !== "string") return null;
  const entryBasis = projectExactExecutionBasisAtPrefix(prefix, route.basisId);
  if (entryBasis === null || !isWorksitePreparationInput(entryBasis.rawInputValue) ||
    !same(entryBasis.rawInputValue, asserted.value.entry) || entryBasis.graphRef !== route.payload.declarationRef ||
  ...
  if (source === null || !same(source.value, asserted.value.source) || !record(source.judgment.payload) ||
  ...
  const sourceGraph = asserted.value.entry.kind === "worksite_branch_command_preparation_input"
  ...
  if (sourceBasis === null || (sourceBasis.graphFunctionRef !== sourceGraph &&
      (asserted.value.entry.kind === "worksite_branch_command_preparation_input" || !nativeWorksiteRecoverySourceAtPrefix(prefix, sourceBasis))) || !sameInvocation(entryBasis, sourceBasis) ||
    sourceBasis.parentExecutionBasisRef !== entryBasis.basisRef || sourceBasis.parentCCallRef !== sourceOpen.aggregateId ||
    !same(sourceBasis.rawInputValue, asserted.value.entry.constructionTask) || sourceBasis.basisDigest !== foldback.payload.childExecutionBasisDigest) return null;
  ...
  const binding = asserted.value.entry.kind === "worksite_revision_command_preparation_input" ? worksiteRevisionRetentionBinding()
    : worksiteRetentionBinding(asserted.value.entry.kind === "worksite_branch_command_preparation_input");
  let input: ReturnType<typeof rawAdmitValue<Readonly<Record<string, JsonValue>>>>;
  try { input = rawAdmitValue(constructRetainedWorksiteInput(binding, entryBasis.rawInputValue, source.value), "invocation_input", binding.targetContractRef); }
  catch { return null; }
  if (input.kind !== "raw_admitted_value" || !same(input, asserted)) return null;
  const targetCursors = events.filter((event) => event.kind === "traversal_cursor_entered" && sameScope(event, route) &&
    record(event.payload) && event.payload.cursorRef === routePayload.targetCursorRef);
  if (targetCursors.length > 1 || targetCursors.some((event) => !record(event.payload) ||
    event.payload.cursorDigest !== routePayload.targetCursorDigest || event.payload.inputRef !== input.admissionRef ||
    event.payload.inputDigest !== input.subjectDigest)) return null;
...
export function projectSameRunWorksiteCommandSourceAtPrefix(prefix: ValidatedRuntimeEventPrefix, input: Readonly<{
  ...
  const events = runtimeEventsFromValidatedPrefix(prefix);
  if (!isWorksiteExecutionTask(input.task)) return null;
...
export function revisionPreparationHasNativeBridgeSourceAtPrefix(prefix: ValidatedRuntimeEventPrefix, input: Readonly<{
  ...
  if (call === null || !record(call.payload)) return false;
  ...
  if (route === null || !record(route.payload) || typeof route.payload.cCallRef !== "string") return false;
  const routePayload = route.payload;
  const result = one(events, e => e.kind === "c_call_result_admitted" && e.aggregateId === routePayload.cCallRef && sameScope(e, call));
  if (result === null || !record(result.payload) || typeof result.payload.resultRef !== "string") return false;
  ...
  if (bridge === null || fibre === null || !record(fibre.payload)) return false;
  ...
  if (fibre.payload.callClass === "workflow") {
    ...
    if (child === null || typeof fibre.payload.childGraphFunctionRef !== "string") return false;
  ...
  const nativeFibre = one(events, e => e.kind === "c_call_fibre_selected" && e.aggregateId === nativeResult.aggregateId && sameScope(e, nativeResult) && record(e.payload) &&
    e.payload.implementationRef === SEMANTIC_REVISION_IDS.bridgeImplementationRef && e.payload.implementationBindingRef === SEMANTIC_REVISION_IDS.bridgeBindingRef);
  return nativeFibre !== null && same(bridge.value, input.entry) && record(bridge.judgment.payload) &&
    ...
    bridge.judgment.admissionOrdinal < route.admissionOrdinal && route.admissionOrdinal < call.admissionOrdinal;
}
