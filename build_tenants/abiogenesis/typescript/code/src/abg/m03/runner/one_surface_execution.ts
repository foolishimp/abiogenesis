...
export interface T270CCallHandlerImplementation {
  readonly kind: "t270_c_call_handler_implementation";
  readonly handlerRef: string;
  readonly handler: CCallHandler;
}
...
interface T270SelectedFdHandler {
  readonly binding: CCallHandlerBinding;
  readonly handler: CCallHandler;
  readonly declaredConfig: unknown;
}

function resolveDeclaredFdHandler(input: {
  readonly executionBasis: ExecutionBasis;
  readonly selectedExecutionBinding: CatalogExecutionBinding;
  readonly vector: T270DirectStageVector;
  readonly locus: T270DirectStageLocus & {
    readonly node: T270DirectStageLocus["node"] & {
      readonly kind: "compiled_c_stage_leaf";
      readonly fibre: "F_D";
    };
    readonly operator: TraversalExecutionFamilyOperatorProjection & {
  ...
  readonly implementations: readonly T270CCallHandlerImplementation[];
}): T270SelectedFdHandler {
  const plan = input.vector.source.completeProgramPlan;
  const graphOperator = input.vector.graphVector.operators[0];
  // Operator-to-handler identity is derived from one selected vector locus:
  // GraphFunction -> complete-C program/stage/arm -> sole vector Operator.
  // The handler declaration does not duplicate Operator.binding authority.
  if (
    input.executionBasis.graphFunction.id !==
      input.selectedExecutionBinding.graphFunctionId ||
    input.executionBasis.graphFunction.name !==
      input.selectedExecutionBinding.graphFunctionHandle ||
    stableSha256Digest(input.executionBasis.graphFunction) !==
      input.selectedExecutionBinding.graphFunctionDigest ||
    plan.executionGraphFunctionRef !==
      input.selectedExecutionBinding.graphFunctionId ||
    plan.executionGraphFunctionDigest !==
      input.selectedExecutionBinding.graphFunctionDigest ||
    plan.programRef !== input.vector.source.selectedProgramRef ||
    plan.root.kind !== "compiled_c_stage_leaf" ||
    plan.root.nodeRef !== input.locus.node.nodeRef ||
    plan.root.domainStageRole !== input.locus.node.domainStageRole ||
    plan.root.armId !== input.locus.node.armId ||
    plan.root.fibre !== input.locus.node.fibre ||
    input.vector.graphVector.operators.length !== 1 ||
    graphOperator === undefined ||
    graphOperator.binding !== input.locus.operator.binding ||
    graphOperator.regime !== input.locus.operator.regime
  ) {
    throw new T270DirectExecutionError({
      code: "authority_mismatch",
      message:
        "selected GraphFunction, complete-C locus, and Operator do not form one AF-15 authority"
    });
  }
  ...
  for (const implementation of input.implementations) {
    if (
      implementation.kind !== "t270_c_call_handler_implementation" ||
      implementation.handlerRef.length === 0 ||
      typeof implementation.handler !== "function"
    ) {
      throw new T270DirectExecutionError({
        code: "implementation_unavailable",
        message: "AF-15 runtime handler implementation is malformed"
      });
    }
    if (handlers.has(implementation.handlerRef)) {
      throw new T270DirectExecutionError({
        code: "implementation_unavailable",
        message:
          `AF-15 runtime handler ${JSON.stringify(implementation.handlerRef)} is duplicated`
      });
    }
    handlers.set(implementation.handlerRef, implementation.handler);
  }

  const declaredBindings =
    input.executionBasis.compiledExecutionDeclarations.handlerBindingRows;
  if (declaredBindings === null) {
    throw new T270DirectExecutionError({
      code: "implementation_unavailable",
      message:
        "selected GraphFunction declares no AF-15 C-call handler binding"
    });
  }
  const registry = assembleHandlerRegistry({
    declaredBindings,
    handlers
  });
  const registryAdmission = admitHandlerRegistry(registry);
  if (!registryAdmission.accepted) {
    throw new T270DirectExecutionError({
      code: "implementation_unavailable",
      message:
        `selected GraphFunction handler registry is incomplete: ${registryAdmission.issues.join("; ")}`
    });
  }

  let selected: ReturnType<typeof resolveHandlerForSelection>;
  try {
    selected = resolveHandlerForSelection(registry, {
      programRef: plan.programRef,
      stageRole: input.locus.node.domainStageRole,
      armId: input.locus.node.armId,
      regime: input.locus.node.fibre
    });
  } catch (error) {
    throw new T270DirectExecutionError({
      code: "implementation_unavailable",
      message:
        `selected AF-15 locus has no admitted handler: ${error instanceof Error ? error.message : String(error)}`
    });
  }

  const configRef = selected.binding.handlerConfigRef;
  const configs = input.executionBasis.compiledExecutionDeclarations.handlerConfigs;
  const declaredConfig = configRef === null ? null : configs?.[configRef];
  if (configRef !== null && declaredConfig === undefined) {
    throw new T270DirectExecutionError({
      code: "authority_mismatch",
      message:
        `selected AF-15 handler config ${JSON.stringify(configRef)} is absent from admitted declarations`
    });
  }
  return Object.freeze({
...
export async function executeSelectedCatalogDirectProgram(input: {
  ...
  const fdHandler = locus.node.fibre === "F_D"
    ? (() => {
        ...
        return resolveDeclaredFdHandler({
          executionBasis,
          ...
          locus: {
            ...
            operator: { ...locus.operator, regime: "F_D" as const }
          },
    ...
    invokeAdmittedAtom: async (
      ...
      if (request.fibre === "F_D") {
        if (fdHandler === null) {
          throw new T270DirectExecutionError({
            code: "implementation_unavailable",
            message: "compiler-selected F_D locus lacks its declared handler"
          });
        }
        const carrierSet = exactCarrierSet({
          values,
          schemaRefs: vector.graphVector.source.map((node) => node.schema.ref)
        });
        const interior = await executeValueHandlerAsync(fdHandler.handler, {
          stage: Object.freeze({
            stageRole: locus.node.domainStageRole,
            defaultRegime: locus.node.fibre,
            armId: locus.node.armId,
            resultBearing: locus.node.resultBearing,
            instructionCategoryRefs: locus.node.instructionCategoryRefs
          }),
          binding: fdHandler.binding,
          declaredConfig: fdHandler.declaredConfig,
          workProjection: carrierSet
        });
        if (interior.kind === "c_call_value_handler_contract_failure") {
          return failedAtomSubmission({
            request,
            reason: interior.failureReason,
            failureClass: "contract_failure",
            evidenceRefs: interior.evidenceRefs,
        ...
        const admitted = admitTargetCandidateForAtom({
          ...
          candidate: interior.targetValueCandidate,
          target,
          ...
          producerRef: fdHandler.binding.handlerRef,
