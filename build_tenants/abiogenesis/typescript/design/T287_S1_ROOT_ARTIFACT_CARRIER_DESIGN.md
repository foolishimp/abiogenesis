...
B_root
  ...
  -> M = digest(final unchanged manifest containing Q,
                contracts/abi5-root-binding.json in productRelativeLocators,
                the existing contextual publication binding P,
                and manifest-owned owner context)
...
the existing locator list and contextual publication binding. `D_root` is
derived from the bytes at the located path and is not a manifest field. The
carrier never owns `P`, `M`, `A`, or the publication owner. S1 adds no
Product or Public schema member.
...
canonical paths, or fixed-path membership in a non-ABI Product refuse before a
receipt. Existing `productRelativeLocators` uniqueness and strict manifest
admission remain unchanged; no root-specific manifest member is introduced.
...
type RefDigest = Readonly<{
  ref: string;
  digest: Sha256Digest;
}>;

type CutARequestRefusalCode =
  | "request_shape_invalid"
  | "expected_digest_invalid";

type CutBRequestRefusalCode =
  | "request_shape_invalid"
  | "expected_digest_invalid"
  | "installed_package_root_invalid"
  | "cut_a_coordinates_invalid";

type PackedSnapshotRefusalCode =
  | "artifact_limit_exceeded"
  | "invalid_gzip_stream"
  | "malformed_tar_stream"
  | "parser_warning"
  | "unsupported_entry_type"
  | "unsafe_entry_path"
  | "duplicate_canonical_path"
  | "case_collision"
  | "invalid_tar_metadata"
  | "entry_limit_exceeded"
  | "payload_limit_exceeded";

type RootBindingRefusalCode =
  | "abi_identity_mismatch"
  | "abi_locator_missing"
  | "abi_carrier_missing"
  | "abi_carrier_duplicate"
  | "abi_carrier_shadow"
  | "non_abi_locator_present"
  | "non_abi_carrier_present"
  | "carrier_shape_invalid"
  | "carrier_bytes_noncanonical"
  | "root_binding_digest_mismatch"
  | "publication_missing"
  | "publication_ambiguous"
  | "publication_owner_mismatch"
  | "publication_digest_mismatch";

type InstalledSnapshotRefusalCode =
  | "installed_root_unreadable"
  | "containment_escape"
  | "link_present"
  | "hardlink_present"
...
type InstalledContentMismatchCode =
  | "missing_path"
  | "extra_path"
  | "file_bytes_mismatch"
  | "directory_set_mismatch";

type Abi5RootDeclarationShape = Readonly<{
  kind: "abi5_root_binding";
  schemaVersion: "5.0.0";
  bindingId: string;
  governorRef: string;
  ...
  runnableForm: Readonly<{
    environment: string;
    entrypoint: string;
  }>;
  programRef: string;
  declaredTraversalRegime: string;
  declaredCallableMembershipCardinality: string;
  graphFunctionRef: string;
  inputContractRef: string;
  outputContractRef: string;
  expectedTerminalOutcome: string;
  nearestWeakerExclusions: string;
}>;

type Abi5S1RootSelection = Readonly<{
  kind: "abi5_s1_root_selection";
  schemaVersion: "5.0.0";
  subjectKind: "s1_development_candidate";
  declaration: Abi5RootDeclarationShape;
  rootBinding: RefDigest;
  productContent: RefDigest;
  publication: RefDigest;
  manifest: RefDigest;
  artifact: RefDigest;
  owner: Readonly<{
    productId: string;
...
type SuccessfulCutACoordinates = Readonly<{
  kind: "s1_cut_a_coordinates";
  schemaVersion: "5.0.0";
...
type ProductVerificationCarrierRefusal<Cut extends "cut_a" | "cut_b"> =
  Readonly<{
    kind: "s1_carrier_refusal";
    schemaVersion: "5.0.0";
    disposition: "refused";
    code: "product_verification_refused";
    cut: Cut;
    refusal: ProductVerificationRefusal;
  }>;

type PackedSnapshotCarrierRefusal<Cut extends "cut_a" | "cut_b"> =
  Readonly<{
    kind: "s1_carrier_refusal";
    schemaVersion: "5.0.0";
    disposition: "refused";
    code: "packed_snapshot_refused";
    cut: Cut;
    reason: PackedSnapshotRefusalCode;
    evidence: RefDigest;
  }>;

type RootBindingCarrierRefusal<Cut extends "cut_a" | "cut_b"> = Readonly<{
  kind: "s1_carrier_refusal";
  schemaVersion: "5.0.0";
  disposition: "refused";
  code: "root_binding_refused";
  cut: Cut;
  reason: RootBindingRefusalCode;
  evidence: RefDigest;
}>;

type CutARequestCarrierRefusal = Readonly<{
  kind: "s1_carrier_refusal";
  schemaVersion: "5.0.0";
  disposition: "refused";
  code: "carrier_request_refused";
  cut: "cut_a";
  reason: CutARequestRefusalCode;
}>;

type CutBRequestCarrierRefusal = Readonly<{
  kind: "s1_carrier_refusal";
  schemaVersion: "5.0.0";
  disposition: "refused";
  code: "carrier_request_refused";
  cut: "cut_b";
  reason: CutBRequestRefusalCode;
}>;

type InstalledSnapshotCarrierRefusal = Readonly<{
  kind: "s1_carrier_refusal";
  schemaVersion: "5.0.0";
  disposition: "refused";
  code: "installed_snapshot_refused";
  cut: "cut_b";
  reason: InstalledSnapshotRefusalCode;
  evidence: RefDigest;
}>;

type InstalledContentMismatchCarrierRefusal = Readonly<{
  kind: "s1_carrier_refusal";
  schemaVersion: "5.0.0";
  disposition: "refused";
  code: "installed_content_mismatch";
  cut: "cut_b";
  reason: InstalledContentMismatchCode;
  packedSnapshotCoordinate: RefDigest;
  installedSnapshotCoordinate: RefDigest;
}>;

type CoordinateMismatchCarrierRefusal = Readonly<{
  kind: "s1_carrier_refusal";
  schemaVersion: "5.0.0";
  disposition: "refused";
...
type S1CarrierRefusal =
  | CutARequestCarrierRefusal
  | CutBRequestCarrierRefusal
  | ProductVerificationCarrierRefusal<"cut_a">
  | ProductVerificationCarrierRefusal<"cut_b">
  | PackedSnapshotCarrierRefusal<"cut_a">
  | PackedSnapshotCarrierRefusal<"cut_b">
  | RootBindingCarrierRefusal<"cut_a">
  | RootBindingCarrierRefusal<"cut_b">
  | InstalledSnapshotCarrierRefusal
  | InstalledContentMismatchCarrierRefusal
  | CoordinateMismatchCarrierRefusal;

type S1CutARefusal = Extract<S1CarrierRefusal, { cut: "cut_a" }>;
type S1CutBVerificationRefusal = Extract<
  S1CarrierRefusal,
  { cut: "cut_b" }
>;
type S1ConvergenceRefusal = Extract<
  S1CarrierRefusal,
  { cut: "convergence" }
>;

export declare const ABI5_S1_ROOT_CARRIER_PORT: Readonly<{
  verifyPacked(input: Readonly<{
    ...
    | Readonly<{
        ...
        coordinates: SuccessfulCutACoordinates;
      }>
    | S1CutARefusal
  >,

  verifyInstalledAndSelect(input: Readonly<{
    verificationRequest: VerifyProductRequest;
    ...
    | Readonly<{
        ...
        coordinates: SuccessfulCutBCoordinates;
        selection: Abi5S1RootSelection;
        selectionCoordinate: RefDigest;
      }>
    | S1CutBVerificationRefusal
    | S1ConvergenceRefusal
  >,
}>;
```
...
`Abi5RootDeclarationShape` closes field names and structural types only.
The parser obtains semantic values from the located canonical bytes and proves
their exact identity through expected `D_root`; TypeScript contains no second
full-value root table. `RefDigest` has exactly the two shown fields; every
shown object rejects unknown keys and every success/selection is recursively
frozen. The reason and refusal-code unions are exhaustive. The existing `VerifiedProductArtifact`,
`VerifyProductRequest`, and exact Public operation-family request, result, and
refusal schemas remain byte/schema/semantic compatible. The port is a package
proof seam, not a new
...
| Carrier refusal | Development-gate result |
|---|---|
...
| effective filesystem entry headers plus public `meta` bodies | 16,384 |
...
admissible effective filesystem entry headers. Symbolic links, hard links,
character/block devices, FIFO, sockets, GNU link/name extensions, GNU sparse or
other GNU metadata, sparse entries, whiteouts, and every unsupported type
...
component. The effective package-relative path must already equal its canonical
normalized representation. Duplicate canonical paths and ASCII case-fold
collisions refuse across all effective filesystem entries.

The adapter observes only the public `tar/parse` surface: effective entry
header fields, ordered public `meta` body emissions, and the next entry's
public `extended` / `globalExtended` projection. Each bounded `meta` body is
correlated in order with exactly that next entry before classification. A local
body must reproduce the entry's `extended` projection; a global body or
nonempty `globalExtended` refuses. Local PAX admits only exact `path`, `size`,
`mtime`, `atime`, or `ctime` keys with one canonical value and no NUL or
control data. GNU metadata, multiple pending bodies, a body without exactly one
following entry, an extended projection without exactly one preceding body,
ambiguous correlation, duplicate/unknown key, link metadata, conflicting
override, noncanonical number, truncation, checksum failure, decompression
warning, or limit breach refuses. Parser warnings are fatal.

The snapshot contains the artifact digest/length, ordered effective filesystem
entry-header projections, correlated ordered `meta` bodies and public extended
projections, canonical file paths, exact file bytes/digests/lengths, directory set,
aggregate sizes, and a domain-separated coordinate. It makes no claim about a
raw metadata header's identity or path because the selected public parser API
does not expose that relation. Byte-bearing identity bodies use unpadded
base64url plus digest and length; no Buffer is JSON-coerced. It is private
deterministic observation, not Product or runtime truth.
...
ABI-one/non-ABI-zero cardinality; exact existing
`productRelativeLocators` membership and `D_root` derived from the located
bytes; strict carrier shape/canonical bytes; exact `moduleRef` plus manifest
owner-context publication selection; and existing `P` equality. Only success
constructs an unexported nominal brand.
...
- that one self subpath and exact internal relative edges inside the verified
  ABI package root;
- the exact relative edge `../../../toolchain/typescript.cjs` from
  `declaration_exports.js`, resolved below the same verified package root;
...
absolute specifier, unexpected relative edge, unexpected builtin edge,
alternate URL scheme/casing,
...
`declaration_exports.ts` must replace the URL-derived absolute argument to the
require created by `createRequire(import.meta.url)` with the literal relative
argument `../../../toolchain/typescript.cjs`, yielding exactly:

```ts
createRequire(import.meta.url)("../../../toolchain/typescript.cjs")
```

The now-unused `fileURLToPath` import is deleted. This single relative edge is
censused and does not authorize any absolute specifier.
`verify_product.ts` removal of `node:child_process` is a precondition to
static closure freeze. Static graph plus runtime census—not V8 call counts or
Product counters—prove the narrow port cannot reach an external process or
absent owner.
...
```text
digest_D(x) = sha256(UTF8("abi5-s1/v4/" + D + NUL) + JCS(body_D(x)))
...
`K` names only the `S1CandidateBasis` record. `C` is reserved for the exact
Git candidate commit in Section 15; the implementation `K` carries that `C`
OID as its candidate-commit field.

| Record | Exact content | Forbidden content |
|---|---|---|
| `S1CandidateBasis` (`K`) | stable entry; exact candidate commit/tree/package identity; artifact ref/digest/length/integrity; D_root/Q/P/M; fixed-path `productRelativeLocators` membership and contextual publication-owner coordinates | copied root semantic body; host path; PID; observation; run/result/archive |
...
| `ProofRunPlan` | exact K/G/L/tool-closure coordinates, ordered cases, logical mount roles, confinement policy, and output grammar | direct F coordinate; PID; installed package root; host realpath; preparation/host observation; module census; actual output/result; enclosing run/archive |
...
2. An independent assessor freezes K, coordinate-free L, tool closure, static
...
|---|---|
| `input_refused` | plan plus canonical input attempt and exact input refusal; no admitted K/G/L/F or preparation fact beyond what admission reached |
...
|---|---|---|
| `E00` | exact K/G/L/F, artifact, Cut-A closure, and fresh Cut-B extraction | `evidence_complete` only |
| `E01` | admitted plan references crossed K/G/L/tool closure, or its G inventory has missing/malformed/crossed F or another immutable input | `input_refused`; no Product import; invalid plan shape/identity creates no run/result |
...
| 5 | `code/src/product/declaration_exports.ts` | replace the URL-derived absolute argument to the require created by `createRequire(import.meta.url)` with exact relative `../../../toolchain/typescript.cjs`; delete the unused `fileURLToPath` import; admit and census only that edge |
...
| 13 | external proof bundle and reproduction | freeze the implementation commit, build once and pack once to A, then freeze K/L/F/G/plan, OCI/tool closure, observations, result, run record, and independent verdict |

`code/src/product/index.ts`, Public operation-family request/result/refusal
schemas and adapters, GTL, HoG, ABG,
...
Let `C` be the exact reviewed docs commit or exact built-and-proved
implementation commit. It retains its reviewed tree and expected remote-main
parent `O`; delivery never recreates it under another commit identity. Let
`R_a` be a distinct archive commit with `tree(R_a) = tree(C)` and
`parent(R_a) = C`. All new archive refs use the full candidate OID `C`, not
`R_a`:
...
Here `<full-oid>` is exactly the 40-hex OID of `C`.

Every archive creation is a compare-and-swap from the all-zero OID to exact
`R_a`. An existing ref, nonzero old value, force move, deletion, or reuse
refuses.

Canonical `ArchiveMetadataCore` is the canonical commit-message block of
`R_a`, never a file in its tree. It binds `C`'s full OID, parent, tree, and
patch digest; subject kind and verdict; candidate version when applicable;
artifact and proof coordinates when applicable; and governing design/law
coordinates. It contains neither `R_a`'s own OID, archive ref, delivery
receipt, nor remote observation. Its domain-separated digest therefore excludes
the commit that encloses it. A candidate parent/tree/patch mismatch refuses
before `R_a` is created.

For an implementation candidate, `T_ref` is exactly
`refs/tags/v5.0.0-dev.<N>`. The frozen annotated tag object `T` targets
`C`; the tag ref never targets `R_a`.

After publication, an external `DeliveryReceipt` binds exact archive ref,
`R_a` OID/tree/parent, `C` OID/tree/parent, `ArchiveMetadataCore` digest,
remote name, expected and observed remote main OIDs, push transaction identity,
and verified remote containment; implementation acceptance additionally binds
`T_ref`, tag-object OID `T`, and target `C`. The receipt is not included in
`C` or `R_a`.
...
|---|---|---|
| docs accepted | one atomic push: expected remote main `O` -> `C` by fast-forward and accepted archive ZERO -> `R_a`; verify both remote OIDs and the archive binding to `C` | main advances to the reviewed commit; no Product tag or version effect |
| docs rejected | create rejected docs archive ZERO -> `R_a`; verify its remote containment and binding to `C`; main remains exact `O` | no main/tag/version effect |
| implementation accepted | after independent reproduction and verdict, one atomic push: expected remote main `O` -> `C`, `T_ref` ZERO -> annotated tag object `T` targeting `C`, and accepted archive ZERO -> `R_a`; verify main/tag/archive identities and bindings | one version consumed by immutable tag; no partial accepted state |
| implementation rejected | create rejected dev archive ZERO -> `R_a`; verify remote `R_a`, its metadata, and its binding to `C`; main and tags remain unchanged | version burns only after that verified remote containment |
...
publication uses atomic push and exact leases for main/tag plus ZERO -> `R_a`
create-only archive. A lease mismatch or atomic-push failure creates no
delivered state. Local preparation alone never advances status or burns a
version.
