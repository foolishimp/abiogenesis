...
1. the selected parent GraphFunction occurs exactly once in the supplied Module;
2. the selected parent GraphVector occurs exactly once in that GraphFunction;
3. the workflow ref resolves exactly once among that Module's GraphFunctions;
4. the parent vector's ordered source interface equals the lift input interface;
5. the parent vector's target interface equals the lift output interface;
6. constructor-authored input retains the M01 Node-interface witness, while raw
   input proves only exact child resolution and internal Node-interface
   continuity; and
7. the Module, parent, child, vector, program selection, and carrier identities
...
Tags, display fragments, effect refs, and nearby siblings never participate.
...
5. validates the returned child identity, carrier pair, terminal, and evidence;
...
those identities is not sub-traversal evidence and fails before the parent
C-call closes as success.
...
sequenceDiagram
  ...
  alt module, ref, or interface relation fails
    ...
    alt canonical product startup before T-267 closes
      ...
      Resolver->>Resolver: revalidate module digest and contained child
...
| Unresolved semantics stop before effects | `C-ALGEBRA-016` | startup block and typed diagnostics are explicit | gate precedes resolver on product path | product entry terminates blocked | no public bypass | T-255/T-267 fence retained | `pass` | T-267 for top-level traversal |
...
3. malformed workflow normalized shapes, non-empty stages plus workflow, absent
   child, ambiguous child, foreign module, stale module digest, and interface
