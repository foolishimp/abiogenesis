...
| --- | --- | --- | --- |
| `code/src/abg/m03/contracts/carriers.ts` | owns `RuntimeEvent`, `actor_result_artifact_observed`, `assessed`, and event kind values | add payload source event variants; keep `assessed` only as legacy traversal assessment projection | unknown or malformed payload event kind rejected |
...
| `code/src/abg/m03/runner/attached_fp_worker.ts` | converts accepted attached worker result to `assessed` events | emit payload observation/validation before evidence or legacy assessed projection | worker success with invalid payload creates no evidence row |
...
| `code/src/app/m04/result_assessment/constructors.ts` | builds assessed events from public result assessment | demote direct assessed construction from closure proof to legacy traversal assessment | old assessed-only output fails payload-ledger legal test |
