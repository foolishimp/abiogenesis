...
The current audit baseline for already migrated design assets lives in:
...
| `M04-app-bootstrap` | Python delivery-boundary and identity-projection design, `ADR-033`, shared `M04-app-bootstrap.yml` | `ABG_3_MODULE_DESIGN.md`, `M04_FIRST_SLICE_IACS.md`, `M04_PUBLIC_START_STRUCTURAL_CARRIER_DIAGRAM.md`, `M04_CONTROL_LOOP_FIRST_SLICE_IACS.md`, `M04_CONTROL_LOOP_STRUCTURAL_CARRIER_DIAGRAM.md`, `M04_EVENT_INGRESS_FIRST_SLICE_IACS.md`, `M04_EVENT_INGRESS_STRUCTURAL_CARRIER_DIAGRAM.md`, `M04_RESULT_ASSESSMENT_FIRST_SLICE_IACS.md`, `M04_RESULT_ASSESSMENT_STRUCTURAL_CARRIER_DIAGRAM.md`, `M04_LIVE_STATUS_FIRST_SLICE_IACS.md`, `M04_LIVE_STATUS_STRUCTURAL_CARRIER_DIAGRAM.md`, `M04_INSTALL_BOOTSTRAP_FIRST_SLICE_IACS.md`, `M04_INSTALL_BOOTSTRAP_STRUCTURAL_CARRIER_DIAGRAM.md`, `M04_BOOTLOADER_FIRST_SLICE_IACS.md`, `M04_BOOTLOADER_STRUCTURAL_CARRIER_DIAGRAM.md`, `M04_PUBLIC_ASSET_ADDRESSING_FIRST_SLICE_IACS.md`, `M04_PUBLIC_ASSET_ADDRESSING_STRUCTURAL_CARRIER_DIAGRAM.md` | package-first public-start boundary, bounded control-loop realization, bounded event-ingress realization, bounded result-assessment ingress, bounded live-status projection, bounded install/bootstrap delivery, bounded bootloader delivery, and bounded public asset-addressing over completed kernel truth; no direct event append, no bootstrap-owned kernel meaning | completed through `T-025`; later qualification families deferred |
| `M05-qualification-scenarios` | Python testcase authority, sandbox/archive proof, public `gen-start` proof lanes | `M05_QUALIFICATION_FIRST_SLICE_IACS.md`, `M05_INSTALLED_SANDBOX_FIRST_SLICE_IACS.md` | TypeScript qualification remains downstream of completed `M04`; method-trace and fake-lane stay module-owned, while installed sandbox/live/archive prove the installed line without replacing module proof lanes | completed through `T-022` |
| `M06-mapping-deferred` | Python mapping/deferred design | `M06_MAPPING_DEFERRED_DERIVATION.md`, `M06_MAPPING_DEFERRED_TRIGGER_IACS.md`, `M06_MAPPING_DEFERRED_STRUCTURAL_CARRIER_DIAGRAM.md` | alternate-runtime mapping remains dormant until one named alternate runtime family is intentionally activated beyond canonical ABG | completed dormant boundary through `T-023` |
