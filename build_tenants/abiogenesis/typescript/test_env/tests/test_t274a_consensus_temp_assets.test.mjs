...
test("T-274A derives nine schemas and two vocabularies as exact repeatable bytes", async () => {
  const first = await deriveConsensusPhaseAArtifactSet();
  const second = await deriveConsensusPhaseAArtifactSet();
...
test("T-274A materializes only temp assets and reads back exact bytes", async () => {
  const artifactSet = await deriveConsensusPhaseAArtifactSet();
  const artifacts = [
...
test("T-274A proves native and projected admission for the complete 9x9 matrix", async () => {
  const artifactSet = await deriveConsensusPhaseAArtifactSet();
