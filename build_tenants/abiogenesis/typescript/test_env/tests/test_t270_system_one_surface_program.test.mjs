...
import {
  ...
  ABG_SYSTEM_ONE_SURFACE_GRAPH_FUNCTION_ID,
...
function handlerDeclaration(binding) {
...
function withHandlerBindings(graphFunction, bindings) {
  return Object.freeze({
    ...graphFunction,
    declarations: graphFunctionDeclarations([
      ...graphFunction.declarations.entries.filter(
        (entry) => entry.key !== "abg.hog_handler_bindings"
      ),
      hogHandlerBindingsDeclarationEntry(bindings)
    ])
  });
}
...
test("T-270 SYSTEM One Surface program owns four distinct ordered authorities", async () => {
  ...
  const conformance = typecheckGtlProgram(program.gtlProgram);
  assert.equal(conformance.passed, true, JSON.stringify(conformance.issues));
  assert.equal(
    conformance.derivedConformanceInventory.counts.hogHandlerBindingCount,
    4
  );
  assert.deepEqual(
    program.authorityProgram.stages.map((stage) => Object.freeze({
      functionKind: stage.functionKind,
      hostGraphFunctionId: stage.handlerBinding.hostGraphFunctionId,
      programRef: stage.handlerBinding.programRef,
      stageRole: stage.handlerBinding.stageRole,
      armId: stage.handlerBinding.armId,
      regime: stage.handlerBinding.regime,
      handlerRef: stage.handlerBinding.handlerRef,
      handlerClass: stage.handlerBinding.handlerClass,
      handlerConfigRef: stage.handlerBinding.handlerConfigRef
    })),
    [
      "synthesize_model",
      "eval_gap",
      "evaluate_next",
      "evaluate_action"
    ].map((functionKind) => Object.freeze({
      functionKind,
      hostGraphFunctionId: ABG_SYSTEM_ONE_SURFACE_GRAPH_FUNCTION_ID,
      programRef:
        `program://abiogenesis/system/one-surface/${functionKind}/v1`,
      stageRole: functionKind,
      armId: `arm://abiogenesis/system/one-surface/${functionKind}/v1`,
      regime: "F_D",
      handlerRef:
        `handler://abiogenesis/system/one-surface/` +
        `${functionKind.replaceAll("_", "-")}/v1`,
...
test("T-270 handler declarations refuse missing, mismatched, and duplicate authority before effects", async () => {
  const { program } = await admittedCatalog();
  ...
  const canonical = program.authorityProgram.stages.map(
    (stage) => handlerDeclaration(stage.handlerBinding)
  );
  assert.equal(compileExecutionDeclarations(graphFunction).handlerBindingRows.length, 4);

  const { handlerBinding: _omitted, ...withoutHandler } =
    program.authorityProgram.stages[0];
  assert.throws(
    () => assertOneSurfaceStageAuthority(Object.freeze(withoutHandler)),
    /handler|admitted traversal authority/u
  );

  const mismatches = [
    Object.freeze({
      ...canonical[0],
      stageRole: "wrong_one_surface_role"
    }),
    Object.freeze({
      ...canonical[0],
      armId: "arm://abiogenesis/system/one-surface/wrong/v1"
    }),
    Object.freeze({
      ...canonical[0],
      regime: "F_P"
  ...
  for (const mismatch of mismatches) {
    assert.throws(
      () => compileExecutionDeclarations(withHandlerBindings(
        graphFunction,
        [mismatch, ...canonical.slice(1)]
      )),
      /handler_binding_(outside_program|regime_mismatch)/u
    );
  }

  assert.throws(
    () => hogHandlerBindingsDeclarationEntry([canonical[0], canonical[0]]),
    /duplicate handler binding/u
  );
});
