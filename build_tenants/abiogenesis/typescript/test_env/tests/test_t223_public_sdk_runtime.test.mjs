...
test("T-223 catalog invoke derives instruction startup and reaches the standard fake transport", async (t) => {
  ...
  const workerScript = [
    ...
    "const marker = evaluate ? 'Set resultContractRef to ' : 'Set result_contract_ref to ';",
    "const contractLine = prompt.split('\\n').find((line) => line.startsWith(marker));",
    "if (contractLine === undefined || !contractLine.endsWith('.')) throw new Error('missing selected result contract');",
    "const resultContractRef = JSON.parse(contractLine.slice(marker.length, -1));",
    "const output = evaluate",
    "  ? { resultContractRef, accepted: true, closeDisposition: 'close', assessmentIds: [], reasons: [] }",
    "  : {",
