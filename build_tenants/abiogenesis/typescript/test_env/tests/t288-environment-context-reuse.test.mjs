...
test('environment predicates and projection conserve raw meanings with one contextual binding guard', {
  ...
  for (const [variant, m] of modules.entries()) {
    ...
    assert.equal(work.resolvedLockCompleteChecks, value.productInstalls.length + (variant === 0 ? 2 : 1));
    assert.equal(work.workspaceBindingConstructions, 0);
  ...
  for (const [variant, m] of modules.entries()) {
    ...
    for (const [label, args, oldCount] of [['neither', [], 1], ['set only', [set], 2],
      ...
      const work = diff(before, counts()); assert.equal(work.resolvedLockCompleteChecks, 1);
      assert.equal(work.workspaceBindingConstructions, 0);
    ...
    for (const [label, action] of [
      ...
      assert.equal(work.resolvedLockCompleteChecks, 1);
