...
import { isWorksiteEffectAuthorization, isWorksiteFileReplaceReceipt, isWorksiteFileReplaceRequest, isWorksiteObservation, WORKSITE_FILE_REPLACE_EFFECT_URI, } from "../product/worksite_effect.js";
import { WORKER_TRANSPORT_FAILURE_CLASS_VALUES } from "./transport_contracts.js";
import { LEGACY_ROOT_EVENT_CONTRACT_DIGEST, ROOT_CURRENT_EVENT_PROFILE_REF, ROOT_EVENT_PROFILE_DECLARATION_REF, ROOT_EVENT_PROFILE_UPGRADE_REASON, constructRootCurrentEventContractDescriptor, isUndispatchedOwnerObservation, } from "./event_contract_profiles.js";
import { isRuntimeLivenessBinding, isRuntimeProbeObservation, isRuntimeSystemProbeContract, isRuntimeThresholdObservation, } from "./runtime_liveness_contracts.js";
...
export const LEGACY_ROOT_EVENT_CONTRACTS = Object.freeze({
    ...
    actor_transport_binding_admitted: {
        ...
        payloadVariants: [payloadVariant(payloadKeys("absoluteTimeoutMs actorRef agentKey archiveRoot args cCallRef command cwd dispatchOrdinal environmentDigest environmentPolicyDigest implementationBindingRef implementationRef inputDigest lane parser paths promptDigest promptTransport responseJsonSchemaDigest terminationGraceMs timeoutMs transportBindingDigest transportBindingRef transportContractDigest transportPlanDigest workerBindingRef"), payloadKeys("transportBindingRef transportBindingDigest cCallRef")), payloadVariant(payloadKeys("absoluteTimeoutMs actorRef agentKey archiveRoot args cCallRef command cwd dispatchOrdinal environmentDigest environmentPolicyDigest implementationBindingRef implementationRef inputDigest lane parser paths promptDigest promptTransport responseJsonSchemaDigest terminationGraceMs timeoutMs transportBindingDigest transportBindingRef transportContractDigest transportPlanDigest workerBindingRef instructionAssembly"), payloadKeys("transportBindingRef transportBindingDigest cCallRef instructionAssembly"))],
    },
    actor_invocation_started: {
    ...
    actor_process_timeout_observed: {
        variants: [PROCESS_EVENT],
        payloadVariants: [
            payloadVariant(payloadKeys("actorInvocationRef processRef timeoutClass timeoutMs"), payloadKeys("actorInvocationRef processRef timeoutClass timeoutMs"), { timeoutClass: "inactivity" }),
            payloadVariant(payloadKeys("actorInvocationRef processRef timeoutClass timeoutMs"), payloadKeys("actorInvocationRef processRef timeoutClass timeoutMs"), { timeoutClass: "absolute" }),
        ],
    },
    actor_process_signal_requested: {
        variants: [PROCESS_EVENT],
    ...
    c_call_evidenced: {
        ...
        payloadVariants: [
            ...
            payloadVariant(combinePayloadKeys(EVIDENCE_IO_PAYLOAD, payloadKeys("authorization receipt request successorObservation")), combinePayloadKeys(EVIDENCE_IO_PAYLOAD, payloadKeys("authorization receipt request successorObservation")), { evidenceClass: "worksite_file_replace" }),
...
const CURRENT_PROBE_CONTRACT = Object.freeze({
...
const currentBindingVariants = LEGACY_ROOT_EVENT_CONTRACTS.actor_transport_binding_admitted.payloadVariants.map(variant => payloadVariant(combinePayloadKeys(variant.allowedPayloadKeys, payloadKeys("livenessBinding")), combinePayloadKeys(variant.requiredPayloadKeys, payloadKeys("livenessBinding"))));
export const ROOT_EVENT_CONTRACTS = Object.freeze({
    ...
    invocation_admitted: {
        ...
        payloadVariants: [
            ...
            ...LEGACY_ROOT_EVENT_CONTRACTS.invocation_admitted.payloadVariants.map(variant => payloadVariant(combinePayloadKeys(variant.allowedPayloadKeys, payloadKeys("runEnvironment")), combinePayloadKeys(variant.requiredPayloadKeys, payloadKeys("runEnvironment")), variant.expectedPayload, variant.nullablePayloadKeys)),
        ],
    },
    actor_result_artifact_observed: {
        ...LEGACY_ROOT_EVENT_CONTRACTS.actor_result_artifact_observed,
        payloadVariants: LEGACY_ROOT_EVENT_CONTRACTS.actor_result_artifact_observed.payloadVariants.map(variant => payloadVariant(combinePayloadKeys(variant.allowedPayloadKeys, payloadKeys("nativeResultAssessment")), combinePayloadKeys(variant.requiredPayloadKeys, payloadKeys("nativeResultAssessment")), variant.expectedPayload)),
    },
    actor_transport_binding_admitted: {
    ...
    actor_process_timeout_observed: {
        ...
        payloadVariants: LEGACY_ROOT_EVENT_CONTRACTS.actor_process_timeout_observed.payloadVariants.map(variant => payloadVariant(combinePayloadKeys(variant.allowedPayloadKeys, payloadKeys("thresholdObservation")), combinePayloadKeys(variant.requiredPayloadKeys, payloadKeys("thresholdObservation")), variant.expectedPayload)),
    },
    c_call_evidenced: {
        ...
        payloadVariants: [
            ...LEGACY_ROOT_EVENT_CONTRACTS.c_call_evidenced.payloadVariants.map(variant => variant.expectedPayload?.evidenceClass === "probabilistic_transport"
                ? payloadVariant(combinePayloadKeys(variant.allowedPayloadKeys, payloadKeys("nativeResultAssessment")), combinePayloadKeys(variant.requiredPayloadKeys, payloadKeys("nativeResultAssessment")), variant.expectedPayload) : variant),
            payloadVariant(combinePayloadKeys(EVIDENCE_IO_PAYLOAD, payloadKeys("ownerObservation failureContractRef failureValue")), combinePayloadKeys(EVIDENCE_IO_PAYLOAD, payloadKeys("ownerObservation failureContractRef failureValue")), { evidenceClass: "undispatched_owner_refusal" }),
        ],
    },
...
export function isKnownRootEventContractDigest(value) {
...
const physicalEventPrefixes = new WeakMap();
...
export function runtimeEventPhysicalPrefix(events) {
    const last = events.at(-1), receipt = last === undefined ? undefined : physicalEventPrefixes.get(last);
    if (receipt === undefined) {
        if (events.some(event => physicalEventPrefixes.has(event)))
            throw new TypeError("physical event prefix has an uncommitted suffix");
...
const RUNTIME_EVENT_CANDIDATE_REQUIRED_KEYS = Object.freeze(RUNTIME_EVENT_REQUIRED_KEYS.filter((key) => key !== "admissionOrdinal" &&
    key !== "eventId" &&
    key !== "payloadDigest"));
function payloadInvocationRef(event) {
    if (typeof event.payload !== "object" || event.payload === null || Array.isArray(event.payload)) {
        return null;
    }
    const value = event.payload.invocationRef;
    return typeof value === "string" ? value : null;
}
export function selectRuntimeEvents(events, scope) {
    if (scope === undefined)
        return Object.freeze([...events]);
    const byId = new Map(events.map((event) => [event.eventId, event]));
    const occurrences = new Map();
    for (const event of events) {
        const rows = occurrences.get(event.eventId) ?? [];
        rows.push(event);
        occurrences.set(event.eventId, rows);
    }
    ...
    for (let index = 0; index < pending.length; index++) {
        for (const event of occurrences.get(pending[index]))
...
function sameDurableIdentity(left, right) {
...
function durableLockPath(identity) {
...
function acquireDurableAppendLock(identity) {
    ...
    try {
        ...
        if (linked && descriptor !== null) {
            try {
                if (sameDurableIdentity(descriptorIdentity(descriptor), durableIdentity(path))) {
                    unlinkSync(path);
                }
            }
            catch {
        ...
        if (isNodeError(error, "EEXIST")) {
            throw new TypeError("ABG durable event sink already has an exclusive append lock; abandoned locks require explicit operator recovery");
        }
        throw error;
    }
}
function releaseDurableAppendLock(lock) {
    if (lock === null)
        return;
    let releaseError;
    try {
        ...
        if (!sameDurableIdentity(descriptorFileIdentity, pathFileIdentity)) {
            throw new TypeError("ABG durable append lock identity changed before release");
        }
...
function openOwnedDurableSink(path, expectedIdentity) {
...
function releaseDurableOwnership(state) {
    let closeError;
...
function readDescriptorBytes(descriptor, byteLength) {
    const bytes = Buffer.alloc(byteLength);
    let offset = 0;
    while (offset < byteLength) {
        const read = readSync(descriptor, bytes, offset, byteLength - offset, offset);
        if (read === 0) {
...
const durableHistoryDerivations = new WeakMap();
...
class DurableHistoryDerivation {
    ...
    #cuts;
    #owner;
    constructor(coordinate, events, source, cuts, owner) {
    ...
    static activeEvents(coordinate, requireCurrent = false) {
        ...
        if (owner === undefined || owner.durableAppendClosed || owner.durableHistory === null)
            return undefined;
    ...
    static events(coordinate) {
        const proof = durableHistoryDerivations.get(coordinate);
        return typeof proof === "object" && proof !== null && #coordinate in proof && proof.#coordinate === coordinate
            ? proof.#events : undefined;
    }
    static cut(current, events) {
        const proof = durableHistoryDerivations.get(current), physical = runtimeEventPhysicalPrefix(events);
        return durablePrefixCoordinate(fileURLToPath(current.eventLogRef), current.storeIdentity, physical.byteLength, physical.digest, runtimeEventProfileAfter(events), {
    ...
    static historical(current, historical) {
        ...
        if (cut === undefined && historical.prefixLength > 0 && DurableHistoryDerivation.activeEvents(current) !== undefined) {
            ...
            const count = proof.#events.findIndex(event => physicalEventPrefixes.get(event)?.byteLength === historical.prefixLength) + 1;
            if (count !== 0) {
        ...
        if (cut === undefined || cut.eventCount > proof.#events.length ||
            ...
            historical.prefixLength !== cut.coordinate.prefixLength)
            return undefined;
        const coordinate = deepFreeze({ ...cut.coordinate, storeIdentity: { ...cut.coordinate.storeIdentity } });
        durableHistoryDerivations.set(coordinate, new DurableHistoryDerivation(coordinate, proof.#source.prefix(proof.#events, cut.eventCount), proof.#source, proof.#cuts, proof.#owner));
        return coordinate;
    }
}
...
export function reidentifyHistoricalDurablePrefixCoordinate(current, historical) {
    ...
    if (known !== undefined && DurableHistoryDerivation.activeEvents(current) !== undefined)
        return known;
    readAuthenticatedRuntimePrefixBytes(captured);
...
export function durableRuntimeEventPrefixThroughEvent(current, eventRef) {
    const events = readRuntimeEventsAtDurablePrefix(current);
    const index = events.findIndex(event => event.eventId === eventRef);
    if (index < 0)
        throw new TypeError("durable historical cut lacks its admitted event");
    return DurableHistoryDerivation.cut(current, events.slice(0, index + 1));
}
function durablePrefixCoordinate(path, identity, byteLength, prefixDigest, eventContractDigest, admitted) {
    const body = {
    ...
    if (admitted !== undefined) {
        const cuts = admitted.source.scope("durable_history", admitted.events)
            .owner(DURABLE_DERIVATION_CUTS, () => new Map());
        ...
        durableHistoryDerivations.set(coordinate, new DurableHistoryDerivation(coordinate, admitted.source.snapshot(admitted.events), admitted.source, cuts, admitted.owner));
    }
    return coordinate;
}
export function validateDurablePrefixCoordinate(value) {
    if (!isRecord(value))
        return false;
    const keys = [
        "coordinateDigest",
    ...
    if (Object.keys(value.storeIdentity).length !== identityKeys.length ||
        Object.keys(value.storeIdentity).some((key) => !identityKeys.includes(key))) {
        return false;
    }
    let canonicalEventLogRef;
    try {
        const url = new URL(value.eventLogRef);
    ...
    return value.kind === "durable_prefix_coordinate" &&
        ...
        sha256Canonical(body) === value.coordinateDigest;
}
export function validateEventStoreReopenAuthority(value) {
    if (!isRecord(value))
        return false;
    const keys = [
        "authorityDigest",
    ...
    return sha256Canonical(body) === value.authorityDigest;
}
export function validateEventStoreCloseHandoff(value) {
    if (!isRecord(value) ||
...
export function assertDurableRuntimePrefixBytes(prefix) {
    if (DurableHistoryDerivation.activeEvents(prefix) === undefined)
        readAuthenticatedRuntimePrefixBytes(prefix);
}
...
export function assertDurableRuntimePrefixCurrent(prefix) {
    if (DurableHistoryDerivation.activeEvents(prefix, true) !== undefined)
        return;
    ...
    let status;
    try {
        status = statSync(fileURLToPath(prefix.eventLogRef));
    }
    catch (error) {
...
export function projectRuntimeEventsAtDurablePrefix(prefix) {
    return DurableHistoryDerivation.events(prefix) ?? readRuntimeEventsAtDurablePrefix(prefix);
}
export function readRuntimeEventsAtDurablePrefix(prefix, options = {}) {
...
function readAuthenticatedRuntimePrefixBytes(prefix, options = {}) {
    if (!validateDurablePrefixCoordinate(prefix)) {
        ...
        const suppliedContract = typeof rawPrefix === "object" &&
            ...
            : undefined;
        throw new DurablePrefixReadError(suppliedContract !== undefined &&
            !isKnownRootEventContractDigest(suppliedContract)
            ? "event_contract_digest_mismatch"
            : "event_envelope_invalid", "ABG durable prefix coordinate is invalid");
    }
    ...
    try {
        ...
        if (beforePath.size < prefix.prefixLength ||
            beforeDescriptor.size < prefix.prefixLength ||
            (options.requireCurrent === true &&
                (beforePath.size !== prefix.prefixLength ||
                    beforeDescriptor.size !== prefix.prefixLength))) {
            throw new DurablePrefixReadError("prefix_length_mismatch", options.requireCurrent === true
        ...
        if (afterPath.dev !== beforePath.dev ||
            afterPath.ino !== beforePath.ino ||
            afterDescriptor.dev !== beforeDescriptor.dev ||
            afterDescriptor.ino !== beforeDescriptor.ino) {
            throw new DurablePrefixReadError("file_identity_mismatch", "ABG durable prefix file identity changed during read");
        }
        if (afterPath.size < prefix.prefixLength ||
        ...
        if (sha256Bytes(bytes) !== prefix.prefixDigest) {
            throw new DurablePrefixReadError("prefix_digest_mismatch", "ABG durable prefix bytes differ from coordinate");
        }
        return bytes;
    }
    finally {
...
export function assertHeldEventStoreAtDurablePrefix(store, prefix) {
...
function heldRuntimePrefixState(store, prefix) {
    const selected = captureDurablePrefixCoordinate(prefix);
    if (!validateDurablePrefixCoordinate(selected))
...
export function projectHeldRuntimeEventsAtDurablePrefix(store, prefix) {
    return readHeldRuntimeEventsAtDurablePrefix(store, prefix);
}
export function readHeldRuntimeEventsAtDurablePrefix(store, prefix) {
    const state = heldRuntimePrefixState(store, prefix);
    return state.derivation.snapshot(state.durableHistory.events);
}
export function selectHeldEventStoreDurablePrefix(store) {
    const state = eventState.get(store);
    if (state === undefined)
        throw new TypeError("event store was not constructed by this ABG module");
    ownedDurableDescriptor(state);
    if (state.durableHistory === null)
        throw new TypeError("durable selection requires its exact private admitted history");
    return state.durableHistory.prefix;
}
function appendDurablyBatch(state, events) {
    if (events.length === 0 || state.durableLogPath === null)
        return null;
    ...
    const physicalRows = events.map((event, index) => {
        ...
        return [event, { source: state.physicalSource, previous: events[index - 1] ?? priorHistory.events.at(-1) ?? null,
                byteLength: nextByteLength, digest: `sha256:${successorHash.copy().digest("hex")}` }];
    });
    ...
    try {
        ...
        const prefix = durablePrefixCoordinate(state.durableLogPath, state.durableFileIdentity, nextByteLength, `sha256:${successorHash.copy().digest("hex")}`, state.activeEventContractDigest, { source: state.derivation, events: nextEvents, owner: state });
        for (const [digest, body] of stagedBodies)
            state.inlineBodies.set(digest, body);
        for (const [event, receipt] of physicalRows)
            physicalEventPrefixes.set(event, receipt);
        ...
        try {
            ...
            catch { /* Preserve both I/O failures. */ }
            throw new AggregateError([error, rollbackError], "ABG durable event append failed and its owned prefix could not be restored");
        }
        throw error;
    }
}
...
function projectRuntimeEventAtContract(events, candidate, profileDigest, eventsById) {
    ...
    if (new Set(candidate.causationEventRefs).size !== candidate.causationEventRefs.length) {
    ...
    const causeEvents = candidate.causationEventRefs.map((eventRef) => {
        if (eventsById === undefined)
            return events.find((event) => event.eventId === eventRef);
    ...
    if (causeEvents.some((cause) => candidate.runId === undefined
...
const EVENT_STORE_CONSTRUCTION_AUTHORITY = Symbol("event_store_construction_authority");
export class AbgEventStore {
    ...
    digest(scope) {
        return runtimeEventPrefixDigest(selectValidatedRuntimeEventPrefix(this.readAll(), scope));
    }
    configuredDurableLogPath() {
    ...
    projectReopenAuthorityAndClose() {
        ...
        if (history === null)
            throw new TypeError("durable close requires its exact private admitted history");
...
export function closeHeldEventStoreAtDurablePrefix(store, expectedPrefix) {
...
function acquisitionRefusal(code, message) {
    return deepFreeze({
        kind: "event_store_acquisition_refusal",
        schemaVersion: "5.0.0",
        disposition: "refused",
        code,
        message,
...
export function createNewEmptyAppendSink(request) {
    if (!isRecord(request) ||
        ...
        return acquisitionRefusal("basis_mismatch", "new-empty append acquisition requires one exact canonical request");
    }
    ...
    try {
        ...
        const code = isRecord(error) && error.code === "EEXIST"
            ? "sink_exists"
            : "sink_unavailable";
        return acquisitionRefusal(code, code === "sink_exists"
    ...
    try {
        ...
        const prefix = durablePrefixCoordinate(path, owned.identity, 0, `sha256:${durableHash.copy().digest("hex")}`, ROOT_EVENT_CONTRACT_DIGEST, { source: derivation, events });
        ...
        return acquisitionRefusal("sink_unavailable", `new-empty append acquisition failed: ${String(error)}`);
    }
}
export function isEventStoreAdmissionOpen(store) {
    const state = eventState.get(store);
...
function assertRuntimeEventContract(candidate, profileDigest) {
    if (!isKnownRootEventContractDigest(profileDigest))
        throw new TypeError("unknown native event-contract profile");
    const contract = profileDigest === LEGACY_ROOT_EVENT_CONTRACT_DIGEST
        ? LEGACY_ROOT_EVENT_CONTRACTS[candidate.kind]
        : ROOT_EVENT_CONTRACTS[candidate.kind];
    if (contract === undefined)
        throw new TypeError("event kind is not in the selected exact profile");
    ...
    if (profileDigest === ROOT_EVENT_CONTRACT_DIGEST) {
        ...
        if (candidate.kind === "runtime_activity_probe_observed" || candidate.kind === "runtime_external_interruption_observed") {
            if (!isRuntimeSystemProbeContract(payload.probeContract) || !isRuntimeProbeObservation(payload.observation) ||
                ...
                payload.observation.scopeDigest !== sha256Canonical(payload.probeContract.scope) ||
...
function hasOnlyRuntimeEventKeys(value) {
...
function hasOnlyRuntimeEventCandidateKeys(value) {
    const allowed = new Set([
        ...RUNTIME_EVENT_CANDIDATE_REQUIRED_KEYS,
        ...RUNTIME_EVENT_OPTIONAL_KEYS,
    ]);
    return RUNTIME_EVENT_CANDIDATE_REQUIRED_KEYS.every((key) => Object.hasOwn(value, key)) && Object.keys(value).every((key) => allowed.has(key));
}
function isRuntimeEventCandidateShape(value) {
    const optionalStringsValid = RUNTIME_EVENT_OPTIONAL_KEYS.every((key) => !Object.hasOwn(value, key) ||
        (typeof value[key] === "string" && value[key].length > 0));
    return (typeof value.kind === "string" &&
        ROOT_EVENT_KIND_VALUES.includes(value.kind) &&
        typeof value.eventTime === "string" &&
        !Number.isNaN(Date.parse(value.eventTime)) &&
        typeof value.aggregateType === "string" &&
        ROOT_EVENT_AGGREGATE_TYPE_VALUES.includes(value.aggregateType) &&
        ...
        value.causationEventRefs.every((eventRef) => typeof eventRef === "string" && eventRef.length > 0) &&
...
export function validateHistoricalEvents(bytes, expectedProfileDigest) {
    return decodeHistoricalEvents(bytes, expectedProfileDigest).events;
}
function decodeHistoricalEvents(bytes, expectedProfileDigest) {
    const inlineByEvent = new Map(), inlineBodies = new Map();
    const physicalSource = {}, physicalRows = [];
    ...
    let profile = null;
    for (let start = 0, end = encoded.indexOf(10); end !== -1; start = end + 1, end = encoded.indexOf(10, start)) {
        ...
        if (!isRecord(physical) || canonicalJson(physical) !== line) {
        ...
        if (!isRecord(decoded) || !hasOnlyRuntimeEventKeys(decoded))
            throw new TypeError("durable ABG event log contains an invalid logical event");
        const { eventId, admissionOrdinal, payloadDigest, eventContractDigest, ...candidateValue } = decoded;
        if (!Number.isSafeInteger(admissionOrdinal) ||
            admissionOrdinal !== admitted.length + 1) {
            throw new DurablePrefixReadError("admission_ordinal_invalid", "durable ABG event log contains an invalid admission ordinal");
        }
        ...
        if (profile === null)
            profile = eventContractDigest === undefined
        ...
        if (representation.physicallyInline) {
            ...
            if (body !== null) {
                ...
                if (!inlineBodies.has(body.digest))
                    inlineBodies.set(body.digest, body);
            }
        }
        ...
        physicalRows.push([reconstructed, { source: physicalSource, previous: admitted.at(-1) ?? null, byteLength,
                digest: `sha256:${physicalHash.copy().digest("hex")}` }]);
        ...
        if (isRootEventProfileBoundary(reconstructed)) {
            ...
            if (profile !== LEGACY_ROOT_EVENT_CONTRACT_DIGEST)
                throw new TypeError("native profile boundary cannot repeat or reverse");
...
function isRootEventProfileBoundary(event) {
...
function assertRootEventProfileBoundary(events, candidate) {
    ...
    if (!isRootEventProfileBoundary(candidate) || !isRecord(p) || !isRecord(prior) ||
        ...
        sha256Canonical(p.contentValue) !== p.contentValueDigest ||
        !["declarationRef", "beforeDigest", "afterDigest", "changeClass", "owningTicketRef", "reason"].every(key => p.contentValue[key] === p[key]) ||
...
export function admitRootEventProfileUpgrade(store, entryPrefix, publicCandidate, repriceCandidate) {
    ...
    const committed = runRuntimeEventTransaction(store, () => {
        ...
        if (publicOperationEvent.eventId !== projectedPublic.eventId)
            throw new TypeError("profile Public predecessor changed");
    ...
    if (committed.successorPrefix === null)
        throw new TypeError("profile upgrade has no durable successor");
...
export function projectRootEventProfileSchedule(events, emptyProfileDigest = ROOT_EVENT_CONTRACT_DIGEST) {
    let profile = events[0]?.eventContractDigest === undefined && events.length !== 0
        ? LEGACY_ROOT_EVENT_CONTRACT_DIGEST : emptyProfileDigest;
    if (!isKnownRootEventContractDigest(profile))
        throw new TypeError("unknown native profile");
...
export function extendRootEventProfileProjection(events, start, prior) {
    ...
    for (let index = start; index < events.length; index += 1) {
        ...
        if (last?.eventContractDigest === profile)
            last.lastOrdinal = event.admissionOrdinal;
        else
            spans.push({ eventContractDigest: profile, firstOrdinal: event.admissionOrdinal, lastOrdinal: event.admissionOrdinal });
        if (isRootEventProfileBoundary(event)) {
            assertRootEventProfileBoundary(events.slice(0, index), event);
            if (profile !== LEGACY_ROOT_EVENT_CONTRACT_DIGEST || boundaryEventRef !== null)
                throw new TypeError("repeated native profile boundary");
...
function runtimeEventProfileAfter(events) {
    const last = events.at(-1);
    if (last === undefined)
        return ROOT_EVENT_CONTRACT_DIGEST;
...
export function authenticateRuntimePrefixAncestry(historical, current) {
    try {
        ...
        if (DurableHistoryDerivation.activeEvents(current) !== undefined &&
            DurableHistoryDerivation.historical(current, historical) !== undefined)
            return true;
        const oldEvents = readRuntimeEventsAtDurablePrefix(historical);
        const currentEvents = readRuntimeEventsAtDurablePrefix(current);
        if (runtimeEventPhysicalPrefix(currentEvents.slice(0, oldEvents.length)).digest !== historical.prefixDigest)
            return false;
        if (historical.storeIdentity.eventContractDigest === current.storeIdentity.eventContractDigest)
            return true;
        if (historical.prefixLength === 0 || historical.storeIdentity.eventContractDigest !== LEGACY_ROOT_EVENT_CONTRACT_DIGEST ||
            current.storeIdentity.eventContractDigest !== ROOT_EVENT_CONTRACT_DIGEST)
            return false;
...
function reopenRefusal(code, message) {
    return deepFreeze({
        kind: "event_store_reopen_refusal",
        schemaVersion: "5.0.0",
        disposition: "refused",
        code,
        message,
...
function seedValidatedDurableHistory(exactPath, owned, byteLength, prefixDigest, profile, durableHash, history) {
    const store = new AbgEventStore(EVENT_STORE_CONSTRUCTION_AUTHORITY);
    const derivation = new RuntimeDerivationSource();
...
export function recoverInterruptedEventStore(selection) {
    ...
    let lockIdentity = abandoned;
    let descriptor, lockDescriptor, acquiredLock;
    let store, stage = "selection";
    const cleanupErrors = [];
    const closeTemporary = (fd) => {
        if (fd !== undefined)
            try {
                closeSync(fd);
            }
    ...
    const descriptorState = (fd) => {
        ...
        try {
            ...
            return isNodeError(error, "EBADF") ? "closed" : "unknown";
        }
    };
    try {
        ...
        if (abandoned !== undefined) {
            stage = "quiescence";
            try {
                process.kill(selection.ownerPid, 0);
                throw new TypeError("selected former owner PID is live");
            }
            catch (error) {
        ...
        const exactIdentity = (path, fd, expected) => {
            ...
            if (!status.isFile() || !sameDurableIdentity({ device: status.dev, inode: status.ino }, expected) ||
                !sameDurableIdentity(held, expected))
                throw new TypeError("recovery resource path/descriptor identity differs");
        };
        exactIdentity(selected.eventLogPath, descriptor, selected);
        if (abandoned !== undefined)
            lockDescriptor = openSync(lockPath, constants.O_RDONLY);
        else {
        ...
        if (fstatSync(descriptor).size !== selection.expectedCurrent.byteLength)
            throw new TypeError("selected current log extent differs");
        const bytes = readDescriptorBytes(descriptor, selection.expectedCurrent.byteLength);
        const durableHash = createHash("sha256").update(bytes), digest = `sha256:${durableHash.copy().digest("hex")}`;
        ...
        if (old !== undefined) {
            const oldCount = old.durableByteLength === 0 ? 0 : history.events.findIndex(event => physicalEventPrefixes.get(event)?.byteLength === old.durableByteLength) + 1;
        ...
        stage = "adoption";
        store = seedValidatedDurableHistory(selected.eventLogPath, { descriptor, identity: { device: selected.device, inode: selected.inode }, lock: { descriptor: lockDescriptor, path: lockPath } }, bytes.byteLength, digest, selected.eventContractDigest, durableHash, history).store;
        stage = "close";
        const closeHandoff = store.projectReopenAuthorityAndClose();
        return deepFreeze({ kind: "recovered", closeHandoff, validatedEventCount: history.events.length });
    }
    catch (error) {
        if (store === undefined) {
            ...
            if (acquiredLock !== undefined) {
                try {
                    releaseDurableAppendLock(acquiredLock);
                }
                catch (secondary) {
                    cleanupErrors.push(String(secondary));
                }
            }
            else
                closeTemporary(lockDescriptor);
            closeTemporary(descriptor);
            if (acquiredLock === undefined || cleanupErrors.length === 0)
                return deepFreeze({ kind: "refused_no_recovery", code: stage, message: String(error), cleanupErrors });
        }
        else {
        ...
        let lockResidue = "other_or_unknown";
        try {
            ...
            if (isNodeError(secondary, "ENOENT"))
                lockResidue = "absent";
        }
        return deepFreeze({ kind: "recovery_fault", code: store === undefined ? "cleanup_failed" : "close_failed", message: String(error),
...
export function reopenEventStore(authority) {
    if (isRecord(authority) &&
        isSha256Digest(authority.eventContractDigest) &&
        !isKnownRootEventContractDigest(authority.eventContractDigest)) {
        return reopenRefusal("contract_mismatch", "event-store reopen authority names another event contract");
    }
    if (!validateEventStoreReopenAuthority(authority)) {
        return reopenRefusal("basis_mismatch", "event-store reopen authority is not self-consistent");
    }
    ...
    if (authority.durableByteLength === 0 &&
        ...
        return reopenRefusal("basis_mismatch", "empty legacy event-store reopen has no authenticated origin");
    }
    ...
    try {
        ...
        return reopenRefusal("sink_unavailable", `durable event sink cannot acquire exact append ownership: ${String(error)}`);
    }
    let bytes;
    try {
        ...
        if (finalStatus.size !== status.size ||
        ...
        closeSync(owned.descriptor);
        releaseDurableAppendLock(owned.lock);
        return reopenRefusal("sink_unavailable", `durable event sink cannot be opened: ${String(error)}`);
    }
    const durableHash = createHash("sha256").update(bytes);
    const prefixDigest = `sha256:${durableHash.copy().digest("hex")}`;
    if (bytes.byteLength !== authority.durableByteLength ||
        prefixDigest !== authority.eventLogDigest) {
        closeSync(owned.descriptor);
        releaseDurableAppendLock(owned.lock);
        return reopenRefusal("durable_log_mismatch", "durable event sink differs from the selected exact prefix");
    }
    let history;
    try {
    ...
    return seedValidatedDurableHistory(exactPath, owned, bytes.byteLength, prefixDigest, authority.eventContractDigest, durableHash, history);
}
export function admitRuntimeEvent(store, candidate) {
...
function isLivenessCandidate(candidate) {
...
export function admitNativeRuntimeLivenessEvent(store, candidate) {
    const state = eventState.get(store);
...
function admitRuntimeEventInternal(store, candidate, preparedEvent) {
    const state = eventState.get(store);
    if (state === undefined) {
        throw new TypeError("event store was not constructed by this ABG module");
    }
...
function appendRefusal(code, message) {
    return deepFreeze({
        kind: "event_store_append_refusal",
        schemaVersion: "5.0.0",
        disposition: "refused",
        code,
        message,
...
export function appendCheckedArtifactEvent(store, expectedPredecessor, initiatedEvent) {
    if (!isRecord(initiatedEvent.payload) ||
        ...
        throw new TypeError("checked artifact ingress is closed to Product install and workspace binding");
    }
    try {
...
export function admitRuntimeEventBatch(store, factories) {
    const state = eventState.get(store);
    if (state === undefined) {
        throw new TypeError("event store was not constructed by this ABG module");
    }
    ...
    const eventsById = { get: (ref) => stagedById.get(ref) ?? state.eventsById.get(ref) };
...
function runRuntimeEventTransaction(store, action, requireDurableSuccessor = false, authenticatedPredecessor = null) {
    const state = eventState.get(store);
    if (state === undefined) {
        throw new TypeError("event store was not constructed by this ABG module");
    }
    ...
    try {
        ...
        if (authenticatedPredecessor !== null) {
            try {
                ...
                throw new AggregateError([error, drift], "runtime transaction refused with a changed durable predecessor");
            }
        }
        throw error;
    }
...
export function admitRuntimeEventTransaction(store, action) {
    return runRuntimeEventTransaction(store, action).value;
}
export function admitRuntimeEventTransactionAtExpectedPrefix(store, expectedPrefixDigest, action) {
    if (store.digest() !== expectedPrefixDigest) {
        throw new TypeError("runtime event append requires the exact expected immutable prefix");
    }
 ...
 */
export function admitRuntimeEventTransactionAtDurablePrefix(store, expectedPredecessor, action) {
...
function assertHeldDurableTransactionHistory(store, expectedPredecessor) {
 ...
 */
export function admitNonEmptyRuntimeEventTransactionAtDurablePrefix(store, expectedPredecessor, action) {
    assertHeldDurableTransactionHistory(store, expectedPredecessor);
    return runRuntimeEventTransaction(store, action, true, expectedPredecessor);
}
...
export function readActiveRuntimeTransactionAtDurablePrefix(store, prefix, options = {}) {
...
export function assertRuntimeEventTransactionActive(store) {
    const state = eventState.get(store);
    if (state === undefined) {
        throw new TypeError("event store was not constructed by this ABG module");
    }
    if (state.transactionStartIndex === null) {
        throw new TypeError("planned event admission requires one active outer transaction");
    }
}
export function isRuntimeEventTransactionActive(store) {
    const state = eventState.get(store);
    if (state === undefined) {
        throw new TypeError("event store was not constructed by this ABG module");
    }
...
export function compareAndAppendExpectedPrefix(store, expectedPrefixDigest, factories) {
    if (store.digest() !== expectedPrefixDigest) {
        throw new TypeError("runtime event append requires the exact expected immutable prefix");
    }
