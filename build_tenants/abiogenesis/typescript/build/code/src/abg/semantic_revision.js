...
function nativeIntakeFacts(basis, input, onRefusal) {
    try {
        ...
        if (owner === null || owner.call.implementationRef !== ids.nativeIntakeImplementationRef ||
            !same(owner.inputValue, input) || !authenticateRuntimePrefixAncestry(input.sourcePrefix, basis.predecessorPrefix))
            return null;
        ...
        if (run === null || !same(run.run, input.sourceRun) || sourceRoot === null)
            return null;
        ...
        if (oldEnvironment.kind !== "exact_prefix_workspace_environment" ||
            ...
            worksiteCommandSourcesInvalidatedAfter(owner.prefix, cause.event.admissionOrdinal, owner.environment.workspaceAuthorityBasis.canonicalRoot, [], expectedContext.readRoots))
            return null;
