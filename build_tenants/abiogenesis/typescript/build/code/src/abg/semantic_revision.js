...
function nativeIntakeFacts(basis, input, onRefusal) {
    ...
    try {
        ...
        if (worksiteCommandSourcesInvalidatedAfter(owner.prefix, operationalInput?.evidence == null ? cause.event.admissionOrdinal : parent.event.admissionOrdinal, owner.environment.workspaceAuthorityBasis.canonicalRoot, [], expectedContext.readRoots))
        ...
        if (operationalInput?.evidence != null && (constructionState?.result.resultRef !== operationalInput.evidence.constructionResultRef ||
            constructionState.result.resultDigest !== operationalInput.evidence.constructionResultDigest))
            return refuse("construction_source_mismatch");
...
function nativeJobRevisionSubject(basis, input, readPhysical) {
    try {
        ...
        if (current.context === null || !semanticJobContextMatches(basis, current, current.context) ||
            readPhysical && !semanticJobContextCurrent(basis, current))
            return null;
        if (current.evidence !== null && same(current.evidence, envelope.evidence) &&
            worksiteCommandSourcesInvalidatedAfter(owner.prefix, indexedRuntimeEvents(owner.prefix, "id:" + acquired.result.admissionEventRef)[0].admissionOrdinal, owner.environment.workspaceAuthorityBasis.canonicalRoot, [], current.context.readRoots))
            return null;
