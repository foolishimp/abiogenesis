...
function assemblyIdentity(domain, plan, owner, predecessorPrefix, content) {
    ...
    const envelope = { planRef, cCallRef: owner.call.cCallRef, cCallDigest: owner.call.cCallDigest,
        ...
        inputRef: owner.inputRef, inputDigest: owner.inputDigest, predecessorPrefix, ...content };
    return { planRef, planDigest, plan, envelope, envelopeDigest: sha256Canonical(envelope) };
}
function renderInstructionSections(order, sections) {
    return order.map(name => `## ${name}\n${canonicalJson(sections[name])}`).join("\n\n");
}
function finishInstructionAssembly(identity, fields, request, promptBytes = Buffer.from(request.prompt, "utf8")) {
...
export function constructWorksiteNativeInstructionAssembly(basis, supplied) {
    try {
        ...
        if (isNativeWorkspaceWorkTask(supplied)) {
            ...
            const identity = assemblyIdentity("native-work", plan, owner, basis.predecessorPrefix, { nativeBasis: { ...basis, declarationGraphFunctions: [basis.graphFunction] }, task: supplied });
        ...
        const identity = assemblyIdentity("worksite-native", plan, owner, basis.predecessorPrefix, { nativeBasis, sections });
        ...
        return finishInstructionAssembly(identity, manifest, { ...base, prompt });
    }
    catch {
        return null;
    }
...
function nativeEvidenceWorkView(value) {
    const inventory = (context) => ({ kind: context.kind, schemaVersion: context.schemaVersion,
    ...
    return { ...value, task: { kind: value.task.kind, schemaVersion: value.task.schemaVersion,
            taskDigest: sha256Canonical(value.task), bodyDisposition: "historical_constructor_input_not_selected" },
...
export function renderSemanticEvidenceTextView(evidence) {
    ...
    const construction = native && evidence.constructionResult.kind === "native_workspace_work_observation"
        ? nativeEvidenceWorkView(evidence.constructionResult)
        : evidence.constructionResult;
    const executionTask = native ? (() => {
        const task = observation.task, source = task.sourceNativeWork;
        const sourceView = sha256Canonical(source) === sha256Canonical(evidence.constructionResult)
    ...
    return deepFreeze({ ...evidence, kind: "semantic_worksite_evidence_text_view",
        ...
        executionObservation: { ...observation, task: executionTask, commandResults: observation.commandResults.map(command => ({ ...command, stdout: stream(command.stdout), stderr: stream(command.stderr) })) },
...
export function evaluateNativeInstructionAssembly(basis, supplied, readPhysical = false) {
    ...
    try {
        ...
        return finishInstructionAssembly(identity, manifest, { actorRef: SEMANTIC_STAGE_IDS.workerActorRef, workerBindingRef: SEMANTIC_STAGE_IDS.workerBindingRef,
            implementationRef: owner.call.implementationRef, inputDigest: owner.inputDigest,
...
function constructJobInstructionAssembly(basis, supplied, readPhysical) {
    ...
    if (!isSemanticStageDeclaration(stage) || stage.assetSurface.authoritySlots.some(s => s.disposition !== "normal") ||
        ...
        return null;
    const material = projectSemanticJobActorMaterial(input, stage.declarationRef, owner.role, revision?.revisionBasis.retainedTerms ?? [], revision?.revisionBasis.request.nativeWorksite?.commandExecutionLimits);
    if (material === null)
        return null;
    ...
    const predecessorMaterial = (row) => {
        if (revision === null)
            return row;
        const asset = input.assets.find(a => a.assetRef === row.assetRef && a.assetDigest === row.assetDigest);
        const materialDigest = sha256Canonical(asset);
        const index = revision.revisionBasis.historicalAssets.findIndex(h => sha256Canonical(h) === materialDigest);
    ...
    const termIndices = new Map(retainedTerms.map((term, index) => [canonicalJson(term), index]));
    const termMaterial = (terms) => {
        if (revision === null || terms.length === 0)
            return terms;
        const indices = terms.map(term => termIndices.get(canonicalJson(term)));
        return indices.some(index => index === undefined) ? terms : { presentationRef: "#/obligations/retainedTerms",
            indices, materialDigest: sha256Canonical(terms) };
    };
    ...
    const candidateViews = new Map(revision?.revisionBasis.historicalAssets.map((asset, index) => [
        canonicalJson(asset.candidate), { presentationRef: `#/task/historicalAssets/${index}/candidate`,
            materialDigest: sha256Canonical(asset.candidate) },
    ]));
    const observedText = (base64, digest, byteLength) => {
        const exact = exactEvidenceText(base64, digest, byteLength);
        if (exact.text === null || candidateViews.size === 0)
            return exact;
        try {
            const match = candidateViews.get(canonicalJson(admitIJsonText(exact.text)));
            if (match !== undefined)
                return { disposition: "typed_json_value", digest: exact.digest, byteLength: exact.byteLength, ...match };
        }
    ...
    const origins = constructionDigest === null || nativeOrigins === null ? revisionSubject?.origins : nativeOrigins.map(origin => {
        const result = origin.result, expected = construction;
        ...
        if (result.resultClass !== "success" || origin.judgment.judgment !== "advance" ||
            ...
            origin.cCall.cCallRef !== expected.provenance.cCallRef || value.provenance.cCallRef !== origin.cCall.cCallRef)
            return origin;
    ...
    const identity = assemblyIdentity("semantic-job", plan, owner, basis.predecessorPrefix, { sections });
...
export function renderNativeRevisionSelectionDecisionView(sections, envelope) {
    ...
    const isCompleted = (value) => completedDigest !== null && sha256Canonical(value) === completedDigest;
    ...
    const nativeWorkView = (value) => isCompleted(value) ? {
        observationRef: value.observationRef, observationDigest: value.observationDigest,
        presentationRef: "#/worksite/construction/result/value", disposition: "same_admitted_construction_evidence"
    } : ({ ...value,
    ...
    const leafView = (leaf, primaryConstruction = false) => {
    ...
    const evidence = { ...originalEvidence,
        ...
        causes: originalEvidence.causes.map(leaf => leafView(leaf)) };
