...
import { isNativeWorkspaceWorkTask, isNativeWorkspaceWorkObservation } from "../product/native_workspace_work.js";
import { isNativeWorksiteCommandExecutionObservation } from "../product/worksite_command_execution.js";
import { constructNativeSemanticTask, constructNativeSemanticConstructionTask, constructNativeSemanticExecutionTask, deriveNativeSemanticAsset, deriveNativeSemanticAssessment, nativeSemanticContextMatches, nativeSemanticEvidenceArtifacts, nativeSemanticCommandExecutionLimits } from "../product/semantic_job.js";
...
import { isSemanticJobInput, isSemanticJobEnvelope, constructSemanticJobEnvelope, deriveSemanticJobAsset, deriveSemanticJobAssessment, deriveSemanticJobPreparation, deriveSemanticJobReadDependencies, semanticJobDesignMatches, semanticJobPathWithin } from "../product/semantic_job.js";
import { constructWorksiteSubject, constructWorksiteTerritory, constructWorksiteObservation, constructWorksiteFileParentsRequest, isWorksiteFileParentsRequest, isWorksiteFileParentsSuccess, isWorksiteContextObservation } from "../product/worksite_effect.js";
...
import { isWorksiteCommandExecutionObservation, constructWorksiteCommandConfiguration, isWorksiteReadDependencyBasis, WORKSITE_COMMAND_EXECUTION_IDS } from "../product/worksite_command_execution.js";
...
import { authenticateNativeInstructionAssemblyBasis, rehydrateExecutionBasisAtPrefix } from "./execution_basis.js";
...
function rootOf(prefix, execution) {
    ...
    while (root !== null && root.parentExecutionBasisRef !== null) {
        if (seen.has(root.basisRef))
            return null;
        seen.add(root.basisRef);
        const next = rehydrateExecutionBasisAtPrefix(prefix, root.parentExecutionBasisRef);
        if (next === null || !sameJobInvocation(execution, next))
            return null;
        root = next;
    }
    return root;
}
function sameJobInvocation(a, b) {
...
export function authenticateSemanticJobBasis(basis) {
    try {
        ...
        if (!isSemanticJobInput(root.rawInputValue) && !acquiring) {
            ...
            if (initial === null || initial.parentExecutionBasisRef !== null || initial.rawInputAdmissionRef !== envelope.basis.rootInputRef ||
                ...
                initial.invocationAdmissionRef !== envelope.basis.invocationAdmissionRef)
                return null;
            root = initial;
        }
        ...
        if (environment.kind !== "exact_prefix_workspace_environment" || invocation?.capabilityGrants.length !== 1 ||
            environment.productInstalls.filter(install => install.productId === publication.owningProductId &&
                ...
                install.manifestDigest === publication.productManifestDigest && install.contributionManifest.publicationBindings.filter(p => p.moduleRef === publication.moduleRef && p.publicationDigest === modulePublicationSemanticDigest(publication)).length === 1).length !== 1)
            return null;
        ...
        const role = nativeAuthor ? "author" : nativeAssessor ? "assessor" : native.call.implementationRef === ids.authorImplementationRef || native.call.implementationRef === revisionIds.authorImplementationRef ? "author" :
            native.call.implementationRef === ids.assessorImplementationRef || native.call.implementationRef === revisionIds.assessorImplementationRef ? "assessor" : null;
        ...
        if (role !== null && (stages.length !== 1 || native.call.regime !== (nativeStage ? "F_D" : "F_P")))
            return null;
...
function predecessor(basis, owner, value, implementationRef) {
    const matches = owner.events.filter(e => e.kind === "c_call_result_admitted" && record(e.payload) && equal(e.payload.value, value)).map(event => {
        ...
        if (previous === null || previous.result.resultClass !== "success" || previous.judgment.judgment !== "advance" ||
            (implementationRef !== undefined && previous.cCall.implementationRef !== implementationRef))
            return null;
...
export function semanticJobInputMatchesBasis(basis, input) {
    try {
        ...
        if (owner === null || !isSemanticJobEnvelope(input) ||
            ...
            input.basis.invocationAdmissionRef !== owner.root.invocationAdmissionRef || predecessor(basis, owner, input) === null)
            return false;
        ...
        if (intake === null || intake.cCall.implementationRef !== ids.jobIntakeImplementationRef || intake.result.resultClass !== "success" ||
            intake.judgment.judgment !== "advance" || !isSemanticJobEnvelope(intake.result.value) || !equal(intake.result.value.basis, input.basis))
            return false;
        for (const asset of input.assets) {
            ...
            if (author === null || author.cCall.implementationRef !== (authorSource === undefined ? ids.authorImplementationRef : ids.nativeAuthorFoldImplementationRef) || author.result.resultClass !== "success" ||
                ...
                !equal(author.result.value.basis, input.basis) || !equal(author.result.value.assets.find(a => a.assetRef === asset.assetRef), { ...asset, assessment: null }))
                return false;
            if (asset.assessment !== null) {
                ...
                if (assessed === null || assessed.cCall.implementationRef !== (assessmentSource === undefined ? ids.assessorImplementationRef : ids.nativeAssessorFoldImplementationRef) || assessed.result.resultClass !== "success" ||
                    ...
                    !equal(assessed.result.value.assets.find(a => a.assetRef === asset.assetRef), asset))
                    return false;
            }
        }
        return true;
...
export function projectSemanticJobIntake(basis, input) {
    try {
        const owner = authenticateSemanticJobBasis(basis);
        if (owner === null || owner.call.implementationRef !== ids.jobIntakeImplementationRef ||
            ...
            !equal(input, owner.root.rawInputValue) || !isSemanticJobInput(input))
            return null;
...
export function semanticJobContextMatches(basis, input, context) {
    const owner = authenticateSemanticJobBasis(basis);
    if (owner === null || !isWorksiteContextObservation(context))
        return false;
...
export function semanticJobContextCurrent(basis, input) {
    try {
        const owner = authenticateSemanticJobBasis(basis), context = input.context;
        if (owner === null || context === null || !semanticJobContextMatches(basis, input, context))
            return false;
        ...
        return context.entries.every(entry => {
            ...
            try {
                ...
                if (entry.state === "absent" || stat.isSymbolicLink() || realpathSync(path) !== path || `${stat.dev}:${stat.ino}` !== entry.fileIdentity)
                    return false;
                if (entry.state === "directory")
                    return stat.isDirectory() && equal(readdirSync(path).sort(), entry.members);
                ...
                return entry.state === "absent" && error.code === "ENOENT";
            }
        });
    }
    catch {
...
export async function projectSemanticJobContext(basis, input) {
    const owner = authenticateSemanticJobBasis(basis);
    if (owner === null || owner.call.implementationRef !== ids.jobContextImplementationRef || !semanticJobInputMatchesBasis(basis, input))
        return null;
...
function planInput(basis, input) {
    ...
    if (owner === null || asset?.assessment?.disposition !== "satisfied" || design === null || design === undefined ||
        design.dependencyDisposition !== "sufficient" || !semanticJobDesignMatches(input, design))
        return null;
    ...
    const targets = design.targets.map(target => {
        ...
        if (territory.kind !== "worksite_territory")
            throw new TypeError("unavailable Design territory");
    ...
    const protectedSubjects = targets.map(target => {
        ...
        if (subject.kind !== "worksite_subject")
            throw new TypeError("unavailable Design subject");
        return subject;
    });
...
export async function projectSemanticJobPlan(basis, input) {
    try {
        const owner = authenticateSemanticJobBasis(basis);
        if (owner?.call.implementationRef !== ids.jobPlanImplementationRef || !semanticJobInputMatchesBasis(basis, input))
            return null;
...
export function validateWorksiteFileParentsPlanAtPrefix(prefix, request, publication) {
    try {
        if (!isWorksiteFileParentsRequest(request) || publication.semanticJobLifecycle === undefined)
            return false;
        const events = runtimeEventsFromValidatedPrefix(prefix);
        const rows = events.filter(e => e.kind === "c_call_result_admitted" && record(e.payload) && equal(e.payload.value, request)).map(event => projectSemanticPredecessorAtPrefix(prefix, events, publication, event.aggregateId)).filter(p => p !== null &&
        ...
        if (source === null || source.cCall.implementationRef !== ids.assessorImplementationRef || source.result.resultClass !== "success" ||
            source.judgment.judgment !== "advance" || !isSemanticJobEnvelope(source.result.value))
            return false;
...
function originalForParents(basis, input) {
    const owner = authenticateSemanticJobBasis(basis);
    if (owner === null || !isWorksiteFileParentsSuccess(input) || !equal(owner.inputValue, input) ||
        ...
        predecessor(basis, owner, input, WORKSITE_FILE_PARENTS_IMPLEMENTATION.implementationRef) === null)
        return null;
...
async function observeDesignWorksite(basis, input) {
    ...
    for (const target of request.targets) {
        ...
        if (observation.kind !== "worksite_observation")
            return null;
        ...
        if (observation.state === "file" && (observation.fileDigest !== sha256Bytes(bytes) || observation.byteLength !== bytes.length))
            return null;
    ...
    for (const dependency of deriveSemanticJobReadDependencies(original, current)?.members ?? []) {
        ...
        if (!equal(observed, dependency.observation))
            return null;
    }
...
export async function projectSemanticJobBridge(basis, input) {
    try {
        const source = originalForParents(basis, input);
        if (source?.owner.call.implementationRef !== ids.jobBridgeImplementationRef)
            return null;
...
export function semanticJobSavedWorksite(original, value) {
    if (!isWorksitePreparationInput(value) || value.kind !== "worksite_command_preparation_input")
        return null;
    const task = value.constructionTask, design = original.assets.at(-1)?.candidate.design;
    if (design === null || design === undefined)
        return null;
    try {
        if (!equal(value.readDependencyBasis ?? null, deriveSemanticJobReadDependencies(original, task) ?? null))
            return null;
    }
    catch {
        return null;
...
export function projectSemanticJobEvidence(basis, input) {
    try {
        const owner = authenticateSemanticJobBasis(basis);
        if (owner?.call.implementationRef !== ids.evidenceInputImplementationRef || !equal(owner.inputValue, input) || !isWorksiteCommandExecutionObservation(input))
            return null;
        ...
        const bridges = owner.events.filter(e => e.kind === "c_call_result_admitted" && record(e.payload) && isWorksitePreparationInput(e.payload.value)).map(event => {
            ...
            if (bridge === null || bridge.event.eventId !== event.eventId || bridge.event.admissionOrdinal >= construction.event.admissionOrdinal ||
                !isWorksiteFileParentsSuccess(bridge.execution.rawInputValue))
                return null;
            ...
            if (!isSemanticJobEnvelope(original))
                return null;
        ...
        const original = savedOriginal;
        if (!equal(prepareWorksiteCommandTask({ kind: "worksite_command_preparation_bound_input", schemaVersion: "5.0.0", entry: expected, source: input.task.sourceConstructionResult }), input.task))
            return null;
        ...
        const artifacts = input.snapshotMembers.map(member => {
            ...
            if (rel.length === 0 || rel.startsWith("..") || isAbsolute(rel) || realpathSync(path) !== path)
                throw new TypeError("snapshot locus mismatch");
            ...
            if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1 || bytes.length !== member.byteLength || sha256Bytes(bytes) !== member.digest || protectedRow === undefined)
                throw new TypeError("snapshot identity mismatch");
            return { subjectRef: protectedRow.subject.subjectRef, observationRef: member.sourceObservationRef, base64: bytes.toString("base64"), role: target?.role === "verifier" ? "verifier_artifact" : "realization" };
        });
        return deepFreeze({ ...original, worksite, evidence: { kind: "semantic_worksite_evidence", constructionResultRef: construction.previous.result.resultRef,
                ...
                executionResultDigest: execution.previous.result.resultDigest, constructionResult: input.task.sourceConstructionResult,
...
export function semanticJobResultMatchesBasis(basis, input, output) {
    try {
        const owner = authenticateSemanticJobBasis(basis);
        if (owner === null || !equal(owner.inputValue, input))
            return false;
        const implementation = owner.call.implementationRef;
        if (NATIVE_SEMANTIC_IMPLEMENTATION_REFS.some(r => r === implementation))
            return nativeSemanticResultMatchesBasis(basis, input, output);
        if (implementation === ids.jobIntakeImplementationRef)
            return equal(projectSemanticJobIntake(basis, input), output);
        if (implementation === ids.evidenceInputImplementationRef)
            return equal(projectSemanticJobEvidence(basis, input), output);
        ...
        if (!semanticJobInputMatchesBasis(basis, input))
            return false;
        ...
        if (!isSemanticJobEnvelope(output))
            return false;
        if (implementation === ids.jobContextImplementationRef)
            return output.context !== null && semanticJobContextMatches(basis, input, output.context) && equal({ ...input, context: output.context }, output);
        if (implementation === ids.terminalImplementationRef)
            return equal(input, output);
        const asset = output.assets.at(-1), source = owner.role === "author" ? asset?.source : asset?.assessment?.source;
        if (owner.stage === undefined || source === undefined || source.cCallRef !== owner.call.cCallRef || source.inputDigest !== owner.inputDigest)
            return false;
...
function semanticJobConstructionSourceMatching(prefix, matches) {
    try {
        const events = runtimeEventsFromValidatedPrefix(prefix);
        const rows = events.flatMap(event => {
            if (event.kind !== "c_call_result_admitted" || !record(event.payload) || !isWorksitePreparationInput(event.payload.value) ||
                event.payload.value.kind !== "worksite_command_preparation_input")
                return [];
            const judgment = events.find(e => e.kind === "c_call_judged" && e.aggregateId === event.aggregateId);
            if (judgment === undefined)
                return [];
            const value = event.payload.value;
            const outcome = projectWorksiteRevisionNativeResult(prefix, { cCallRef: event.aggregateId, resultRef: event.payload.resultRef,
            ...
            if (outcome?.cCall.implementationRef !== ids.jobBridgeImplementationRef || outcome.result.resultClass !== "success" ||
                outcome.judgment.judgment !== "advance" || seed === null || !isWorksiteFileParentsSuccess(seed.rawInputValue))
                return [];
            ...
            if (!isSemanticJobEnvelope(original) || original.assets.at(-1)?.assetRef !== request.sourceDesignAssetRef ||
                original.assets.at(-1)?.assetDigest !== request.sourceDesignAssetDigest || !matches(original, request))
                return [];
...
export function semanticJobConstructionSourceAtPrefix(prefix, envelope) {
...
export function semanticJobReadDependenciesAtPrefix(prefix, readBasis) {
    try {
        if (!isWorksiteReadDependencyBasis(readBasis))
            return null;
        const events = runtimeEventsFromValidatedPrefix(prefix);
        ...
        if (source === null || !equal(deriveSemanticJobReadDependencies(source.original, source.worksite), readBasis))
            return null;
        ...
        const contexts = events.flatMap(event => {
            if (event.kind !== "c_call_result_admitted" || !record(event.payload) || !isSemanticJobEnvelope(event.payload.value) ||
                !equal(event.payload.value.context, original.context) || !equal(event.payload.value.basis, original.basis))
                return [];
...
function nativeStageSource(basis, envelope, source) {
    const owner = authenticateSemanticJobBasis(basis);
    if (owner === null || !semanticJobInputMatchesBasis(basis, envelope))
        return null;
    ...
    if (native === null || native.sourceResult.runId !== owner.call.runId || !sameJobInvocation(owner.execution, native.sourceBasis) ||
        ...
        worksiteCommandSourcesInvalidatedAfter(owner.prefix, native.sourceResult.admissionOrdinal, source.task.workspaceAuthorityBasis.canonicalRoot, [], source.after.readRoots))
        return null;
    return { owner, native };
}
export async function projectNativeSemanticTask(basis, input, selectedLimits) {
    const owner = authenticateSemanticJobBasis(basis);
    if (owner === null || !semanticJobInputMatchesBasis(basis, input))
        return null;
    ...
    try {
        ...
        if (retained !== null && (selectedLimits === undefined || !equal(retained, selectedLimits)))
            return null;
    }
    catch {
        return null;
    ...
    if (!isWorksiteContextObservation(context))
        return null;
    try {
        if (owner.call.implementationRef === ids.nativeConstructionTaskImplementationRef)
            return constructNativeSemanticConstructionTask(input, operating(owner), context);
...
export function projectNativeSemanticFold(basis, input, onContractIssues) {
    try {
        if (!isRetainedGraphInput(input) || !isSemanticJobEnvelope(input.entry) || !isNativeWorkspaceWorkObservation(input.source))
            return null;
        const source = nativeStageSource(basis, input.entry, input.source);
        if (source === null || source.owner.stage === undefined || source.owner.role === null)
            return null;
        ...
        if (!equal(expected, input.source.task))
            return null;
...
export function projectNativeSemanticExecution(basis, input) {
    try {
        if (!isRetainedGraphInput(input) || !isSemanticJobEnvelope(input.entry) || !isNativeWorkspaceWorkObservation(input.source))
            return null;
        const source = nativeStageSource(basis, input.entry, input.source);
...
function nativeSemanticResultMatchesBasis(basis, input, output) {
    const owner = authenticateSemanticJobBasis(basis);
    if (owner === null)
        return false;
    const implementation = owner.call.implementationRef;
    if (implementation === ids.nativeAuthorFoldImplementationRef || implementation === ids.nativeAssessorFoldImplementationRef)
        return equal(projectNativeSemanticFold(basis, input), output);
    if (implementation === ids.nativeExecutionTaskImplementationRef)
        return equal(projectNativeSemanticExecution(basis, input), output);
    if (implementation === ids.nativeEvidenceImplementationRef)
        return equal(projectNativeSemanticEvidence(basis, input), output);
    if (!isSemanticJobEnvelope(input) || !semanticJobInputMatchesBasis(basis, input) || !isNativeWorkspaceWorkTask(output) ||
        !semanticJobContextMatches(basis, input, output.context))
        return false;
    try {
...
export function projectNativeSemanticEvidence(basis, input) {
    try {
        const owner = authenticateSemanticJobBasis(basis);
        if (owner?.call.implementationRef !== ids.nativeEvidenceImplementationRef || !isRetainedGraphInput(input) ||
            !isSemanticJobEnvelope(input.entry) || !semanticJobInputMatchesBasis(basis, input.entry) || !isNativeWorksiteCommandExecutionObservation(input.source))
            return null;
        ...
        if (!record(payload) || typeof payload.resultRef !== "string" || typeof payload.resultDigest !== "string")
            return null;
        ...
        return deepFreeze({ ...original, context: source.after, worksite: null, evidence: { kind: "semantic_worksite_evidence",
                constructionResultRef: payload.resultRef, constructionResultDigest: payload.resultDigest,
