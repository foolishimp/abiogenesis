...
import { NATIVE_WORKSPACE_WORK_IDS as nativeIds, isNativeWorkspaceWorkTask, nativeWorkspaceWorkResultContractRef, isNativeWorkspaceWorkReport, isNativeWorkspaceWorkObservation, isNativeWorkspaceWorkFailure, resolveNativeWorkspaceWorkJudgmentRelation } from "./native_workspace_work.js";
...
import { isWorksiteCommandForwardRequest, isWorksiteCommandForwardTask, isWorksiteCommandForwardWorkerResult } from "./worksite_command_forward.js";
...
import { isSemanticRevisionRequest, isSemanticRevisionEnvelope, isSemanticRevisionSelection, isSemanticRevisionSelectionInput, deriveRevisionAsset, deriveRevisionAssessment, isSemanticJobRevisionEnvelope, deriveJobRevisionAsset, deriveJobRevisionAssessment, isNativeSemanticRevisionIntake, evaluateNativeSemanticRevisionRelation } from "./semantic_revision.js";
...
function admitInput(contractRef, value) {
    ...
    if (contractRef === WORKSITE_REVISION_IDS.taskContractRef && isWorksiteRevisionCommandExecutionTask(value))
        return deepFreeze(value);
    ...
    if (contractRef === WORKSITE_BRANCH_CONSTRUCTION_IDS.taskContractRef &&
        isWorksiteBranchConstructionTask(value)) {
        return deepFreeze(value);
    }
    if (contractRef === WORKSITE_BRANCH_CONSTRUCTION_IDS.vectorContractRef &&
    ...
    if (contractRef === WORKSITE_CONSTRUCTION_IDS.taskContractRef &&
        isWorksiteConstructionTask(value)) {
        return deepFreeze(value);
    }
    if (contractRef === WORKSITE_CONSTRUCTION_IDS.candidateBundleContractRef &&
        isWorksiteCandidateBundle(value)) {
        return deepFreeze(value);
    }
    if (contractRef === WORKSITE_CONSTRUCTION_IDS.fileReplaceVectorContractRef &&
...
export const ABI5_PRODUCT_SEMANTICS = Object.freeze({
    ...
    admitInput,
    evaluateInteractionResponse(basis, responseCandidate) {
    ...
    resolveProbabilisticWorkerContracts(basis) {
        if (basis.inputContractRef === nativeIds.taskContractRef && basis.outputContractRef === nativeIds.observationContractRef &&
            isNativeWorkspaceWorkTask(basis.input))
            return Object.freeze({ instructionContractRef: nativeIds.taskContractRef,
        ...
        if (basis.inputContractRef === forwardIds.taskContractRef && basis.outputContractRef === forwardIds.observationContractRef &&
            isWorksiteCommandForwardTask(basis.input))
            return Object.freeze({ instructionContractRef: forwardIds.taskContractRef,
        ...
        if (basis.inputContractRef === WORKSITE_REVISION_IDS.taskContractRef && basis.outputContractRef === WORKSITE_REVISION_IDS.observationContractRef &&
            isWorksiteRevisionCommandExecutionTask(basis.input))
            return Object.freeze({ instructionContractRef: WORKSITE_REVISION_IDS.taskContractRef,
                resultContractRef: WORKSITE_REVISION_IDS.workerResultContractRef });
        if (basis.inputContractRef === WORKSITE_COMMAND_EXECUTION_IDS.taskContractRef &&
        ...
        if (basis.inputContractRef === WORKSITE_CONSTRUCTION_IDS.taskContractRef &&
    ...
    validateInvocationBasis(basis) {
        if (isNativeWorkspaceWorkTask(basis.input))
        ...
        if (isNativeWorksiteCommandReacquisitionRequest(basis.input))
            return basis.sourceResultBasis === null &&
                basis.input.workspaceBinding.bindingId === basis.workspaceBindingId &&
                basis.input.workspaceBinding.bindingDigest === basis.workspaceBindingDigest &&
                basis.input.workspaceBinding.workspaceId === basis.workspaceId;
        if (isWorksiteCommandForwardRequest(basis.input))
            return basis.sourceResultBasis === null &&
                basis.input.workspaceBinding.bindingId === basis.workspaceBindingId &&
                basis.input.workspaceBinding.bindingDigest === basis.workspaceBindingDigest &&
                basis.input.workspaceBinding.workspaceId === basis.workspaceId;
        if (isWorksitePreparationInput(basis.input)) {
            ...
            return basis.sourceResultBasis === null && tasks.length > 0 && tasks.every((task) => task.workspaceBinding.bindingId === basis.workspaceBindingId &&
        ...
        if (isC2WorksiteCommandExecutionTask(basis.input)) {
            ...
            if (isObservedWorksiteCommandExecutionTask(basis.input))
                return source === null &&
                    basis.input.workspaceBinding.bindingId === basis.workspaceBindingId &&
                    basis.input.workspaceBinding.bindingDigest === basis.workspaceBindingDigest &&
                    basis.input.workspaceBinding.workspaceId === basis.workspaceId;
            if (isNativeWorksiteCommandExecutionTask(basis.input))
                return source !== null &&
                    source.sourceGraphFunctionRef === nativeIds.graphFunctionRef && source.sourceResultContractRef === nativeIds.observationContractRef &&
                    source.sourceResultValueDigest === sha256Canonical(basis.input.sourceNativeWork) &&
            ...
            return source !== null &&
                ...
                source.sourceResultValueDigest === sha256Canonical(basis.input.sourceConstructionResult) &&
                sha256Canonical(source.sourceResultValue) === source.sourceResultValueDigest &&
                sha256Canonical(source.sourceResultValue) === sha256Canonical(basis.input.sourceConstructionResult) &&
                source.sourceWorkspaceId === basis.workspaceId &&
                source.workspaceBindingId === basis.workspaceBindingId &&
                source.workspaceBindingDigest === basis.workspaceBindingDigest;
        }
        if (isWorksiteBranchConstructionVector(basis.input) ||
...
export const ABI5_NATIVE_WORKSPACE_WORK_PRODUCT_SEMANTICS = Object.freeze({
    ...
    validateResultEvidenceLineage(basis) {
        if (basis.outputContractRef !== nativeIds.observationContractRef)
            return true;
        if (!isNativeWorkspaceWorkObservation(basis.value) || basis.admittedEvidence.length !== 1)
            return false;
        ...
        return e.evidenceClass === "probabilistic_transport" && e.cCallRef === p.cCallRef &&
            e.transportDigest === p.transportDigest && e.outputDigest === sha256Canonical(basis.value);
    },
});
function resolveWorksiteCommandForwardJudgmentRelation(predicateRef) {
    if (![forwardIds.stepPredicateRef, forwardIds.rootPredicateRef, forwardIds.judgmentPredicateRef].includes(predicateRef))
        return null;
    ...
    return Object.freeze({ predicateRef, advanceReasonRef: "reason://abiogenesis/worksite/command-forward/observed@5",
        ...
        evaluate: (input, output) => {
            ...
            if (!isWorksiteCommandForwardObservation(output))
                return false;
...
function hasExactConsensusCatalogApplications(basis, invocation) {
    const expected = consensusCatalogApplicationBindings(invocation);
    const applications = basis.catalogApplications ?? [];
    if (applications.length !== expected.length)
        return false;
    return expected.every((binding) => {
        const expectedKind = binding.handle === CONSENSUS_IDS.rulingOverlayCatalogHandle
            ? "overlay"
            : "node_type";
        const expectedContractRef = binding.handle === CONSENSUS_IDS.subjectCatalogHandle
        ...
        const expectedTargetDigest = binding.nodeTypeTarget === null
            ? null
            : sha256Canonical(binding.nodeTypeTarget);
        const matching = applications.filter((application) => application.kind === "declaration_application" &&
...
function admitSystemInput(contractRef, value) {
    if (contractRef === CONSENSUS_IDS.invocationContractRef &&
        isConsensusInvocation(value)) {
        return deepFreeze(value);
    }
    if (contractRef === CONSENSUS_IDS.observationContractRef &&
        isConsensusObservationSnapshot(value)) {
        return deepFreeze(value);
    }
    if (contractRef === CONSENSUS_IDS.resolutionContractRef &&
        isConsensusEscalationRequest(value)) {
        return deepFreeze(value);
    }
    if (contractRef === CONSENSUS_IDS.escalationDecisionContractRef &&
        isConsensusEscalationDecision(value)) {
        return deepFreeze(value);
    }
    return null;
}
function validateSystemContractValue(valueKind, value) {
    return validateConsensusContractValue(valueKind, value) &&
        isRecord(value);
}
function resolveSystemCatalogApplicationValue(basis) {
    const value = basis.value;
    if (basis.contractRef === CONSENSUS_IDS.subjectContractRef &&
        isConsensusSubject(value)) {
        return { valueRef: value.subjectRef, programMembershipRefs: [] };
    }
    if (basis.contractRef === CONSENSUS_IDS.profileContractRef &&
        isConsensusReviewerProfile(value)) {
        return { valueRef: value.profileRef, programMembershipRefs: [] };
    }
    if (basis.contractRef === CONSENSUS_IDS.reviewerInstructionContractRef &&
    ...
    if (basis.contractRef === CONSENSUS_IDS.submitterProfileContractRef &&
        isConsensusSubmitterProfile(value)) {
        return { valueRef: value.profileRef, programMembershipRefs: [] };
    }
    if (basis.contractRef === CONSENSUS_IDS.submitterInstructionContractRef &&
    ...
    if (basis.contractRef === CONSENSUS_IDS.policyContractRef &&
        isConsensusRoundPolicy(value)) {
        return { valueRef: value.policyRef, programMembershipRefs: [] };
    }
    if (basis.contractRef === CONSENSUS_IDS.rulingOverlayContractRef &&
...
function validateSystemResultEvidenceLineage(basis) {
    if (basis.outputContractRef !== CONSENSUS_IDS.findingsContractRef &&
        basis.outputContractRef !== CONSENSUS_IDS.submitterResponseContractRef) {
        return true;
    }
    if (basis.admittedEvidence.length !== 1 ||
    ...
    const expectedEvidenceRef = `transport-evidence://abg/${transportDigest.slice("sha256:".length)}`;
    if (basis.outputContractRef === CONSENSUS_IDS.findingsContractRef &&
        ...
        return basis.admittedEvidence[0]?.cCallRef ===
            ...
            basis.value.findings.every((finding) => finding.evidenceRefs.length === 1 &&
...
export const ABI5_SYSTEM_PRODUCT_SEMANTICS = Object.freeze({
    ...
    evaluateInteractionResponse(basis, responseCandidate) {
        if (basis.requestContractRef ===
            ...
            if (!isConsensusEscalationRequest(basis.requestValue) ||
                ...
                sha256Canonical(responseCandidate.roundDecision) !==
                    sha256Canonical(basis.requestValue)) {
                return null;
            }
            return deepFreeze(responseCandidate);
        }
    ...
    resolveProbabilisticWorkerContracts(basis) {
        if (basis.inputContractRef === CONSENSUS_IDS.reviewerTaskContractRef &&
            basis.outputContractRef === CONSENSUS_IDS.findingsContractRef &&
            isConsensusReviewerTask(basis.input)) {
            return Object.freeze({
                instructionContractRef: basis.input.profile.instructionContractRef,
                resultContractRef: basis.input.profile.resultContractRef,
            });
        }
        if (basis.inputContractRef === CONSENSUS_IDS.submitterTaskContractRef &&
            basis.outputContractRef === CONSENSUS_IDS.submitterResponseContractRef &&
    ...
    validateInvocationBasis(basis) {
        if (isConsensusInvocation(basis.input)) {
            return basis.sourceResultBasis === null &&
                ...
                hasExactConsensusCatalogApplications(basis, basis.input);
        }
        if (isConsensusObservationSnapshot(basis.input)) {
            return basis.sourceResultBasis === null &&
                ...
                basis.input.consensusInvocation.subject.workspaceRef ===
                    basis.workspaceId &&
                hasExactConsensusCatalogApplications(basis, basis.input.consensusInvocation) &&
                basis.actionCatalog !== null &&
                sha256Canonical(basis.input.actionCatalog) === sha256Canonical(basis.actionCatalog);
        }
        return basis.sourceResultBasis === null;
    },
    projectPublicResult(basis) {
        if (basis.projectionKind !== "result" &&
        ...
        if (basis.admittedResultContractRef !==
            CONSENSUS_IDS.resultCandidateContractRef) {
            return null;
        }
        const result = bindConsensusReplay(basis.value, basis.admittedResultRef, basis.replayRef);
        return {
            kind: "product_public_result_projection",
            schemaVersion: "5.0.0",
            ...
            value: (basis.projectionKind === "ticket.consensus"
                ? projectTicketConsensus(result)
                : result),
        };
    },
});
export const ABI5_REQUIREMENT_HANDOFF_PRODUCT_SEMANTICS = Object.freeze({
    ...
    resolveJudgmentRelation(predicateRef) {
        return predicateRef === REQUIREMENT_HANDOFF_IDS.judgmentPredicateRef ? {
            ...
            evaluate(input, output) {
                return isRequirementHandoffInput(input) && isRequirementHandoffOutput(output) && sha256Canonical(input) === sha256Canonical(output.source);
            },
        } : null;
    },
});
export const ABI5_SEMANTIC_STAGE_PRODUCT_SEMANTICS = Object.freeze({
    ...ABI5_PRODUCT_SEMANTICS,
    ...
    admitInput(contractRef, value) {
        if ((contractRef === revisionIds.nativeIntakeContractRef && isNativeSemanticRevisionIntake(value)) || (contractRef === revisionIds.selectionContractRef && isSemanticRevisionSelection(value)) || (contractRef === revisionIds.selectionInputContractRef && isSemanticRevisionSelectionInput(value)) || (contractRef === revisionIds.requestContractRef && isSemanticRevisionRequest(value)) ||
            (contractRef === revisionIds.envelopeContractRef && (isSemanticRevisionEnvelope(value) || isSemanticJobRevisionEnvelope(value))))
            return deepFreeze(value);
        if (contractRef === SEMANTIC_STAGE_IDS.jobInputContractRef && isSemanticJobInput(value))
            return deepFreeze(value);
    ...
    resolveProbabilisticWorkerContracts(basis) {
        if (basis.inputContractRef === revisionIds.selectionInputContractRef && basis.outputContractRef === revisionIds.selectionContractRef && isSemanticRevisionSelectionInput(basis.input))
            return { instructionContractRef: revisionIds.selectionInputContractRef, resultContractRef: revisionIds.selectionRawContractRef };
    ...
    resolveJudgmentRelation(predicateRef) {
        if ([revisionIds.nativeIntakePredicateRef, revisionIds.nativeRequestPredicateRef, revisionIds.nativeConstructionPredicateRef, revisionIds.nativeExecutionPredicateRef, revisionIds.nativeEvidencePredicateRef].some(ref => ref === predicateRef))
            return { predicateRef, advanceReasonRef: "reason://abiogenesis/semantic-revision/native-source-satisfied@5", rejectionReasonRef: "reason://abiogenesis/semantic-revision/native-source-refused@5",
                evaluate: (input, output) => evaluateNativeSemanticRevisionRelation(predicateRef, input, output) === true };
            ...
            return { predicateRef, advanceReasonRef: "reason://abiogenesis/semantic-revision/step-satisfied@5",
                ...
                evaluate(input, output) {
                    try {
                        ...
                        const matches = (ref, before, after) => ABI5_SEMANTIC_STAGE_PRODUCT_SEMANTICS.resolveJudgmentRelation(ref)?.evaluate(before, after) === true;
                        ...
                        const evidenceMatches = (envelope, observation) => {
                            if ((!isSemanticRevisionEnvelope(envelope) && !isSemanticJobRevisionEnvelope(envelope)) || !isWorksiteRevisionCommandExecutionObservation(observation))
                                return false;
                            ...
                            const resultCoordinate = (ref, digest) => isSha256Digest(digest) &&
                        ...
                        if (isSemanticRevisionRequest(input))
                            return matches(revisionIds.projectionPredicateRef, input, output);
                        if (isWorksiteRevisionCommandExecutionObservation(input))
                            return evidenceMatches(output, input);
                        ...
                        if (!isSemanticRevisionEnvelope(input) && !isSemanticJobRevisionEnvelope(input))
                            return false;
                        ...
                        if ((!isSemanticRevisionEnvelope(output) && !isSemanticJobRevisionEnvelope(output)) || !equal(input.revisionBasis, output.revisionBasis))
                            return false;
        ...
        if ([revisionIds.selectionPredicateRef, revisionIds.projectionPredicateRef, revisionIds.authorPredicateRef, revisionIds.assessorPredicateRef, revisionIds.bridgePredicateRef,
            ...
            return { predicateRef, advanceReasonRef: "reason://abiogenesis/semantic-revision/relation-satisfied@5", rejectionReasonRef: "reason://abiogenesis/semantic-revision/relation-refused@5",
                evaluate(input, output) {
                    try {
                        const equal = (a, b) => sha256Canonical(a) === sha256Canonical(b);
                        if (predicateRef === revisionIds.selectionPredicateRef)
                            return isSemanticRevisionSelectionInput(input) && isSemanticRevisionSelection(output) && equal(input.parent, output.parent) && equal(input.causes, output.causes);
                        if (predicateRef === revisionIds.projectionPredicateRef)
                            return isSemanticRevisionRequest(input) && (isSemanticRevisionEnvelope(output) || isSemanticJobRevisionEnvelope(output)) && equal(input, output.revisionBasis.request);
                        if (predicateRef === revisionIds.bridgePredicateRef)
                            return (isSemanticRevisionEnvelope(input) || isSemanticJobRevisionEnvelope(input)) && isWorksitePreparationInput(output) && output.kind === "worksite_revision_command_preparation_input";
                        if (predicateRef === revisionIds.evidenceInputPredicateRef)
                            return isWorksiteRevisionCommandExecutionObservation(input) && (isSemanticRevisionEnvelope(output) || isSemanticJobRevisionEnvelope(output)) && equal(input, output.current.evidence?.executionObservation);
                        if (isSemanticJobRevisionEnvelope(input) || isSemanticJobRevisionEnvelope(output)) {
                            if (!isSemanticJobRevisionEnvelope(input) || !isSemanticJobRevisionEnvelope(output))
                                return false;
                            if (predicateRef === revisionIds.terminalPredicateRef)
                                return equal(input, output);
                            const asset = output.current.assets.at(-1);
                            if (asset === undefined)
                                return false;
                            if (predicateRef === revisionIds.authorPredicateRef)
                                return equal(deriveJobRevisionAsset(input, asset.stageRef, asset.candidate, asset.source), output);
                        ...
                        if (!isSemanticRevisionEnvelope(input) || !isSemanticRevisionEnvelope(output))
                            return false;
                        if (predicateRef === revisionIds.terminalPredicateRef)
                            return equal(input, output);
                        const asset = output.current.assets.at(-1);
                        if (asset === undefined)
                            return false;
                        if (predicateRef === revisionIds.authorPredicateRef)
                            return equal(deriveRevisionAsset(input, asset.stageRef, asset.candidate, asset.source), output);
                        if (asset.assessment?.disposition !== "satisfied")
                            return false;
        ...
        if ([ids.nativeAuthorTaskPredicateRef, ids.nativeAuthorFoldPredicateRef, ids.nativeAssessorTaskPredicateRef,
            ...
            return { predicateRef, advanceReasonRef: "reason://abiogenesis/semantic-stage/native-source-satisfied@5",
                ...
                evaluate: (input, output) => evaluateNativeSemanticRelation(predicateRef, input, output) === true };
        }
                ...
                try {
                    ...
                    if (predicateRef === ids.lifecycleStepPredicateRef) {
                        ...
                        if (isSemanticStageEnvelope(input) && isSemanticStageEnvelope(output)) {
                            ...
                            if (equal(input, output))
                                return matches(ids.lifecyclePredicateRef, input, output);
                        ...
                        if (isSemanticStageEnvelope(input))
                            return matches(ids.bridgePredicateRef, input, output);
                        if (isWorksiteCommandExecutionObservation(input))
                            return matches(ids.evidenceInputPredicateRef, input, output);
                    ...
                    if (predicateRef === ids.bridgePredicateRef)
                        return isSemanticStageEnvelope(input) &&
                    ...
                    if (!isSemanticStageEnvelope(output))
                        return false;
                    if (predicateRef === ids.evidenceInputPredicateRef)
                        return isWorksiteCommandExecutionObservation(input) && output.evidence !== null && equal(output.evidence.executionObservation, input);
                    if (!isSemanticStageEnvelope(input))
                        return false;
                    ...
                    if (predicateRef === ids.authorPredicateRef)
                        return equal(deriveSemanticAsset(input, asset.stageRef, asset.candidate, asset.source), output);
                    if (predicateRef === ids.assessorPredicateRef) {
                        ...
                        return assessmentInput !== null && equal(deriveSemanticAssessment(assessmentInput, asset.stageRef, asset.assessment.candidate, asset.assessment.source), output);
                    }
    ...
    validateInvocationBasis(basis) {
        if (isSemanticJobInput(basis.input))
            return basis.sourceResultBasis === null;
        if (isSemanticJobEnvelope(basis.input))
            return false; // only admitted native children carry job envelopes
        if (!isSemanticStageEnvelope(basis.input))
            return ABI5_PRODUCT_SEMANTICS.validateInvocationBasis(basis);
...
export const ABI5_SEMANTIC_REVISION_PRODUCT_SEMANTICS = Object.freeze({ ...ABI5_SEMANTIC_STAGE_PRODUCT_SEMANTICS, bindingRef: revisionIds.semanticsBindingRef });
