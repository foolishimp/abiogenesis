...
import { validateImplementationResolutionSet, validateProgram, } from "../validator/index.js";
...
export function isProductExecutionResolution(value) {
    try {
        ...
        if (candidate.kind !== "product_execution_resolution" ||
            ...
            return false;
        const { kind: _kind, schemaVersion: _schemaVersion, disposition: _disposition, resolutionRef: _resolutionRef, resolutionDigest: _resolutionDigest, ...body } = candidate;
        const membershipDigest = sha256Canonical({
            programRef: candidate.programRef,
            graphFunctionRef: candidate.graphFunctionRef,
        });
        const membershipRef = `program-graph-function-membership://abiogenesis/${membershipDigest.slice("sha256:".length)}`;
        return candidate.programGraphFunctionMembership.digest ===
            membershipDigest &&
            candidate.programGraphFunctionMembership.ref === membershipRef &&
            candidate.inputContractDigest === sha256Canonical(candidate.inputContract) && candidate.outputContractDigest === sha256Canonical(candidate.outputContract) && candidate.resolutionDigest === sha256Canonical(body) && candidate.resolutionRef ===
...
function refusal(code, stage, message) {
    return deepFreeze({
        kind: "product_execution_resolution_refusal",
        schemaVersion: "5.0.0",
        disposition: "refused",
        code,
        stage,
...
function exact(values, predicate) {
    const matches = values.filter(predicate);
    return matches.length === 1
        ? { kind: "one", value: matches[0] }
        : { kind: matches.length === 0 ? "absent" : "ambiguous" };
}
function candidateFromInstall(install) {
    const { kind: _kind, disposition: _disposition, admissionEventRef: _admissionEventRef, ...body } = install;
    return {
...
function installMatchesPublication(install, publication) {
...
function exactInstallForCoordinate(installs, coordinate) {
    return exact(installs, (install) => install.installId === coordinate.installId &&
...
function ownerCoordinate(install, publication) {
    return deepFreeze({
...
function closureRefusal(result) {
    return refusal(result.code === "absent"
        ? "absent"
        : result.code === "ambiguous"
            ...
            : result.code === "missing_dependency"
                ...
                : result.code === "wrong_owner"
                    ? "wrong_owner"
                    : "incompatible_or_unproven", "declaration_closure", result.message);
}
async function resolveProductExecution(input) {
    ...
    if (reconstructedCatalog === null ||
        reconstructedCatalog.kind !== "graph_function_catalog" ||
        canonicalJson(reconstructedCatalog) !==
            canonicalJson(input.catalog)) {
        return refusal("incompatible_or_unproven", "catalog", "execution resolution requires the exact reconstructed ready Catalog");
    }
    const reconstructedView = (() => {
        try {
            return narrowGraphFunctionCatalog(reconstructedCatalog, input.catalogView.allowlist);
        }
        catch {
            return null;
        }
    })();
    if (reconstructedView === null ||
        reconstructedView.kind !== "graph_function_catalog_view" ||
        canonicalJson(reconstructedView) !==
            canonicalJson(input.catalogView)) {
        return refusal("incompatible_or_unproven", "catalog", "execution resolution requires the exact reconstructed CatalogView");
    }
    if (input.admittedInstalls.length !==
        ...
        new Set(input.admittedInstalls.map((install) => install.admissionEventRef)).size !== input.admittedInstalls.length ||
        input.admittedInstalls.some((install) => !input.verifyInstallAdmission(install) ||
            reconstructedCatalog.readinessBasis.installedProducts.filter((candidate) => canonicalJson(candidate) ===
                canonicalJson(candidateFromInstall(install))).length !== 1) ||
        reconstructedCatalog.readinessBasis.installedProducts.some((candidate) => input.admittedInstalls.filter((install) => canonicalJson(candidateFromInstall(install)) === canonicalJson(candidate)).length !== 1)) {
        return refusal("incompatible_or_unproven", "dependency", "execution resolution requires one ABG-admitted install per exact Catalog readiness candidate");
    }
    const programDeclarationClosure = resolveProgramDeclarationClosure(reconstructedCatalog, reconstructedView, input.programRef);
    ...
    const programMatches = programPublication.programs.filter((candidate) => candidate.programRef === programDeclarationClosure.programRef);
    if (programMatches.length !== 1) {
        return refusal(programMatches.length === 0 ? "absent" : "ambiguous", "declaration_closure", "selected Program does not reproduce from its exact publication owner");
    }
    const program = programMatches[0];
    let selectedCatalogEntry;
    let resolvedProgramStart = null;
    if (input.selection.kind === "direct") {
        const selected = lookupGraphFunction(reconstructedView, input.selection.catalogHandle);
        if (selected === null) {
            return refusal("absent", "catalog", "direct execution selection requires one exact canonical CatalogView handle");
        }
        selectedCatalogEntry = selected;
    }
    else if (input.selection.kind === "start" &&
        "graphFunctionHandle" in input.selection) {
        const selected = lookupGraphFunction(reconstructedView, input.selection.graphFunctionHandle);
        if (selected === null) {
            return refusal("absent", "catalog", "GraphFunction start requires one exact CatalogView handle");
        }
        selectedCatalogEntry = selected;
    }
    else if (input.selection.kind === "start") {
        ...
        if (resolved.kind !== "resolved_program_start") {
            return refusal(resolved.code === "ambiguous_target"
                ? "ambiguous"
                : resolved.code === "missing_target"
                    ? "absent"
                    : "incompatible_or_unproven", "catalog", resolved.message);
        }
        const selected = lookupGraphFunctionDefinition(reconstructedView, resolved.start.graphFunctionRef, input.programRef);
        if (selected.kind !== "graph_function_definition_lookup_exact") {
            return refusal(selected.kind === "graph_function_definition_lookup_ambiguous"
                ? "ambiguous"
                : "absent", "catalog", "Program start does not select one exact Program-aware CatalogView definition");
        }
        resolvedProgramStart = resolved;
        selectedCatalogEntry = selected.entry;
    }
    else if (input.selection.kind === "admitted") {
        const selected = lookupGraphFunctionDefinition(reconstructedView, input.selection.graphFunctionRef, input.programRef);
        if (selected.kind !== "graph_function_definition_lookup_exact") {
            return refusal(selected.kind === "graph_function_definition_lookup_ambiguous"
                ? "ambiguous"
                : "absent", "catalog", "admitted GraphFunction coordinate does not reproduce one exact Program-aware CatalogView definition");
        }
        selectedCatalogEntry = selected.entry;
    }
    else {
        return refusal("incompatible_or_unproven", "catalog", "execution selection is outside the closed direct, start, or admitted family");
    }
    if (!selectedCatalogEntry.programMembershipRefs.includes(input.programRef) ||
        !program.callableMembership.includes(selectedCatalogEntry.definitionRef)) {
        return refusal("wrong_owner", "catalog", "selected CatalogView definition is outside the exact Program callable membership");
    }
    const declarationClosure = resolveExecutionDeclarationClosure(reconstructedCatalog, reconstructedView, input.programRef, selectedCatalogEntry.definitionRef);
    ...
    const graphFunctionOwner = declarationClosure.graphFunctionOwners.find((owner) => owner.declarationRef === selectedCatalogEntry.definitionRef);
    if (graphFunctionOwner === undefined) {
        return refusal("wrong_owner", "declaration_closure", "selected GraphFunction lacks one exact declaration owner");
    }
    const graphFunctionPublicationMatch = publicationForCoordinate(publications, graphFunctionOwner);
    const graphFunctionMatches = graphFunctionPublicationMatch.kind === "one"
        ? graphFunctionPublicationMatch.value.graphFunctions.filter((candidate) => candidate.name === graphFunctionOwner.declarationRef)
        : [];
    if (graphFunctionPublicationMatch.kind !== "one" || graphFunctionMatches.length !== 1) {
        return refusal(graphFunctionPublicationMatch.kind === "ambiguous" ||
            graphFunctionMatches.length > 1
            ? "ambiguous"
            : "wrong_owner", "declaration_closure", "selected GraphFunction does not reproduce from its exact declaration owner");
    }
    const graphFunctionPublication = graphFunctionPublicationMatch.value;
    const graphFunction = graphFunctionMatches[0];
    if (selectedCatalogEntry.definitionRef !== graphFunctionOwner.declarationRef ||
        selectedCatalogEntry.definitionDigest !==
            sha256Canonical(graphFunction) ||
        ...
        canonicalJson(selectedCatalogEntry.definition) !==
            canonicalJson(graphFunction)) {
        return refusal("wrong_owner", "catalog", "Program-aware GraphFunction selection disagrees with its declaration owner");
    }
    if (graphFunction.inputs.length !== 1 ||
        graphFunction.outputs.length !== 1) {
        return refusal("incompatible_or_unproven", "declaration_closure", "execution resolution requires one exact input and output contract");
    }
    ...
    if (inputContract === null || outputContract === null) {
        return refusal("wrong_owner", "declaration_closure", "selected GraphFunction contracts lack exact resolved owners");
    }
    const programInstallMatch = exactInstallForPublication(input.admittedInstalls, programPublication);
    const graphFunctionInstallMatch = exactInstallForCoordinate(input.admittedInstalls, graphFunctionOwner);
    if (programInstallMatch.kind !== "one" ||
        graphFunctionInstallMatch.kind !== "one") {
        return refusal(programInstallMatch.kind === "ambiguous" ||
            graphFunctionInstallMatch.kind === "ambiguous"
            ? "ambiguous"
            : "wrong_owner", "dependency", "Program or selected GraphFunction lacks one exact admitted owner install");
    }
    const validationInput = constructCatalogProgramValidationInput(reconstructedCatalog, reconstructedView, programDeclarationClosure, program);
    if ("kind" in validationInput &&
        validationInput.kind === "raw_admission_refusal") {
        return refusal("incompatible_or_unproven", "declaration_closure", validationInput.message);
    }
    const programValidation = validateProgram(validationInput);
    if (programValidation.kind !== "program_validation") {
        return refusal(programValidation.diagnostics.some((diagnostic) => diagnostic.code === "duplicate_identity")
            ? "ambiguous"
            : "incompatible_or_unproven", "declaration_closure", "whole-Program declaration inventory failed static validation");
    }
    // Static shape proof and installed declaration authority are separate gates.
    const retentionGraphs = programDeclarationClosure.publications.flatMap((publication) => publication.graphFunctions.filter((candidate) => program.callableMembership.includes(candidate.name)));
    for (const graph of retentionGraphs)
        for (const edge of graph.template.edges) {
            ...
            const exactAbiOwner = (owner, moduleRef) => {
                if (owner === undefined || owner.productId !== ABI5_PRODUCT_ID || owner.moduleRef !== moduleRef)
                    return false;
                ...
                const installs = input.admittedInstalls.filter((install) => install.installId === owner.installId &&
                    ...
                    install.productId === ABI5_PRODUCT_ID && install.contributionManifest.publicationBindings.some((row) => row.moduleRef === moduleRef && row.publicationDigest === owner.publicationDigest));
            ...
            if (binding.targetContractRef === RETAINED_GRAPH_INPUT_CONTRACT.contractRef) {
                ...
                const sameOwner = (a, b) => a !== null && b !== null && a.productId === b.productId && a.installId === b.installId &&
    ...
    const implementationOwnerCoordinates = declarationClosure.implementationBindingOwners;
    ...
    for (const coordinate of implementationOwnerCoordinates) {
        ...
        if (publicationMatch.kind !== "one") {
            return refusal(publicationMatch.kind, "implementation", "ImplementationBinding owner does not resolve to one exact publication");
        }
    ...
    for (const ownerPublication of implementationPublicationMap.values()) {
        ...
        const installMatch = exactInstallForCoordinate(input.admittedInstalls, ownerCoordinateValue);
        ...
        if ("kind" in descriptors) {
            return refusal(descriptors.code === "ambiguous_implementation"
                ? "ambiguous"
                : "wrong_owner", "implementation", descriptors.message);
        }
        packagedImplementations.push(...descriptors);
    }
    const implementationSetCandidate = resolveImplementationSet(reconstructedView, declarationClosure, programValidation, packagedImplementations);
    if (implementationSetCandidate.kind !==
        "implementation_resolution_set_candidate") {
        return refusal(implementationSetCandidate.code === "ambiguous_implementation"
            ? "ambiguous"
            : implementationSetCandidate.code === "implementation_absent"
                ? "absent"
                : "incompatible_or_unproven", "implementation", implementationSetCandidate.message);
    }
    const implementationSetValidation = validateImplementationResolutionSet(implementationSetCandidate, reconstructedView, declarationClosure, programValidation, packagedImplementations);
    if (implementationSetValidation.kind !==
        "implementation_resolution_set_validation") {
        return refusal("incompatible_or_unproven", "implementation", "resolved implementation closure failed independent validation");
    }
    const closureOwner = declarationClosure.closureContractOwners.find((owner) => owner.declarationRef === program.closureContractRef);
    if (closureOwner === undefined) {
        return refusal("absent", "declaration_closure", "Program closure contract lacks one selected declaration owner");
    }
    const closurePublicationMatch = publicationForCoordinate(publications, closureOwner);
    const closureContractMatches = closurePublicationMatch.kind === "one"
        ? closurePublicationMatch.value.closureContracts.filter((contract) => contract.closureContractRef === closureOwner.declarationRef)
        : [];
    if (closurePublicationMatch.kind !== "one" ||
        closureContractMatches.length !== 1) {
        return refusal(closurePublicationMatch.kind === "ambiguous" ||
            closureContractMatches.length > 1
            ? "ambiguous"
            : "absent", "declaration_closure", "Program closure contract does not resolve exactly");
    }
    const closureContract = closureContractMatches[0];
    const semanticsPublicationMatch = publicationForCoordinate(publications, declarationClosure.semanticsOwner);
    const semanticsInstallMatch = exactInstallForCoordinate(input.admittedInstalls, declarationClosure.semanticsOwner);
    if (semanticsPublicationMatch.kind !== "one" ||
        semanticsInstallMatch.kind !== "one") {
        return refusal(semanticsPublicationMatch.kind === "ambiguous" ||
            semanticsInstallMatch.kind === "ambiguous"
            ? "ambiguous"
            : "wrong_owner", "semantics", "Product semantics lacks one exact publication and admitted owner install");
    }
    ...
    ].sort((left, right) => compareUnicodeCodeUnits(left.declarationKind, right.declarationKind) ||
    ...
    const programGraphFunctionMembership = deepFreeze({
        ref: `program-graph-function-membership://abiogenesis/${membershipDigest.slice("sha256:".length)}`,
    ...
    const body = deepFreeze({
        ...
        programRef: program.programRef,
        programDigest: sha256Canonical(program),
        ...
        graphFunctionOwner: ownerCoordinate(graphFunctionInstallMatch.value, graphFunctionPublication),
        programGraphFunctionMembership,
        inputContract: inputContract.contract,
        inputContractDigest: sha256Canonical(inputContract.contract),
        inputContractOwner: inputContract.owner,
        outputContract: outputContract.contract,
        outputContractDigest: sha256Canonical(outputContract.contract),
        ...
        implementationSetCandidateRef: implementationSetCandidate.setCandidateRef,
        implementationSetCandidateDigest: implementationSetCandidate.setCandidateDigest,
        implementationSetValidationRef: implementationSetValidation.validationRef,
        implementationSetValidationDigest: implementationSetValidation.validationDigest,
        closureContractRef: closureContract.closureContractRef,
        closureContractDigest: sha256Canonical(closureContract),
    });
    const resolutionDigest = sha256Canonical(body);
    const resolution = deepFreeze({
        kind: "product_execution_resolution",
        schemaVersion: "5.0.0",
        disposition: "resolved",
        resolutionRef: `product-execution-resolution://abiogenesis/${resolutionDigest.slice("sha256:".length)}`,
        resolutionDigest,
        ...body,
    });
    const selectedInstallIds = new Set(declarationOwners.map((owner) => owner.installId));
    selectedInstallIds.add(programInstallMatch.value.installId);
    return Object.freeze({
        kind: "loaded_product_execution_resolution",
        schemaVersion: "5.0.0",
        resolution,
        declarationClosure,
        ...
        ownerInstalls: Object.freeze(input.admittedInstalls.filter((install) => selectedInstallIds.has(install.installId))),
...
export function nativeSemanticRetentionOwnersMatch(publication, graph, graphOwner, entry, target, semantics) {
