...
| `genesis.binding` | resolves jobs, workers, deterministic precompute, governed prompt assembly | resolve semantic jobs against published graph functions first, then materialize internal graph structure for executable traversal, use role policy hooks during eligibility and binding, admit prompt text through the `PromptAssembly` / `PromptAssemblyBudget` carrier, record every prompt compaction with inspection references, and attach resolved policy provenance to bound jobs/manifests | High |
