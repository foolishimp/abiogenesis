...
this_spec = Context(
    ...
    locator="workspace://build_tenants/abiogenesis/codex/code/gtl_spec/packages/abiogenesis.py",
    digest="sha256:" + "0" * 64,
)
...
pytest_op = Operator(
    "pytest",
    F_D,
    "exec://python -m pytest build_tenants/abiogenesis/codex/tests/ -q --tb=short",
)
check_impl_op = Operator(
    "check_impl",
    F_D,
    "exec://python -m genesis check-tags --type implements --path build_tenants/abiogenesis/codex/code/genesis/",
)
...
check_bootloader_op = Operator(
    "check_bootloader",
    F_D,
    "exec://python -m genesis check-bootloader-consistency --spec-module gtl.core --bootloader build_tenants/abiogenesis/codex/code/gtl_spec/GTL_BOOTLOADER.md",
)
...
eval_bootdoc_consistency = Evaluator(
    ...
    command="python -m genesis check-bootloader-consistency --spec-module gtl.core --bootloader build_tenants/abiogenesis/codex/code/gtl_spec/GTL_BOOTLOADER.md",
)
...
eval_impl_tags = Evaluator(
    ...
    command="python -m genesis check-tags --type implements --path build_tenants/abiogenesis/codex/code/genesis/",
)
eval_impl_coverage = Evaluator(
    "impl_coverage",
    F_D,
    "Every REQ key in Package.requirements appears in source traceability markers",
    command="python -m genesis check-impl-coverage --package gtl_spec.packages.abiogenesis:package --path build_tenants/abiogenesis/codex/",
)
...
eval_tests_pass = Evaluator(
    ...
    command="python -m pytest build_tenants/abiogenesis/codex/tests/ -q --tb=short -m 'not e2e'",
)
eval_test_tags = Evaluator(
    "validates_tags",
    F_D,
    "All test files carry at least one Validates traceability marker",
    command="python -m genesis check-tags --type validates --path build_tenants/abiogenesis/codex/tests/",
)
eval_validates_coverage = Evaluator(
    "validates_coverage",
    F_D,
    "Every REQ key in Package.requirements appears in test traceability markers",
    command="python -m genesis check-validates-coverage --package gtl_spec.packages.abiogenesis:package --path build_tenants/abiogenesis/codex/tests/",
)
