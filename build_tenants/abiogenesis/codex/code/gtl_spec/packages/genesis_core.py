...
bootloader = Context(
    name="bootloader",
    locator="workspace://build_tenants/abiogenesis/codex/code/gtl_spec/GTL_BOOTLOADER.md",
    digest="sha256:" + "0" * 64,
)

genesis_core_spec = Context(
    name="genesis_core_spec",
    locator="workspace://build_tenants/abiogenesis/codex/code/gtl_spec/packages/genesis_core.py",
    digest="sha256:" + "0" * 64,
)

design_adrs = Context(
    name="design_adrs",
    locator="workspace://build_tenants/abiogenesis/codex/design/adrs/",
    digest="sha256:" + "0" * 64,
)


codex_agent = Operator("codex_agent", F_P, "agent://codex/genesis")
pytest_op = Operator("pytest", F_D, "exec://python -m pytest build_tenants/abiogenesis/codex/tests/ -q")
check_tags_impl = Operator(
    "check_tags_impl",
    F_D,
    "exec://python -m genesis check-tags --type implements --path build_tenants/abiogenesis/codex/code/",
)
check_tags_test = Operator(
    "check_tags_test",
    F_D,
    "exec://python -m genesis check-tags --type validates --path build_tenants/abiogenesis/codex/tests/",
)
...
eval_code_tags = Evaluator("impl_tags", F_D, "All code files carry Implements traceability markers", command="python -m genesis check-tags --type implements --path build_tenants/abiogenesis/codex/code/")
eval_six_modules = Evaluator("six_modules", F_D, "exactly six engine modules exist", command="python -c \"import os,sys; p='build_tenants/abiogenesis/codex/code/genesis'; m={f[:-3] for f in os.listdir(p) if f.endswith('.py') and f!='__init__.py'}; e={'core','bind','schedule','manifest','commands','__main__'}; sys.exit(0 if m==e else 1)\"")
eval_code_fp = Evaluator("code_complete", F_P, "Agent implements the engine without V2 leakage")
eval_tests_pass = Evaluator("tests_pass", F_D, "pytest reports zero failures and zero errors", command="python -m pytest build_tenants/abiogenesis/codex/tests/ -q --tb=short -m 'not e2e'")
eval_test_tags = Evaluator("validates_tags", F_D, "All test files carry Validates traceability markers", command="python -m genesis check-tags --type validates --path build_tenants/abiogenesis/codex/tests/")
