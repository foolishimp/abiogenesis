...
- release_line: `support/4.6.x` (cut from `v4.6.0-rc.3`); release scope: successor cut `v4.6.0-rc.5` (`v4.6.0-rc.4` is immutable and carries the unconnected dispatch lane)
...
- **RC5 regression closure (2026-07-14).** The proof-only `composeAgentTransportArgs` export and its direct unit test are removed. The existing T-109 fake-Claude substrate now crosses the actual `runAgentTransport` process boundary in both postures: closed-prompt argv carries `--safe-mode --tools ""` and tool activity classifies `contract_failure`; worker-executes argv carries neither gate, the parsed tool event is preserved, and classification is `null`. The written transport artifact is checked on the worker path. Scope audit against `PRODUCT.md` and `REQ-R-ABG3-TRANSPORT` found no product or requirement reprice: this remains a subordinate transport-carrier repair under existing execution-default and ABG outcome-classification law. Gates: lint green; semantic 1435/1435.
- Remaining before closure claim: qualify the packed rc.5 candidate through the downstream lane and publish the immutable successor cut; rc.4 remains the immutable cut with the unconnected lane.
