...
|---|---|---|---|---|
| `A5-F01` | Exact product, install, workspace, lock, binding, and ABG-owned catalog. PRODUCT installed-product law; REQ-P-INSTALL; REQ-P-CATALOG; REQ-R-ABG3-BINDING. | T-223 proves a packed predecessor install/workspace/catalog steel thread. It is not full target-contract closure. | Deliver T-282 Phase A's ProductToolchainManifest v2 hard break, exact provisional work basis, Product-fixed One Surface and published-declaration coordinates, exact product resolution/verification/install/bind, dependency lock, catalog admission/view/application, source isolation, and negatives; advance the same path through T-276. Owners: T-282 Phase A plus install/catalog and public-operation boundaries, DS-2/DS-5. | The packed source-blind T-276 path admits ProductToolchainManifest v2 as the only Product manifest, then proves `product.install -> workspace.create -> workspace.bind -> catalog.admit -> catalog.view -> catalog.apply -> run.invoke` with distinct Product, declaration, application, install, workspace, catalog, event, and replay identities; v1, conflicts, mutable-source fallback, implicit rebinding, and rival authority paths fail typed. |
...
| `A5-F06` | Versioned public SDK and thin `abg.cli` graph shell over the 19-operation projection. INTENT public CLI law; PRODUCT Public Operator Contract; REQ-P-POLICY; REQ-P-PUBLIC-CONTRACTS. | T-223 proves a thin SDK/CLI predecessor steel thread. Current public handlers target the superseded roster; only 3 target identities exist by name. | Reconcile every target definition, handler, SDK method, CLI coordinate, result/refusal, and workspace-binding rule; in T-282 Phase A transactionally retire authored `cli-runtime.mjs` and workspace `typescript-runtime.mjs` selection authority and admit only a digest-verified neutral projection. Owners: T-270/T-272/T-274/T-275/T-281 semantic-operation and public SDK/CLI boundaries, then T-276. | Source-blind SDK and CLI expose the same 19 functions and typed outcomes through the packed T-276 path; CLI owns parsing, formatting, and ignition only, and no runtime file, fixture, fallback, or adapter authors topology, selection, callable, policy, ExecutionBasis, event, or closure truth. |
| `A5-F07` | Complete One Surface operator loop: synthesize model, evaluate gaps, select lawful action, invoke or continue, admit evidence, evaluate action, stop/hold/respond/resume. PRODUCT One Surface; REQ-P-POLICY; REQ-R-ABG3-CONTINUATION; ABG5-S03. | T-258 closes public F_H carrier admission; T-267/T-271 preserve the static interpreter and result loci. No target end-to-end public loop is closed. | Realize the four distinct authorities through the Product-owned admitted GTL program, preserve Product-fixed, published-declaration, and AF-10 workspace-application authority as distinct, then reconcile T-270/T-272 without ingress-, SDK-, CLI-, or T-280-owned orchestration. Owner: One Surface design/runtime owners, DS-2; T-276 remains the root governor. | The installed T-276 path first proves converged invocation and then the same admitted program completes ABG5-S03 with replay-derived frontier/actions and typed F_H response/continuation; public ingress and adapters transport projections but never select, order, or implement work. |
...
| `A5-F14` | Packed install, Hello World, and bounded live F_P proof. PRODUCT source-independent distribution; REQ-P-INSTALL; ABG5-S01/ABG5-S02. | T-223 provides packed Hello World, source isolation, package, and catalog predecessor evidence. It predates the accepted hard break. | Complete T-282 Phase A's Product-owned `product.materialize(context_bootstrap)` path, admitted artifact boundary event, legacy bootloader retirement, and installed-authority hard break; then rerun the existing T-276 packed temporary-workspace governor from exact candidate bytes, followed by Hello World, malformed-output and dependency-cycle negatives, and one real Claude `worker_executes` proof that conserves the RC5 lane behavior. Owners: T-282 Phase A plus packaging/install/runtime owners, DS-2/DS-6. | One clean source-blind T-276 root installs the tarball and proves the complete admitted operation/event/replay/result chain; the same candidate completes Hello World and one live F_P path with typed result/replay, Product-owned context, conserved lane-aware transport, and no source/private imports or executable binding bypass. |
| `A5-F15` | One Prime `ExactCandidateQualification<K>` basis/verdict family with law basis, ordered gate-result vector, prospective RC identity, installed-RC authorization, final-tap delta, and bounded release read model. PRODUCT Release Framing; REQ-P-QUAL; REQ-P-SELF-CONFORMANCE. | The accepted constitutional family and lifecycle are specified, and GOALS/PRODUCT select the future exact tapped STDO 2.0 release for 5.0 qualification. No tapped STDO 2.0 release, exact ABIogenesis candidate, or qualification verdict exists. | Complete T-282 Phase A and the T-276 root proof without waiting for STDO 2.0; after the immutable STDO 2.0 tap, T-282 Phase B verifies its release manifest, authoritative source refs/digests, released compression, and declared-source mapping. T-247 then binds those distinct lineages with the exact Product/program/install/workspace/event/replay basis, admits owning-gate vectors and one non-bypassed verdict, and T-248 performs installed-RC authorization and final-delta reruns. Owners: T-282 Phase B, T-247, and T-248, DS-6/DS-7. | Same-subject/same-law-basis results reduce to one green non-bypassed verdict whose `QualificationLawBasis` selects the exact tapped STDO 2.0 authoritative sources while Product/context manifests separately bind the released compression projection; candidate, stale, mixed-version, missing, or digest-mismatched method inputs refuse, qualified bytes and prospective RC identity materialize unchanged, and the typed final-only delta is fully reconciled. |
