# STRATEGY: ABIogenesis 5.0 Product Definition

**Author**: codex
**Date**: 2026-07-19T15:22:42Z
**Updated**: 2026-07-20
**Addresses**: the single ABIogenesis 5.0 product destination
**Status**: Product-reprice basis complete; pending F_H acceptance and
constitutional reification
**Authority**: commentary only; this post never becomes constitutional source

## Purpose

This document defines ABIogenesis 5.0 as one product. It is the sole discussion
surface for deciding what 5.0 is.

It deliberately does not classify the current implementation, prescribe a
migration, amend tickets, or decide which current files survive. Those actions
require a closed destination and therefore follow this definition.

The sequence is:

```text
accept this candidate semantic basis
  -> reify it into one aligned constitutional truth
  -> close the ABIogenesis 5.0 product definition
  -> derive the current-state-to-5.0 correction vector
  -> classify, contract, complete, qualify, and release
```

No companion commentary is required to interpret this definition.
...
product lineage authority and is not defined here. It will be evaluated against
this destination only after this definition closes.
...
HoG advances the admitted GTL traversal. ABG decides what execution facts enter
runtime truth and what the next lawful state is. There is one runtime path, not
an HoG path and a competing ABG path.
...
implementation may propose one. ABG admits or rejects it and remains the sole
owner of cursor movement, graph re-entry, continuation events, and replay
truth. Missing route declaration is negative authority, not permission for an
SDK or product controller to improvise.
...
| `A5-F08` | **Consensus free construction.** The bounded Consensus GraphFunction executes through ordinary public GTL and ABG atoms with no special engine path. |
...
## Reification And Closure Boundary

F_H acceptance of this post may accept only these candidate claims:

1. ABIogenesis 5.0 is the feature-complete, source-independent successor to the
   practical 4.6 product.
2. GTL.TypeScript is the sole program language; the GTL validator validates and
   does not lower; HoG directly traverses admitted GTL; ABG owns admitted
   runtime truth.
3. A GTL program owns topology and starts; GraphFunction is the sole named
   callable work contract; implementations own only their admitted seams.
4. The SDK and CLI are thin catalog and invocation shells, not controllers.
5. The 4.6 traversal conservation contract, 17 feature families, seven
   cumulative scenarios, root outcome, explicit exclusions, and completion
   predicate above are the complete 5.0 destination.
6. Stable 5.0 precedes self-use in 5.0.1, and odd_glc is not a 5.0 dependency.
7. ABIogenesis 5.0 qualification and release require one tapped STDO version,
   targeting version `2.0.0`.

Acceptance does not make this post constitutional source. It does not accept
any current code, artifact, ticket, design, feature count, public operation
roster, test result, migration, or release claim. It authorizes one bounded
`product_reprice` that reifies the accepted basis into the owning constitutional
surfaces.

The exact F_H ruling for that re-entry is:

> Accept this exact post as the candidate semantic basis for one ABIogenesis 5.0
> product_reprice. Authorize reification of its product identity, exact 4.6
> origin baseline, GTL/HoG/ABG architecture, traversal conservation contract,
> 17 feature families, seven cumulative scenarios, root binding, exclusions,
> governance boundary, and completion predicate into the owning constitutional
> surfaces. Do not derive or execute the current-state-to-5.0 correction vector
> until that constitutional product definition closes.

The decision must be recorded as an immutable direct or lawfully proxied
`F_HDecisionReceipt` binding the human authority, acting identity, exact post
digest, subject, basis, scope, decision, change class, and disposition. The
receipt authorizes re-entry; it does not itself prove that reification is
complete.

The ABIogenesis 5.0 product definition closes only when:

1. `INTENT.md`, `PRODUCT.md`, `GOALS.md`, the affected requirements, and the
   bounded scenario catalog form one present-tense, contradiction-free
   constitutional surface derived from this accepted basis;
2. the exact 4.6 origin baseline and all feature, traversal, scenario, root, and
   release obligations remain represented without loss;
3. no live commentary, ticket, design, or generated projection claims a rival
   5.0 destination;
4. deterministic consistency checks pass over the exact constitutional cut;
   and
5. F_H directly or through an admitted proxy records the closure disposition
   against that exact reified cut.

Only after those conditions hold may the closed constitutional destination
govern the `X -> 5.0` conservation, deletion, and completion vector.
