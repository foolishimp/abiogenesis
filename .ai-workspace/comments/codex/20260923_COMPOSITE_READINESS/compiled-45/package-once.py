...
B=Path('/Users/jim/src/apps/abiogenesis');T=B/'build_tenants/abiogenesis/typescript';D=B/'.ai-workspace/comments/codex/20260923_COMPOSITE_READINESS/compiled-45';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest();save=lambda name,data:(D/name).write_text(json.dumps(data,indent=2)+'\n')
...
bootstrap=Path(tempfile.mkdtemp(prefix='abi5-composite45-install-'));(bootstrap/'package.json').write_text(json.dumps({'name':'abi5-composite45-bootstrap','private':True,'version':'1.0.0'})+'\n')
...
prior={r['path']:r for r in json.loads((D.parent/'compiled-44/archive-members.json').read_text())}
...
for r in json.loads((D/'accepted-source.json').read_text())['members']: assert sha(Path(r['path']))==r['sha256'],r['path']
save('conservation.json',{'predecessor':'compiled-44','unchanged':len(records)-len(delta),'changed':len(delta),'changedPaths':[r['path'] for r in delta],'outsideAcceptedScope':[]})
