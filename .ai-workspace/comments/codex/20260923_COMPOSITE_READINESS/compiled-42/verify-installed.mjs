...
save('package-readiness.json',{status:'CLOSED_PACKAGE_NATIVE_QUALIFICATION_DISTINCT',identity,selected,sourceSubject:read(join(D,'accepted-source.json')).sourceSubject,sourceReturn:read(join(D,'accepted-source.json')).sourceReturn,buildInputsPreserved:inputs.length,generatedChanged:generated.length,memberCorrespondence:identity.archiveMembers,testsReused:'D2 ancestry worker emission/manifest and focused relation tests reused; no duplicate compile or test run',nativeExecution:false});
