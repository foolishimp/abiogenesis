# STRATEGY: T-284 X-To-5 Correction Vector

**Author**: codex
**Observed at**: 2026-07-20T02:33:14Z
**Ticket**: T-284
**Status**: replacement review candidate; exact refreeze pending; classification and requirement-reentry evidence only
...
No accepted 5.0 feature or traversal row is classified `satisfied`. X has no
source-blind installed proof of the accepted direct-GTL path. `partial` means
useful target behavior exists but the complete authority path or installed
proof does not. `contradictory` means X contains useful behavior behind a
carrier that actively conflicts with the accepted Product.

The largest retained investment is the typed language and runtime-truth
substrate. The largest retirement is the executable compiler/plan/controller
...
|---|---|
| 4.6 semantic origin | `4.6.0-rc.5`, tag `v4.6.0-rc.5`, published tag commit `8d43dc8968e3df16029e6201680a0301eda035f1` |
| 4.6 clean source | `bab609ab353304324b939a4528371603eef0a05d` |
| 4.6 package | `@abiogenesis/typescript-tenant@4.6.0-rc.5`, SHA-256 `d9c99382f2c5b787ebe48ce72c320616baeac9187863078332df18c0036853ea` |
| accepted 5.0 constitution | commit `afb35def08b2259046830f87c18b45c95c84001c` |
| accepted 5.0 aggregate | `c85ca7ae34352b91d579fcfae035ca3aa3d9a27428b584ac81c425b0d837d260` |
| X source HEAD | `1b8b2b0a22ad5dc484e3db5c19fd562cd7935ff8` |
| X source tree | `e3700c2a8c333eef520e07440a766a1150107713` |
| X snapshot | commit `676766a648066eaa69dce05f636d5ec98fb40dec`, tree `77a81cb16d196a016edca0af8d7ac7fd39d2e016` |
...
published RC5 tag. RC5 is therefore the semantic origin, while X is a separately
frozen current-state observation.
...
| Measure | RC5 clean source | X snapshot | Delta |
|---|---:|---:|---:|
| TypeScript source files | 269 | 389 | +120 |
| TypeScript source lines | about 133,885 | about 227,464 | about +93,579 |
| complete tenant change | - | 757 files | 211,394 insertions / 7,759 deletions |

X's invalid authority is not isolated. It crosses M03 contracts and runners,
M04 public SDK and installer code, CLI runtime loading, generated manifests,
tests, probes, and accepted historical designs. A pruning strategy would need
to prove a negative over all those surfaces while preserving useful behavior
inside mixed files. A clean successor line makes the default state absence and
requires every imported carrier to prove its purpose and authority.
...
4. Tests migrate only with the behavior they prove; compiler-plan tests do not
   become target proof.
...
9. Final-integration planning commit `d4db5a93` is constitutional and ticket
   history only. Accepted T-283 Product/Goals and the current held T-282
   disposition supersede its authority; none of its bytes enters the successor.

## 3. Classification Law

Every row uses three independent dimensions.

| Dimension | Values |
|---|---|
| 4.6 semantic disposition | `conserved`, `superseded`, `intentionally_removed`, `not_applicable`, `unresolved` |
| accepted-5.0 coverage in X | `satisfied`, `partial`, `missing`, `contradictory` |
| X carrier action | `retain`, `refactor`, `replace`, `create`, `delete`, `archive` |

Actions describe how an X semantic carrier relates to the clean successor:

- `retain`: migrate the carrier with behavioral equivalence evidence;
- `refactor`: migrate only its admitted semantic interior into a new owner;
- `replace`: build the accepted target on RC5 rather than migrating the X
  authority shape;
- `create`: realize missing accepted behavior on the clean line;
- `delete`: do not migrate the carrier and prove its identity is absent from
  the installed successor; and
- `archive`: retain it only in the immutable X evidence line.

These finer T-284 actions project into the method's required inherited-
implementation classes as follows:

| T-284 action | Fundamental-migration class |
|---|---|
| `retain` | `carry_across`, subject to explicit re-adoption and proof |
| `refactor`, `replace` | `rewrite` |
| `delete`, `archive` | `redundant` as live implementation; archive remains evidence only |
| `create` | no inherited implementation exists to classify |

Mixed files must extract retained validation, diagnostic, event, or
implementation behavior into destination-owned carriers. They are never copied
and pruned in place as the default migration method.

### 3.1 Evidence, Authority, Confidence, And Open Decisions

The following shared bindings apply to every classification row in sections 4
through 10. They are stated once rather than repeated as identical table
columns:

| Row family | Source evidence | Accepted target authority |
|---|---|---|
| RC5 baseline and exclusions | Exact RC5 release note, manifest, source commit, and package in section 2 | Accepted `PRODUCT.md` lines 81-136 and the Product identities and boundaries that follow |
| Traversal rows and fibre substitution | Accepted 4.6 conservation inventory plus exact X snapshot carrier inspection | Accepted `PRODUCT.md` lines 218-432 and traced GTL/ABG requirements |
...
- `RN` = `abiogenesis-4.6-support/.../4.6.0-rc.5/release-note.md`
- `RM` = the adjacent `release-snapshot-manifest.json`
- `P` = accepted `specification/PRODUCT.md`

| ID | Origin obligation or exclusion | Source | 4.6 disposition | X coverage | X action | Rationale |
|---|---|---|---|---|---|---|
| `B01` | GTL declares graph structure and lawful work | `P:102-108` | conserved | partial | retain | M01/M02 carriers, constructors, admission, and serialization remain the correct language substrate. |
| `B02` | One traversal law composes deterministic, probabilistic, and human work | `P:102-108` | conserved | partial | replace | X has all three interiors but no single direct-GTL traversal path. |
| `B03` | ABG, not caller or worker, owns admitted runtime truth | `P:102-108` | conserved | partial | refactor | Event admission and replay are strong; SDK, engine, and feature runners still own competing decisions. |
...
| `B18` | Specification reconciliation lineage | `RN:49-55` | conserved | partial | archive | Preserve as release lineage and evidence, not runtime behavior. |
| `B19` | Typed GTL and seven-C authoring boundary | `RN:49-55`, `RM:24` | conserved | partial | retain | X materially extends this correct foundation. |
| `B20` | Semantic validation diagnostics and repair affordances | `RN:49-55`, `RM:24` | conserved | contradictory | refactor | Keep non-lowering diagnostics; remove compiler, plan, and execution-authority semantics from the validator. |
...
| `X02` | odd_glc adoption and data-mapper work are outside 4.6 | `RN:84-85` | not_applicable | missing | create | 5.0 requires only the bounded F17 fixture; odd_glc release remains independent. |
| `X03` | `workflow.C`, `C.batch`, `C.retry` runtime was not a 4.6 claim | `RN:86-87` | not_applicable | contradictory | replace | 5.0 newly requires direct generic execution; X realizes them through compiled/special runners. |
| `X04` | Observer/tuner campaign, executable Consensus, sticky sessions, and zero-out-of-framework proof were not 4.6 claims | `RN:88-90` | not_applicable | partial | refactor | Only the accepted 5.0 F08/F12/S04/S05 obligations now govern them. |
| `X05` | Monolith splitting and barrel pruning were not release claims | `RN:91` | not_applicable | partial | refactor | Compression is design hygiene, never Product proof. |
| `X06` | ABG 5 catalog, graph shell, marketplace, public consumption, self-hosting were outside RC5 | `RN:92-93` | not_applicable | partial | refactor | Catalog/shell/public consumption are now 5.0 obligations; marketplace and self-hosting remain excluded. |
| `X07` | Typed environment-denial and sandbox escalation were follow-up work | `RN:94-97` | not_applicable | missing | create | Add only where required by current typed failure and trusted-desktop contracts. |
| `X08` | Trusted desktop or declared external sandbox is the operating boundary | `RN:99-107` | conserved | partial | retain | Do not expand 5.0 into hostile workstation or multi-tenant hardening. |
...
| ID | Exact RC5 path family | Method class | Successor treatment | Admission order | Owning proof |
|---|---|---|---|---|---|
| `RCI-01` | `code/src/shared/abg_library/{agent_transport.ts,transport_contracts.ts}` and `test_env/tests/{test_m03_transport_protocol_unit.test.mjs,test_t109_agent_callout_traced_substrate.test.mjs}` | `carry_across` | Re-adopt only after M3 binds transport as an `F_P` implementation seam; no graph or scheduler authority crosses | D4, after the all-`F_D` root and before the first `F_P` slice | B07-B14 and B24 real-worker mutation proof before first `F_P` slice |
| `RCI-02` | `code/src/gtl/m01/**`, `code/src/gtl/m02/**`, and GTL requirement exports | `rewrite` | Re-derive typed authoring, Module publication, serialization, and non-lowering validation against 5.0 GTL law | D2, after D1 Product identity and publication carriers | `ABG5-S01`, `ABG5-S02`, and traversal matrix |
| `RCI-03` | `code/src/abg/m03/**` | `rewrite` | Re-adopt event, replay, admission, result, continuation, and implementation interiors only behind direct HoG traversal; compiled handoff and engine authority do not cross | D3 for root atoms, then D4 for remaining regimes, one accepted owner at a time | `ABG5-S01` through `ABG5-S03`, real-path authority mutations |
| `RCI-04` | `code/src/app/m04/**`, `code/src/cli/**`, and `code/src/bin/**` | `rewrite` | Re-derive Product/install/workspace/catalog and thin SDK/CLI projections; no controller or private runtime binding crosses | D1 for identity/install/catalog; D5 for public shells after D3 is green | `ABI5-ROOT-001`, `ABG5-S03`, public-path absence proof |
| `RCI-05` | `code/src/qualification/m05/**` | `rewrite` | Re-derive exact-subject qualification and release carriers against the 5.0 subject chain and tapped STDO 2.0 | D6, after D5 behavior and the STDO 2.0 tap | `ABG5-S04`, `ABG5-S07` |
| `RCI-06` | `code/src/shared/**` not matched by `RCI-01` | `rewrite` | Re-adopt only small validation, process, identity, configuration, or evidence primitives with an accepted 5.0 owner | Immediately before the consuming admitted carrier; never as a shared-library wave | proof of the consuming feature plus no rival shared authority |
| `RCI-07` | remaining `code/**` | `rewrite` | Recreate only required package exports, implementation bindings, and shims from accepted design | With the first consuming D1-D5 carrier, not before it | installed typecheck, package export, and `ABI5-ROOT-001` proof |
| `RCI-08` | `test_env/tests/**`, `test_env/live/**`, `test_env/sandbox/**`, `test_env/type_tests/**`, `test_env/gates/**`, and test support/corpus not matched by `RCI-01` | `rewrite` | Port tests only with the re-adopted behavior; old engine/compiler expectations remain predecessor evidence | After or transactionally with the exact behavior under proof | owning Product scenario and mutation-negative proof |
| `RCI-09` | `test_env/downstream/**` | `redundant` | Archive the frozen odd_sdlc fixture; create the smaller independent flavored-catalog fixture required by F17 | D5 after the public path is stable and before STDO-dependent qualification; no predecessor fixture admission | `ABG5-S06` |
| `RCI-10` | `design/**` and design ADRs | `redundant` | Preserve as historical evidence only; M3 derives a new direct-GTL design | No successor admission; review as M3 negative precedent | M3 independent design review |
| `RCI-11` | `package.json`, lockfile, TypeScript/ESLint config, current README files, `.gitignore`, and `config/**` | `rewrite` | Build a minimal 5.0 package/install surface from Product and accepted design; no predecessor script is ambient authority | D1, before any successor carrier can be promoted | source-independent pack/install and `ABI5-ROOT-001` |
| `RCI-12` | legacy packed `.tgz` files and every remaining RC5 tenant path | `redundant` | Preserve only in the immutable RC5 source/package snapshot; do not copy into the successor | No successor admission | installed package inventory absence proof |
...
|---|---|---|---|---|
| `F_D` | conserved | partial | refactor | Total handlers and deterministic evidence exist; route them through one direct GTL leaf seam instead of engine/feature runners. |
| `F_P` | conserved | partial | refactor | Transport, result admission, evidence, and retries exist; bind them to the declared GTL stage and ABG C-call. |
| `F_H` | conserved | partial | refactor | `fh_interaction.ts` preserves hold/response/resume identity; join it to one generic traversal cursor. |
| mixed | conserved | partial | replace | X composes fibres through compiled plans and controller branches; M3 must define one fibre-polymorphic traversal unit. |

### 5.2 Structural Form

| Row | 4.6 disposition | X coverage | X action | Current X evidence and correction |
|---|---|---|---|---|
| atomic call | conserved | partial | refactor | Leaf interiors and C-call receipts exist; consume the admitted `C.of` node directly. |
| flat composition | conserved | contradictory | replace | `C.compose` authoring is correct, but `CompiledCProgramPlan` becomes executable authority. Traverse the authored term directly. |
| edge program | conserved | contradictory | replace | X lowers `C.edge` to HoG declarations and a baked triple. Preserve open GTL edge structure and remove the triple default. |
| adaptive declared selection | conserved | contradictory | delete | `CompiledHogProgramPlan` catalog/default/ladder selection is rival program authority. GTL gate/policy plus ABG-admitted selected identity must own selection. |
| batch | conserved | contradictory | refactor | Preserve task identity, cardinality, evidence, and judgments from `c_batch_runtime.ts`; remove its compiled-plan entry and special runner ownership. |
| transparent child traversal | conserved | contradictory | refactor | Preserve child graph-call/frame/sub-traversal evidence; invoke the named GraphFunction through the same HoG traversal recursively. |
| graph recursion | conserved | contradictory | refactor | Preserve termination, foldback, parent re-evaluation, and lineage; remove `typed_recurse_runtime.ts` as an alternate executor. |
| retry | conserved | contradictory | refactor | Preserve fresh attempt identity and bounded policy; remove compiler lowering and special retry control authority. |

### 5.3 Consequence Route

| Row | 4.6 disposition | X coverage | X action | Current X evidence and correction |
|---|---|---|---|---|
| `same_edge_retry` | conserved | contradictory | refactor | Existing retry/repair carriers are useful; one ABG transition must outrank fallback retry. |
| `depth_traversal` | conserved | partial | refactor | Zoom, recurse, and child lineage exist; unify them under direct GraphFunction traversal. |
| `graph_span_reentry` | conserved | partial | refactor | Re-entry contracts and events exist; apply them to the admitted GTL cursor rather than a plan coordinate. |
| `public_start_reentry` | conserved | contradictory | replace | Legacy start paths load private runtime bindings and call `runEngineStart`; public re-entry must consume installed catalog/program truth. |
| `ticket_traversal` | conserved | partial | refactor | Requirements/ticket routes exist; Product GraphFunctions own meaning and ABG owns only runtime facts. |
| `fh_input_required` | conserved | partial | refactor | Hold/response/resume is well typed; complete same-locus direct-GTL continuation. |
| `escalation_or_reprice` | conserved | partial | refactor | Preserve attributed proposal and F_H admission; remove worker/SDK application authority. |
| `gap_stop` | conserved | partial | retain | Gap and non-progress projections are reusable; prove they stop the real installed path. |
| `non_admit` | conserved | partial | retain | X has extensive typed admission failures; retain them and add real-path mutation negatives. |

### 5.4 Runtime Disposition
...
| `re_enter` | conserved | partial | refactor | Preserve basis and target identity; replace plan coordinates with GTL coordinates. |
| `yield_continuation` | conserved | partial | retain | Continuation carriers are useful; prove public resume on the same locus. |
| `inspect_runtime_archive` | conserved | partial | retain | Replay/archive projection remains read-only ABG truth. |
| `reprice` | conserved | partial | retain | Preserve proposal and authority boundary; no automatic application. |
| `human_assurance_required` | conserved | partial | retain | Preserve typed F_H hold and attributed evidence. |
| `escalate` | conserved | partial | retain | Preserve typed escalation without worker authority. |
| `gap_stop` | conserved | partial | retain | Preserve nonterminal truth and refusal to project completion. |
| `block` | conserved | partial | retain | Preserve real event-derived block; fixtures may not author it. |
| `non_admit` | conserved | partial | retain | Preserve pre-effect refusal and canonical failure evidence. |

### 5.5 Public Start And Control

| Row | 4.6 disposition | X coverage | X action | Current X evidence and correction |
|---|---|---|---|---|
| advance next lawful work | conserved | contradictory | replace | Legacy CLI `resolveNextTarget` and One Surface selection improvise control; GTL program/policy and ABG replay must determine the next step. |
| target GraphFunction | conserved | partial | refactor | Catalog resolution exists; require exact selected program membership and one public invocation. |
| target asset through owner | conserved | partial | refactor | Asset ownership projection exists; preserve non-callability and remove adapter fallback. |
| bounded `until` | conserved | partial | refactor | Typed `until` exists; HoG/ABG must enforce it without a public control loop. |
| direct or proxied F_H control | conserved | partial | retain | Preserve admitted identity-bearing response and proxy authority. |
| direct or supervised root control | conserved | contradictory | delete | `publicControlLoop` and max-autonomy wrappers are rival controllers; retain only policy and invocation inputs. |

### 5.6 Shape-Preserving Fibre Substitution

| 4.6 disposition | X coverage | X action | Required correction |
|---|---|---|---|
| conserved | missing | create | Prove on one admitted graph that substituting an equivalent `F_P` leaf with `F_D` changes only implementation/evidence class, while graph topology, C-call locus, event-spine shape, and continuation law remain identical. Compiler-output equivalence is not this proof. |
...
| `A5-F17` downstream portability | B23 source-blind consumption conserved; the independent flavored-catalog fixture is a 5.0 addition | missing | create | Historical/downstream fixtures are not the accepted proof; create and prove the replacement in D5 before the STDO 2.0 tap. |
...
| `ABG5-S06` | B06/B23 conserved; bounded host parity and independent flavored catalog are new | partial | refactor | Native surfaces exist; complete no-copy parity and the independent fixture in D5 before STDO-bound qualification begins. |
...
superseded authority. M3 must design through R10, but M4 implementation advances
the root in rank order and reruns the governor at every promoted checkpoint.
...
`F_P`, and `F_H`; and `D5` Product constructions, public projections, and
pre-qualification downstream portability. `D6` owns STDO-bound conformance,
...
| Stage | Exact admission boundary |
|---|---|
| `D1` | Minimal package, Product/install/workspace identity, Module publication, catalog admission, and configuration needed to address one installed target. |
| `D2` | GTL.TypeScript declarations, serialization, raw admission, and non-lowering static validation for that target. |
| `D3` | One all-`F_D` direct HoG traversal with ABG event, result, replay, and typed public outcome satisfying `ABI5-ROOT-001`. |
| `D4` | Remaining C relations and declared `F_P`/`F_H` seams; B-001 is proved before the first `F_P` transport admission. |
| `D5` | Standard Product constructions, Consensus, One Surface, observer/tuner, thin public SDK/CLI projections, and the independent flavored-catalog portability proof over the same D3/D4 path. This stage closes `ABG5-S06` before the STDO 2.0 tap. |
| `D6` | Exact-candidate self-conformance, qualification, release, and post-publication proof under tapped STDO 2.0. This stage owns `ABG5-S04` and `ABG5-S07`, not pre-tap portability. |

| ID | Carrier family | 4.6 disposition | Coverage / action | Successor owner; admission stage; owning proof |
|---|---|---|---|---|
...
| `XC08` | C-to-HoG and execution-declaration lowering: `c_algebra_hog_compiler.ts`, `execution_declaration_compiler.ts` | superseded | compiled plans and executable handoff; `delete` | none; D3 retirement; direct-path plus lowering absence proof |
...
| `XC40` | saga-frontier contract and runner: `saga_frontier.ts`, `saga_frontier_runner.ts` | conserved | useful replay/frontier and idempotency semantics mixed with scheduler/admission authority; `refactor` | GTL topology plus declared transition relation, HoG traversal, selected implementation leaf effects, and ABG admission/replay; D4; serial/parallel equivalence and no-scheduler mutations |
| `XC41` | ABG core carriers, schemas, admission, execution-family, and result-assessment contracts not matched above | conserved | useful typed runtime boundaries mixed with X-era owner assumptions; `refactor` | ABG runtime-truth substrate and selected implementation interfaces; D3-D4; R8-R10 causal/result proof plus no carrier-authored topology |
| `XC42` | direct-relation, consequence, handler, retry-frontier, temporal, and traversal-modulation contracts not matched above | conserved | reusable relation semantics mixed with compiler and scheduler assumptions; `refactor` | GTL declares topology, HoG traverses, implementations propose effects, ABG admits transitions; D2-D4; 40-row matrix plus scheduler/topology mutations |
| `XC43` | assurance, evaluation, observation, proof-strength, liveness, and structure-probe contracts | conserved | useful evidence and judgment relations with stale subject/authority coupling; `refactor` | Product evaluators and ABG replay/evidence projections; D3-D5; replay-grounded positive proof and forged-evidence negatives |
| `XC44` | requirement algebra, witness, instruction, diagnosis, reprice, and workspace-hygiene contracts | conserved | useful governed-context and gap semantics mixed with predecessor workflow identity; `refactor` | Product GraphFunctions over admitted workspace and ABG evidence; D4-D5; requirement-route, re-entry, and no-silent-reprice proofs |
| `XC45` | public-operation contract families, ingress/admission, artifact projection, construction runner, and runtime authoring/projection routes | conserved | public contract and replay interiors mixed with private ingress and special runners; `refactor` | one Product-owned public contract authority, thin public projection, and ordinary HoG traversal; D3-D5; public-path parity plus private-ingress/controller absence |
| `XC46` | shared ABG delivery, identity, policy, conformance, and validation primitives | conserved | reusable total helpers and negative authority guards without standalone Product authority; `refactor` | consuming admitted D1-D6 carrier only; proof of the consuming feature plus no shared controller, scheduler, or truth register |
| `XC47` | TypeScript barrels, requirement exports, root indexes, and node shims | not_applicable | X export topology is not successor authority; `replace` | minimal exports derived with each consuming D1-D5 carrier; installed typecheck and package-export proof |
| `XC48` | package/configuration files and current README surfaces not matched above | not_applicable | useful build inputs mixed with X scripts and descriptions; `replace` | minimal D1 package/install/configuration surface; source-independent pack/install and root proof |
| `XC49` | legacy packed tarballs and tracked `node_modules` marker | not_applicable | historical or generated residue; `archive` | no successor admission; immutable X archive and successor package-absence proof |

The ordered first-match predicates, complete 1,935-path membership count,
per-family counts, and membership digest are fixed in
...
| ID | Exact carrier | Method class | Carrier action | Destination | Admission order | Owning proof |
|---|---|---|---|---|---|---|
| `Y01` | planning commit `d4db5a93` (`PRODUCT.md`, `GOALS.md`, and first T-282 shape) | `redundant` | `archive` | Accepted T-283 Product/Goals and current held T-282 disposition supersede its authority; no implementation destination | No successor admission; retained only as decision lineage before M3 | Diff census proves all three changed paths are constitutional/ticket planning; successor package absence and authority trace prove no carrier crossed |
| `Y02` | committed transport repairs `e736fa49` and `53b3a72c` | `rewrite` | `refactor` | Selected `F_P` implementation binding and ABG transport/result admission | D4, after RCI-01 and before first `F_P` promotion | B07-B14/B24 plus F_P result-admission proof; do not cherry-pick the commits |
| `Y03` | dirty snapshot `3c2d86d4` with 346 status entries | `redundant` | `archive` | None unless a later reviewed amendment names one exact carrier | No admission under T-284 | Archive identity plus successor package absence; later extraction requires Product obligation, destination, loss, order, and proof |
| `Y04` | rejected T-270 stash `d4d1a3cc` | `redundant` | `archive` | None; negative prototype evidence only | No successor admission | Archive identity and successor absence proof |

## 9. Explicit Retirement Set

The following identities have no lawful executable role in 5.0 and must be
absent from the installed public path after their reusable checks are extracted:

1. `HogProgramDeclaration` as a second authored or executable program.
2. `CompiledHogProgramPlan`, including `default`, `catalog`,
   `catalog_unselected`, and `ladder` runtime selection.
3. `CompiledExecutionDeclarations` as an execution authority.
4. C-to-HoG lowering and `CAlgebraHogCompilation` as a runtime handoff.
5. `CompiledGraphVectorCProgramBinding` and
   `CompiledGraphVectorExecutionHandoff` as executable carriers.
6. `CompiledCProgramPlan` and its publication through package exports.
7. One Surface compilation as a feature-specific executable program.
8. Installer-authored GraphFunction, Module, policy, and ExecutionBasis in
   `.abiogenesis/cli-runtime.mjs`.
9. Arbitrary `.abiogenesis/typescript-runtime.mjs` executable override.
10. SDK-owned GraphFunction selection, One Surface ordering, post-action
    evaluation, continuation routing, or closure.
11. `publicControlLoop` and max-autonomy wrappers as traversal controllers.
12. HoG-local default program, baked transform/evaluate/consequence triple,
    program catalog, attempt ladder, or selector.

Historical source, tests, designs, and review records are archived rather than
erased. Files containing mixed retained and retired behavior are replaced only
after the retained contracts and diagnostics have an accepted destination.

## 10. Held Ticket Disposition

| Ticket | Disposition | Reason and successor use |
|---|---|---|
| `T-268` | archive shape; carry claim | Retain canonical capability/conformance publication as a successor claim after M3/M4, not as the X ticket sequence or fixed-count proof. |
| `T-270` | archive shape; replace claim | Retain exact public invocation/program/view/function binding checks. M3 owns a smaller direct invocation join with no compiler or One Surface selector. |
| `T-272` | archive shape; carry claim | Selectively port exact F_H held-locus response and resume semantics onto the direct GTL cursor. |
| `T-274` | archive shape; carry assets | Selectively port Consensus schemas, Module, GraphFunction, and publication assets after their source authority is reverified. |
| `T-275` | archive shape; carry claim | Selectively port attributed profiles and ticket-result projection as ordinary Product-owned leaves and read projections. |
| `T-276` | archive shape; replace proof | Reuse only the parameterized source-blind driver idea for `ABG5-S05`; `ABI5-ROOT-001` owns M4. |
| `T-278` | archive | Its ontology informed T-283, but the accepted Product now owns the sole target. The 27/7/19 counts are historical derived projections, not Product scope. |
| `T-281` | archive shape; carry assets | Selectively port the typed public definition/schema family. Re-derive SDK/CLI projections; no operation count becomes engine ontology. |
| `T-282` | archive shape; carry constraints | Preserve the private-runtime-bypass diagnosis, one manifest truth, context materialization, and STDO M6/M7 boundary in compressed successor work. |
| stale X copy of `T-283` | archive | The canonical T-283 is completed in the accepted product-reprice line. X's active copy has no remaining authority. |

No held X ticket resumes directly and no active ticket directory is copied onto
...
| `REQ-R-ABG3-SAGA-FRONTIER` | GTL declares topology and routes, the validator checks static relations, an implementation proposes a declared transition, ABG admits or rejects it, HoG applies it, and selected implementations realize leaf effects without a scheduler. |
| `REQ-R-ABG3-WORKER` | Product/implementation binding proposes a worker, ABG admits it, HoG traverses admitted work, and the selected implementation realizes leaf effects. |
| `REQ-L-GTL3-SUBWORK` | GTL owns sub-work topology and scope, the validator checks it, HoG traverses it, a selected implementation realizes leaf effects, and ABG admits runtime truth. |
| `REQ-R-ABG3-BINDING` | Binding truth surrounds validator, HoG traversal, and selected-implementation realization without assigning execution or static validation to ABG or HoG. |
| `REQ-R-ABG3-LEAFTASK` | Bounded leaf work uses the same GTL/validator/HoG/implementation/ABG split and cannot become a rival executor. |
...
| `REQ-R-ABG3-PROJECTION-011` | `evaluateNext` emits a candidate selection projection; only ABG admission creates current action or continuation truth. |
| `REQ-R-ABG3-FP-CONSCIOUSNESS` | `evalGap` and `evaluateNext` emit candidate evaluator results; ABG separately admits observation, evaluator, intent, and continuation truth. |
...
2. complete X and final-integration carrier classification, including exact
   1,935-path predicate membership, saga frontier, observer/tuner, plugins,
   repair, F_P result admission, and planning commit `d4db5a93`;
...
5. the GTL/validator/HoG/implementation/ABG/catalog authority split and
   repaired requirement consistency;
