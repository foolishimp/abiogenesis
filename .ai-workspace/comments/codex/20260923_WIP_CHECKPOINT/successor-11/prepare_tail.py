...
def sha(p):
 ...
 return h.hexdigest()
selection=json.loads((root/'tail-selection.json').read_text());activation=json.loads((root/'activation.json').read_text());assert git('rev-parse','HEAD').decode().strip()==activation['basisCommit'];assert not paths('diff','--cached','--name-only')
tracked=paths('ls-files','--cached');prior={};receipts=[]
transport_roots=[repo/r['root']for r in json.loads((previous/'transport-reuse.json').read_text())['prior']]+[previous/'transport-01',root/'transport-01']
...
with gzip.open(root/'tail-incremental-inventory.jsonl.gz','xt')as f:
 for row in rows:f.write(json.dumps(row)+'\n')
write('tail-classification.json',{'baseCommit':activation['basisCommit'],'directPaths':sorted(selection['directExpected']),'priorTransportReceipts':receipts,'inheritedIndexedMembers':len(prior),'reusedSelectedMembers':list(reused.values()),'newArchiveMembers':len(rows),'newArchiveBytes':sum(r['byteLength']for r in rows),'selectedFreezes':freezes,'canonicalFreezeReferencesConservedThroughPostimages':sorted(set(canonical)),'pending':selection['pending'],'scope':selection['scope'],'priorPayloadReadOrReupload':False})
write('tail-freeze-expectations.json',{p:r['expectedFreeze']for p,r in selected.items()})
write('tail-exclusions.json',{'prefixesAndFiles':{},'active':selection['pending'],'note':'No new blanket ignore of active or reproducible trees; only exact newly transported originals will be ignored.'})
write('tail-transport-reuse.json',{'prior':receipts,'script':'.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-01/checkpoint_transport.py','scriptSha256':sha(root.parent/'successor-01/checkpoint_transport.py'),'unchangedPriorByteProof':'Only prior indexes/catalogs/verification receipts acquired; accepted payload proofs reused. No prior transport payload scanned, repacked or uploaded.'})
(root/'tail-selected-archive-paths.nul').write_bytes(b''.join(os.fsencode(r['path'])+b'\0'for r in rows))
