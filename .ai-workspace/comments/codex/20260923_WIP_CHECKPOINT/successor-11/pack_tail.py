...
repo=Path(sys.argv[1]).resolve();root=repo/'.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-11';previous=root.parent/'successor-01';out=root/'transport-02'
...
selected=[os.fsdecode(x)for x in(root/'tail-selected-archive-paths.nul').read_bytes().split(b'\0')if x]
out.mkdir(exist_ok=False);m.CHECKPOINT=str(out.relative_to(repo));m.prepare(repo,out,[],selected=selected)
restoration=Path('/private/tmp')/('20260924-checkpoint-successor11-tail-'+repo.name+'-'+str(os.getpid()));restoration.mkdir(exist_ok=False)
...
expected=json.loads((root/'tail-freeze-expectations.json').read_text());checked=[]
for p,e in expected.items():
 ...
 checked.append(p)
m.write_json(root/'tail-frozen-member-correspondence.json',{'status':'all selected original freeze identities exact','checked':len(checked),'paths':checked,'noRuntimeResourceMutation':True,'restoration':str(restoration)})
