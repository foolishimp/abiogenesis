...
def sha(p):
 ...
 return h.hexdigest()
selection=json.loads((root/'selection-02.json').read_text());activation=json.loads((root/'activation.json').read_text());assert git('rev-parse','HEAD').decode().strip()==activation['basisCommit'];assert not paths('diff','--cached','--name-only')
...
with gzip.open(root/'incremental-inventory-02.jsonl.gz','xt')as f:
 for row in rows:f.write(json.dumps(row)+'\n')
write('classification-02.json',{'baseCommit':activation['basisCommit'],'directPaths':sorted(selection['directExpected']),'priorTransportReceipts':receipts,'inheritedIndexedMembers':len(prior),'reusedSelectedMembers':list(reused.values()),'newArchiveMembers':len(rows),'newArchiveBytes':sum(r['byteLength']for r in rows),'selectedFreezes':freezes,'canonicalFreezeReferencesConservedThroughPostimages':sorted(set(canonical)),'pending':selection['pending'],'scope':selection['scope'],'priorPayloadReadOrReupload':False})
write('freeze-expectations-02.json',{p:r['expectedFreeze']for p,r in selected.items()})
write('exclusions-02.json',{'prefixesAndFiles':{},'active':selection['pending'],'note':'No new blanket ignore of active or reproducible trees; only exact newly transported originals will be ignored.'})
write('transport-reuse-02.json',{'prior':receipts,'script':'.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-01/checkpoint_transport.py','scriptSha256':sha(root.parent/'successor-01/checkpoint_transport.py'),'unchangedPriorByteProof':'Only prior indexes/catalogs/verification receipts acquired; accepted payload proofs reused. No prior transport payload scanned, repacked or uploaded.'})
(root/'selected-archive-paths-02.nul').write_bytes(b''.join(os.fsencode(r['path'])+b'\0'for r in rows))
