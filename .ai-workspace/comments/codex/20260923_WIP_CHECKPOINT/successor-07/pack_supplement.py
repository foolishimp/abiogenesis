...
repo=Path(sys.argv[1]).resolve();root=repo/'.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-07';previous=root.parent/'successor-01';out=root/'transport-02'
...
selected=[os.fsdecode(x)for x in(root/'selected-archive-paths-02.nul').read_bytes().split(b'\0')if x]
out.mkdir(exist_ok=False);m.CHECKPOINT=str(out.relative_to(repo));m.prepare(repo,out,[],selected=selected)
restoration=Path('/private/tmp')/('20260923-checkpoint-successor07-supplement-'+repo.name+'-'+str(os.getpid()));restoration.mkdir(exist_ok=False)
...
expected=json.loads((root/'freeze-expectations-02.json').read_text());checked=[]
for p,e in expected.items():
 ...
 checked.append(p)
m.write_json(root/'frozen-member-correspondence-02.json',{'status':'all selected original freeze identities exact','checked':len(checked),'paths':checked,'noRuntimeResourceMutation':True,'restoration':str(restoration)})
