...
The supported revision child has exact catalog membership in a separate
no-start, supervised Program in the same publication. Its existing graph-call
closure and parent-source authentication remain authoritative; it exposes no
root entry. The ordinary C2 root Program keeps its own unchanged command
environment and callable membership. Static catalog membership and permission
to invoke a root are distinct obligations; neither may be deleted to satisfy
the other.
