...
export function evaluateSelfConformance(input: SelfConformanceInput, owner: SelfConformanceOwner): Readonly<SelfConformanceResult> {
  ...
  const ruleGroups = scope?.ruleGroups ?? catalog.rules.map(r => ({ groupRef: r.ruleRef, ruleRefs: [r.ruleRef], sourceRefs: [r.sourceRef] }));
  const surfaceGroups = scope?.surfaceGroups ?? (input.inventory?.members ?? []).map(m => ({ groupRef: m.ref, memberRefs: [m.ref],
  ...
  const assess = (role: string, evidenceRole: string, surfaceRef: string, ruleRef: string | null): { disposition: "passed" | "failed" | "blocked_incomplete" | "inapplicable_with_reason"; refs: string[] } | null => {
    ...
    const matches = (assessmentIndex.get([`qualification-role://abiogenesis/${role}@5`, evidenceRole, surfaceRef].join("\n")) ?? [])
      .filter(({ j, c }) => (ruleRef === null || c.ruleRef === ruleRef) && (() => {
          ...
          const required = scope !== undefined && role === "catalog" ? rule?.sourceRefs ?? []
            ...
            : role === "catalog" ? [catalog.catalogRef, ...catalog.sources.map(s => s.ref)]
            ...
            : role === "rule" ? [c.surfaceRef, ruleByRef.get(c.ruleRef)?.sourceRef,
              ...(verificationRule ? [verification?.recipe.ref, verification?.execution.ref] : [])]
            : [c.surfaceRef];
          return (scope === undefined || c.grouping !== undefined) &&
            (scope === undefined || role !== "rule" && role !== "inventory" || surface !== undefined && surface.memberRefs.every(ref => j.task.subjectMembers.some(m => m.ref === ref))) &&
            (!verificationRule || verification !== null && c.evidenceRefs.includes(verification.execution.ref)) &&
            ...
            (scope !== undefined || required.filter(ref => ref !== undefined && inventoryRefs.has(ref)).every(ref => j.task.subjectMembers.some(m => m.ref === ref)));
        })());
    ...
    return { disposition: matches.some(({ c }) => c.disposition === "falsified" || c.grouping === "falsified") ? "failed"
      : matches.some(({ c, j }) => c.disposition === "indeterminate" || c.applicability === "unknown" || c.grouping === "unknown" || c.residuals.length > 0 || j.raw.residuals.length > 0) ? "blocked_incomplete"
  ...
  if (scope === undefined) assessed("catalog_fidelity_assessment_required", "Derived catalog fidelity and full governing-source coverage require an admitted independent assessment.", "catalog", "catalog_fidelity", [catalog.catalogRef]);
  else for (const group of ruleGroups) assessed("catalog_fidelity_assessment_required", "Exact rule group fidelity, governing-source coverage and common scope require independent J.", "catalog", "catalog_fidelity", [catalog.catalogRef], group.groupRef);
  const inventory = input.inventory;
  if (inventory === null) add("subject_inventory_missing", "blocked_incomplete", "The subject inventory is absent.");
  else {
    if (inventory.coverage !== "complete_claim") add("subject_inventory_declared_incomplete", "blocked_incomplete", "The frozen inventory explicitly retains incomplete coverage.");
    ...
    if (scope === undefined) assessed("inventory_coverage_assessment_required", "Whole candidate inventory/classification completeness requires its source-grounded assessment.", "inventory", "inventory_coverage", inventory.selectedRoots);
    else for (const group of surfaceGroups) {
      assessed("inventory_coverage_assessment_required", "Complete explicit source scope and root ownership require source-grounded independent J.", "inventory", "inventory_coverage", [group.groupRef], inventory.inventoryRef);
      assessed("subject_member_classification_incomplete", "All group members, classified roles and common ownership require independent J.", "inventory", "inventory_classification", [group.groupRef], inventory.inventoryRef);
    }
    for (const member of inventory.members) {
      ...
      if (scope === undefined) assessed("subject_member_classification_incomplete", "Member needs actual classification evidence.", "inventory", "inventory_classification", [member.ref]);
    }
    if (input.qualification !== undefined) {
      const actual = input.qualification.sourceMembers;
      const actualByRef = new Map(actual.map(m => [m.ref, m]));
      if (!unique(actual.map(m => m.ref)) || actual.length !== inventory.members.length || actual.some(m => !inventoryRefs.has(m.ref)))
      ...
      for (const m of inventory.members) {
        const bytes = actualByRef.get(m.ref), raw = bytes === undefined ? null : Buffer.from(bytes.contentBase64, "base64");
