...
| `3` | `T-272` | **Typed `reprice_required` stop accepted at `f611a72d`; external ticket traversal proven at `f79e1358`; S03 open:** the separately packed developer Product publishes its action catalog, four-authority construction composition, and an independently owned ticket Program and GraphFunction. ABG preserves Product-derived `gap_stop` and `reprice_required`, exact single-use re-entry, durable projections, and ticket result truth through the same generic path. Product-selected graph-span re-entry and remaining S03 consequence and runtime-disposition behavior remain. |
