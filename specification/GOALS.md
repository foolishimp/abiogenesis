...
for this wave. T-244 is the sole derived feature traceability and closure
register over that authority. Delivery neither narrows a retained
constitutional claim by silence nor adds a capability outside the lawful
scope. A new product capability re-enters through the smallest lawful goal,
intent, product, or requirement reprice.
...
| `DS-1` executable Consensus design probe | GTL, GraphFunction, C-algebra, and public-contract law | One admitted Consensus GTL body run through the ABG semantic compiler | The exact typed gap census is retained; no plugin, service, shell, or script supplies a missing construct. |
...
| `DS-3` complete generic C runtime | Accepted `workflow.C`, `C.batch`, and `C.retry` designs | Singular generic atom realizations | The same Consensus body recompiles after each atom; each atom also proves a non-Consensus consumer family. |
| `DS-4` bounded Consensus product | Consensus requirements and accepted GTL design | Published graph body, schemas, profiles, result projection, and installed scenarios | A caller invokes Consensus through `abg.cli` on existing, alternate, and caller-created temporary workspaces; converge, recurse, and F_H escalation outcomes are replay-citable. |
| `DS-5` complete retained 5.0 product | Constitutional requirements and accepted designs, traced by T-244 | Singular realization leaves | The complete SDK/CLI, catalog, interactive operator, schema, vocabulary, corpus, observer, tuner, and compatibility surfaces close at their row-specific gates. |
