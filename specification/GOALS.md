...
| current work carrier | `T-286` implementation migration |
| current constitutional state | accepted Product, requirements, correction vector, and direct-GTL design |
| implementation state | active only for exact `ABI5-ROOT-001`; RC5, X, and final-integration remain sideways donor evidence |
...
| `M4` | Establish installed root | Exact `ABI5-ROOT-001` runs green through packed bytes, clean install, direct HoG traversal, ABG replay twice, and typed CLI result. | Active under T-286; sunny root reached `R10` at `39a0b6bc`; exact review at `16bca623` found bounded artifact-admission, durable/scoped replay, failure-totalization, rival-path, and retained-evidence defects; repair in progress before refreeze |
...
`T-283`, `T-284`, and `T-285` are complete. `T-286` is the sole executable work
owner during M4.
...
During M4, every status report shall distinguish:
