...
**REQ-R-ABG3-INSTRUCTION-ASSEMBLY-005**: An F_P dispatch shall not occur without an admitted compiled prompt plan, an immutable runtime-bound instruction envelope, and a replayable prompt manifest or equivalent projection that preserves the compiled plan ref, bound runtime refs, renderer identity, response contract, and rendered prompt digest.
