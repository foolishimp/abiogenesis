...
**REQ-R-ABG3-WITNESS-009**: There shall be ONE operator-command/event grammar. Every operator interaction is a typed operator command; every command admits actor-attributed events; operator acts route through the grammar or they did not happen. The abg CLI is the grammar's REFERENCE ADAPTER, not the grammar itself: alternative operator surfaces (dashboards, APIs, managers) are lawful only as adapters emitting the same admitted operator events through the same admission. A surface that bypasses the grammar is not an operator surface; tests driving the harness in-process are not operators.
