...
Gap: typed basis-fork results at operator routes are unrealized — the routes refuse the fork but fail untyped (a raw thrown error carrying the `basis_fork_detected` reason) rather than returning typed blocked truth. Owner: T-217 (the active 4.6 wave).
