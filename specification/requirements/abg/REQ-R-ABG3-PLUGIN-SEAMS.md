...
Gap: per-instance approval attribution is not realized — an operator invocation that injects a capability row carries no per-instance actor identity, approval ref, or admitted approval event; ratification lives at the capability class only. Owner: T-217 (the active 4.6 wave).
...
Gap: expected assessment identities are not rendered into the PromptManifest — corroboration is engine-internal and the worker attests identities it never sees declared. Two acceptance-by-omission paths also remain open: the review is admitted through a tolerant private parser (JSON-object extraction from free text, `closeDisposition` defaulted from `accepted`, absent `assessmentIds` tolerated as empty) rather than a declared schema; and when no expected assessment identities derive, a bare `{accepted: true}` review is close-eligible with no attestation at all. Owner: T-217 (the active 4.6 wave).
