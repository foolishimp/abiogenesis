...
Gap: per-instance approval attribution is not realized — an operator invocation that injects a capability row carries no per-instance actor identity, approval ref, or admitted approval event; ratification lives at the capability class only. Owner: T-218 intake pending DS-0 delivery-leaf assignment.
