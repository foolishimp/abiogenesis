...
- AC-6: **Degenerate case:** when no work_keys are active, reports per-edge only
...
**Acceptance Criteria**:
- AC-1: Selects first unconverged work instance in topological edge order. Each `(job, work_key)` pair is a candidate. **Degenerate case:** when work_keys are absent, each job is a single candidate with `work_key=None`
...
**Acceptance Criteria**:
- AC-1: `edge_converged` event data includes `work_key` field. **Degenerate case:** when `work_key` is absent, the `feature` field is used for backward-compatible certificate identification
- AC-2: Deduplication key is `(edge, work_key)` — same edge can converge separately for different work units. **Degenerate case:** when `work_key` is absent, deduplication falls back to `(edge, feature)`
